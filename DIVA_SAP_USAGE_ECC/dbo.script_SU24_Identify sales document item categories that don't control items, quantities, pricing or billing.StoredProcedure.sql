...
EXEC SP_REMOVE_TABLES 'SU24_02_TT_VBAP_SALE_ORDER'
		...
		WHERE B33_VBAK_VBELN+B33_VBAP_POSNR IN
		(
			SELECT DISTINCT B36_VBRP_AUBEL+B36_VBRP_AUPOS FROM B36_01_IT_INVOICE_DOCS
		)
-- Step 2: Create SD invoices cube
--Get only the cases relate to sale orders
EXEC SP_REMOVE_TABLES 'SU24_03_TT_SD_INV'
		SELECT DISTINCT B36_01_IT_INVOICE_DOCS.*,
		...
		ON B36_VBRK_ERNAM = V_USERNAME_BNAME
		...
		WHERE B36_VBRP_AUBEL+B36_VBRP_AUPOS IN 
		(
...
SELECT B36_VBRP_AUPOS,B36_VBRP_AUBEL,
		SUM (B36_ZF_VBRP_NETWR_S_CUC) AS ZF_ZF_VBRP_NETWR_S_CUC_SUM,
		SUM (B36_VBRP_FKIMG) AS ZF_VBRP_FKIMG_SUM
INTO SU24_04_TT_VBRK_VBRP_GROUP_BY_AUBEL_AUPOS
FROM SU24_03_TT_SD_INV
GROUP BY B36_VBRP_AUBEL,B36_VBRP_AUPOS

--Step 3.2 Link to sale document to do compare

EXEC SP_DROPTABLE 'SU24_05_TT_VBRK_VBRP_COMPARE_VBAP_VBAK'

SELECT  B36_VBRP_AUPOS,
		B36_VBRP_AUBEL,
		...
		B33_VBAK_VBELN,
		B33_VBAP_ZMENG,
		B33_ZF_VBAP_NETWR_S_CUC,
		CASE 
			WHEN ZF_ZF_VBRP_NETWR_S_CUC_SUM>B33_ZF_VBAP_NETWR_S_CUC THEN 'Higher'
			WHEN ZF_ZF_VBRP_NETWR_S_CUC_SUM=B33_ZF_VBAP_NETWR_S_CUC THEN 'Equal'
			WHEN ZF_ZF_VBRP_NETWR_S_CUC_SUM<B33_ZF_VBAP_NETWR_S_CUC THEN 'Lower'
        END AS ZF_COMPARE_VBRP_VBAP_NETWR,
		CASE 
			WHEN ZF_VBRP_FKIMG_SUM>B33_VBAP_ZMENG THEN 'Higher'
			WHEN ZF_VBRP_FKIMG_SUM=B33_VBAP_ZMENG THEN 'Equal'
			WHEN ZF_VBRP_FKIMG_SUM<B33_VBAP_ZMENG THEN 'Lower'
...
ON B36_VBRP_AUPOS=B33_VBAP_POSNR AND 
   B36_VBRP_AUBEL=B33_VBAK_VBELN
...
LEFT JOIN SU24_05_TT_VBRK_VBRP_COMPARE_VBAP_VBAK AS B
	ON A.B33_VBAP_POSNR=B.B33_VBAP_POSNR AND
		A.B33_VBAK_VBELN=B.B33_VBAK_VBELN
LEFT JOIN SU24_01_XT_TVAP_SALE_DOC_ITEM_CATEGORIES_SETTING
ON TVAP_PSTYV = B33_VBAP_PSTYV
...
LEFT JOIN SU24_05_TT_VBRK_VBRP_COMPARE_VBAP_VBAK AS B
	ON A.B36_VBRP_AUBEL=B.B36_VBRP_AUBEL AND
		A.B36_VBRP_AUPOS=B.B36_VBRP_AUPOS
LEFT JOIN SU24_06_RT_VBAK_VBAP_ADD_FLAG AS C
ON C.B33_VBAK_VBELN = A.B36_VBRP_AUBEL
AND C.B33_VBAP_POSNR = A.B36_VBRP_AUPOS

-- Remove temporary table
EXEC SP_REMOVE_TABLES 'SU24_%[_]TT[_]%'

-- Unname the fields
EXEC SP_UNNAME_FIELD 'B33_', 'SU24_06_RT_VBAK_VBAP_ADD_FLAG'
EXEC SP_UNNAME_FIELD 'B36_', 'SU24_07_RT_VBRK_VBRP_ADD_FLAG'
