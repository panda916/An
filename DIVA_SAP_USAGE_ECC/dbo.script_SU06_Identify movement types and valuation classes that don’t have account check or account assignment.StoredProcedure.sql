...
--STep 4 Filter BSEG/BKPF of BKPF_BLART = WE, 
--keep only BSEG_BUKRS, BSEG_GJAHR, BSEG_BELNR, BSEG_BUZEI, BSEG_HKONT, BKPF_AWKEY, 
--and 
EXEC SP_DROPTABLE SU06_04_TT_BSEG_BKPF_BLART_EQ_WE

 SELECT DISTINCT B04_11_IT_FIN_GL.*,T001_BUTXT,T003T_LTEXT,USER_ADDR_NAME_TEXTC,SKAT_TXT50
 INTO SU06_04_TT_BSEG_BKPF_BLART_EQ_WE
 FROM B04_11_IT_FIN_GL
 LEFT JOIN A_T001 ON T001_BUKRS=B04_BSEG_BUKRS
 LEFT JOIN A_USER_ADDR ON B04_BKPF_USNAM=USER_ADDR_BNAME
 LEFT JOIN A_LFA1 ON LFA1_LIFNR=B04_BSEG_LIFNR
LEFT JOIN A_T003T ON B04_BKPF_BLART=T003T_BLART
LEFT JOIN A_SKAT ON B04_BSEG_HKONT=SKAT_SAKNR AND SKAT_SPRAS='EN'
 WHERE B04_BKPF_BLART='WE' AND B04_BKPF_AWTYP='MKPF'
...
--Step 7 Add to BKPF/BSEG infor about Good receipt
...
EXEC SP_DROPTABLE SU06_07_RT_BSEG_BKPF_ADD_FLAG

SELECT DISTINCT SU06_04_TT_BSEG_BKPF_BLART_EQ_WE.*,
'' AS ZF_BSEG_HKONT_EQ_EKKN_SAKTO,
'' AS ZF_BSEG_HKONT_EQ_T030_KONTH,
'' AS ZF_BSEG_HKONT_EQ_T030_KONTS
INTO SU06_07_RT_BSEG_BKPF_ADD_FLAG
FROM SU06_04_TT_BSEG_BKPF_BLART_EQ_WE
INNER JOIN BP03_01_IT_GR AS A
ON 
B04_ZF_BKPF_AWKEY_YEAR=A.BP03_01_MKPF_MJAHR  AND
 B04_ZF_BKPF_AWKEY_DOC_NUM=A.BP03_01_MKPF_MBLNR

--Update the flag, check if BSEG_HKONT is found in T030 (KONTH)
UPDATE SU06_07_RT_BSEG_BKPF_ADD_FLAG
SET ZF_BSEG_HKONT_EQ_T030_KONTH='X'
WHERE B04_BSEG_HKONT IN
(
		SELECT DISTINCT SU06_02_T030_KONTH
		FROM SU06_06_TT_GR_ADD_T030  INNER JOIN SU06_07_RT_BSEG_BKPF_ADD_FLAG 
		ON B04_ZF_BKPF_AWKEY_YEAR=BP03_01_MKPF_MJAHR  AND
			B04_ZF_BKPF_AWKEY_DOC_NUM=BP03_01_MKPF_MBLNR
)
--Update the flag, check if BSEG_HKONT is found in T030 (KONTS)
UPDATE SU06_07_RT_BSEG_BKPF_ADD_FLAG
SET ZF_BSEG_HKONT_EQ_T030_KONTS='X'
WHERE B04_BSEG_HKONT IN
(
...
--Update the flag, check if BSEG_HKONT is found in EKKN_SAKTO
UPDATE SU06_07_RT_BSEG_BKPF_ADD_FLAG
SET ZF_BSEG_HKONT_EQ_EKKN_SAKTO='X'
WHERE B04_BSEG_HKONT IN
(
		SELECT DISTINCT EKKN_SAKTO
		FROM SU06_05_TT_GR_ADD_EKKN  INNER JOIN SU06_07_RT_BSEG_BKPF_ADD_FLAG 
		ON B04_ZF_BKPF_AWKEY_YEAR=BP03_01_MKPF_MJAHR  AND
			B04_ZF_BKPF_AWKEY_DOC_NUM=BP03_01_MKPF_MBLNR
)
...
EXEC SP_UNNAME_FIELD 'B04_','SU06_07_RT_BSEG_BKPF_ADD_FLAG'
EXEC SP_RENAME_FIELD 'SU06_07_','SU06_07_RT_BSEG_BKPF_ADD_FLAG'
