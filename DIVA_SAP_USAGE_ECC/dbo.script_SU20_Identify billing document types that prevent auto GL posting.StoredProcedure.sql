...
-- Step 2 Find the items that are in VBRP/VBRK and that are not found in BSAD/BSID
	...
	SELECT 
		A.*, 
		CONCAT(DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE), ' day(s)') AS ZF_DOWN_DIFF_CRE_DATE_D,
		CASE
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 31 THEN '0-30 days'
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) > 30 AND DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 91 THEN '30-90 days'
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) > 90 AND DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 181 THEN '90-180 days'
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) > 180 AND DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 366 THEN '180-365 days'
			ELSE '>365 days'
		END AS ZF_DOWN_DIFF_CRE_BUCKET,
		CASE
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 31 THEN 1
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) > 30 AND DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 91 THEN 2
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) > 90 AND DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 181 THEN 3
			WHEN DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) > 180 AND DATEDIFF(DAY,B36_VBRK_ERDAT, @DOWNLOADDATE) < 366 THEN 4
			ELSE 5
		END AS ZF_DOWN_DIFF_CRE_BUCKET_ORDER
	INTO SU20_02_XT_TVFK_IN_VBRP_VBRK_NOT_IN_BSAD_BID
	FROM B36_01_IT_INVOICE_DOCS A
	LEFT JOIN A_BKPF B
		ON A.B36_VBRK_VBELN = B.BKPF_AWKEY
	LEFT JOIN SU20_01_RT_TVFK_BILL_DOC_POSTING_BLOCK D
		ON D.TVFK_FKART = A.B36_VBRK_FKART
...
EXEC SP_UNNAME_FIELD 'B36_', 'SU20_02_RT_TVFK_IN_VBRP_VBRK_NOT_IN_BSAD_BID' 
-- Rename the fields
