...
GO
CREATE PROCEDURE [dbo].[MERGE_script_B36_INVOICE_DOCS]
...
/*  Change history comments
	...
	-- 07-08-2024	| Nam	| Add fields and joins from SAP USAGE script_BO04A_SD_INVOICES
*/
...
--	Step 9: Extract all invoice document from VBRK AND VBRP base on VBFA sale document flow table
	...
	EXEC SP_REMOVE_TABLES 'B36_01_IT_INVOICE_DOCS'
	SELECT DISTINCT 
		   VBRK_MANDT,
		   VBRK_VBELN,
		   VBRP_POSNR, -- Item nr
		 ...
		 B00_DD07T_FKTYP.DD07T_DDTEXT ZF_VBRK_FKTYP_DESC, -- Get billing category desc
		   ...
		   (VBRP_NETWR)*ISNULL(B00_TCURX.TCURX_FACTOR,1) * VBRP_KURSK * COALESCE(TCURF_COC.TCURF_TFACT,1)/COALESCE(TCURF_COC.TCURF_FFACT,1) AS ZF_VBRP_NETWR_S_COC, -- Item net value
...
--		   VBRP_BZIRK_AUFT,-- Sale district
		   ...
		   VBRP_VBELN, -- Billing document
		   T001_WAERS, -- Currency key

		   -- Nam: Add from SAP Usage
		   VBRK_BELNR, -- Document number,
		   VBRK_GJAHR, -- Fiscal year,
		   VBRK_BUKRS, -- Company code
		   VBRK_VBTYP, -- SD document category
		   AM_SCOPE.SCOPE_BUSINESS_DMN_L1,
		   AM_SCOPE.SCOPE_BUSINESS_DMN_L2,
		   T001_BUTXT, -- Name of Company Code or Company
		   T001_BUKRS, -- Company code
	...
	EXEC SP_RENAME_FIELD 'B36_',B36_01_IT_INVOICE_DOCS
