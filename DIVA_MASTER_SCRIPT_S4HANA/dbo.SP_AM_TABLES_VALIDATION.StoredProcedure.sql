...
BEGIN
	...
	*/
	...
	/*
		Step 8: Check and raise an error if we get new value in AM_TANGO table which have not been updated hierachy
	*/
	...
	/*
		Step 9: Check and raise an error if we get new value in AM_SPEND_CATEGORY table which have not been updated hierachy
	*/
	...
	/*
		Step 10: Check and raise an error if any gl account in AM_IRGR_GINI_ARDR_MAPPING have appear more than one time.
	*/
	...
	/*
		Step 11: Check and raise an error if any gl account in AM_IRGR_GINI_ARDR_MAPPING have appear more than one time.
	*/
	...
	/*
		Step 12: Check and raise an error if we get new value in AM_SPEND_CATEGORY table which have not been updated hierachy
	*/
	...
	/*
		Step 13: Check and raise an error if we get new value in AM_COMPANY_CODE table which have not been updated hierachy
	*/
	IF EXISTS (SELECT * FROM AM_COMPANY_CODE WHERE BUKRS_MAPPING IS NULL )
	BEGIN
		RAISERROR('***CHECK BUKRS_MAPPING column in AM_COMPANY_CODE again***',	20,	1) WITH LOG
	END

	/*
		Step 14: Check and raise an error if any gl account in AM_AP_CLEARING have appear more than one time.
	*/

	--IF EXISTS (SELECT * FROM AM_AP_CLEARING GROUP BY ZF_ACCOUNTS HAVING COUNT (DISTINCT ZF_ACCOUNTS) > 1)
	--BEGIN
	--	RAISERROR('***DUPLICATE GL ACCOUNTs IN AM_AP_CLEARING***',	20,	1) WITH LOG
	--END

	/*
		Step 15: Check and raise an error if we get new value in AM_AP_CLEARING table which have not been updated AP CLEARING is valid
	*/

	--IF EXISTS (SELECT * FROM AM_AP_CLEARING WHERE ZF_AP_CLEARING IS NULL OR LEN(ZF_AP_CLEARING) = 0)
	--BEGIN
	--	RAISERROR('***NULL VALUE IN ZF_AP_CLEARING COLUMN IN AM_AP_CLEARING TABLE***',	20,	1) WITH LOG
	--END

	/*
		Step 16: Check and raise an error if any gl account in AM_AR_CLEARING have appear more than one time.
	*/

	--IF EXISTS (SELECT * FROM AM_AR_CLEARING GROUP BY ZF_ACCOUNTS HAVING COUNT (DISTINCT ZF_ACCOUNTS) > 1)
	--BEGIN
	--	RAISERROR('***DUPLICATE GL ACCOUNTs IN AM_AR_CLEARING TABLE***',	20,	1) WITH LOG
	--END

	/*
		Step 17: Check and raise an error if we get new value in AM_AR_CLEARING table which have not been updated AR CLEARING is valid
	*/

	--IF EXISTS (SELECT * FROM AM_AR_CLEARING WHERE ZF_AR_CLEARING IS NULL OR LEN(ZF_AR_CLEARING) = 0)
	--BEGIN
	--	RAISERROR('***NULL VALUE IN AM_AR_CLEARING COLUMN IN ZF_AR_CLEARING TABLE***',	20,	1) WITH LOG
	--END

	/*
		Step 18: Check and raise an error if any gl account in AM_AR_ACCOUNT have appear more than one time.
	*/

	--IF EXISTS (SELECT * FROM AM_AR_ACCOUNT GROUP BY ACDOCA_RACCT HAVING COUNT (DISTINCT ACDOCA_RACCT) > 1)
	--BEGIN
	--	RAISERROR('***DUPLICATE GL ACCOUNTs IN AM_AR_ACCOUNT TABLE***',	20,	1) WITH LOG
	--END

	/*
		Step 19: Check and raise an error if we get new value in  table AM_AR_ACCOUNT which have not been updated AR ACCOUNT is valid
	*/

	--IF EXISTS (SELECT * FROM AM_AR_ACCOUNT WHERE ZF_AR_ACCOUNT IS NULL OR LEN(ZF_AR_ACCOUNT) = 0)
	--BEGIN
	--	RAISERROR('***NULL VALUE IN ZF_AR_ACCOUNT COLUMN IN AM_AR_ACCOUNT TABLE***',	20,	1) WITH LOG
	--END

	/*
		Step 20: Check and raise an error if we get new value in  table AM_COPA_MAPPING which have not been updated Level is valid
	*/

	IF EXISTS (SELECT * FROM AM_COPA_MAPPING WHERE COPA_RACCT_Hierarchy_L1 IS NULL )
	BEGIN
		RAISERROR('***NULL VALUE IN COPA_RACCT_Hierarchy_L1 COLUMN IN AM_COPA_MAPPING TABLE***',	20,	1) WITH LOG
	END
