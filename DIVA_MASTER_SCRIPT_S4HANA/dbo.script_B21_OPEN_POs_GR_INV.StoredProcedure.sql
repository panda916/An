...
SELECT DISTINCT 
	...
	,CONVERT(money,A.EKBE_DMBTR * COALESCE(TCURX_CC.TCURX_Factor,1) * (CASE WHEN (A.EKBE_SHKZG = 'S') THEN 1 ELSE -1 END) * COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1))	AS ZF_EKBE_WBRTR_S_CUC,
...
--CREATE INDEX GL_BUKRS_EBELN_EBELP ON B04_08_IT_FIN_GL(B04_BSEG_BUKRS,B04_BSEG_EBELN,B04_BSEG_EBELP);
			...
			SELECT DISTINCT 
				...
				B04_GL_ZF_ACDOCA_HSL_S,
				B04_GL_ZF_ACDOCA_HSL_S_CUC,
				B04_GL_ACDOCA_RBUKRS,
				BUKRS_MAPPING as ZF_GL_BUKRS_MAPPING -- Thuan update 2023-12-07 add BUKRS_MAPPING for GL we used in transform Qliksense
			INTO B21_13_IT_BSEG_INFO
			FROM  B04_08_IT_FIN_GL
			LEFT JOIN A_T001                                             
				ON A_T001.T001_BUKRS = B04_08_IT_FIN_GL.B04_GL_ACDOCA_RBUKRS 
			-- Thuan update 2023-12-07
			LEFT JOIN AM_COMPANY_CODE
			ON B04_08_IT_FIN_GL.B04_GL_ACDOCA_RBUKRS  = COMPANY_CODE
			-- Add GL acoounts text.
			LEFT JOIN A_SKAT
				ON  A_SKAT.SKAT_SAKNR = B04_08_IT_FIN_GL.B04_GL_ACDOCA_RACCT AND
					A_SKAT.SKAT_KTOPL = A_T001.T001_KTOPL AND
					A_SKAT.SKAT_SPRAS = 'EN'
			-- Add document type
			LEFT JOIN B00_T003T
					   ON B04_GL_ACDOCA_BLART = B00_T003T.T003T_BLART
			LEFT JOIN B00_TBSLT
						ON  B04_GL_ACDOCA_UMSKZ = B00_TBSLT.TBSLT_UMSKZ 
						AND B04_GL_ACDOCA_BSCHL = B00_TBSLT.TBSLT_BSCHL
			--Get Spend category
			...
			WHERE EXISTS(
					...
					-- Thuan update 2023-12-07 link PO with AM_COMPANY_CODE based on BUKRS then check with GL based on BUKRS_MAPPING
					LEFT JOIN AM_COMPANY_CODE B
						ON B21_12_TT_CR_6M_PR_EXT.B09_EKPO_BUKRS  = B.COMPANY_CODE
					WHERE AM_COMPANY_CODE.BUKRS_MAPPING = B.BUKRS_MAPPING
					AND B04_GL_ACDOCA_EBELN = B09_EKPO_EBELN
				)
...
B21_12_TT_CR_6M_PR_EXT.*,
	...
	BUKRS_MAPPING as ZF_PO_BUKRS_MAPPING -- Thuan update 2023-12-07 add BUKRS_MAPPING for PO we used in transform Qliksense
