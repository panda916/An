...
GO
CREATE       PROCEDURE [dbo].[script_B27_PTP_GLOBAL_CUBES_MATDOC]
AS
...
CONCAT(DBO.TRIM(B26_02_MATDOC_MBLNR),'-',
DBO.TRIM(B26_02_MATDOC_MJAHR),'-',
DBO.TRIM(B26_02_MATDOC_ZEILE)) AS ZF_MATDOC_EBELN_EBELP_BUKRS,--Good receipt key
B26_02_MATDOC_USNAM AS MATDOC_USNAM,--Good receipt user name
B26_02_MATDOC_CPUDT AS MATDOC_CPUDT,--Good receipt date
B26_02_MATDOC_CPUTM AS MATDOC_CPUTM,
B26_02_MATDOC_LIFNR AS MATDOC_LIFNR,--Good receipt supplier
B26_02_LFA1_ERNAM AS LFA1_ERNAM_MATDOC,--User who create supplier
B26_02_V_USERNAME_NAME_TEXT AS V_USERNAME_NAME_TEXT_MATDOC,--GR name of the user
B26_02_ZF_MATDOC_DMBTR_SIGNED AS ZF_MATDOC_DMBTR_SIGNED,
B26_02_ZF_MATDOC_DMBTR_SIGNED_CUC AS ZF_MATDOC_DMBTR_SIGNED_CUC,
B26_02_MATDOC_BUKRS AS MATDOC_BUKRS,
B26_02_MATDOC_WAERS AS MATDOC_WAERS,
B26_02_T003T_LTEXT AS MATDOC_T003T_LTEXT,
...
CONVERT(MONEY,(RSEG_WRBTR *RBKP_KURSF*  COALESCE(TCURX_FACTOR,1) *
		COALESCE(TCURF_TFACT,1))/COALESCE(TCURF_FFACT,1) 
* iif(RSEG_SHKZG = 'H', -1, 1)) AS ZF_RSEG_WRBTR_S_COC,
...
FULL OUTER JOIN B26_02_IT_GR_CUBE
 ON B26_02_MATDOC_EBELN=B09_SS02_EKPO_EBELN
AND B26_02_MATDOC_EBELP=B09_SS02_EKPO_EBELP
AND B26_02_MATDOC_BUKRS_MAPPING=B09_SS02_EKPO_BUKRS_MAPPING
--Join PR_PO_GR_INV with Payment table,base on AUGBL and AUGDT

FULL OUTER JOIN 
(SELECT A_RSEG.*,
		A_RBKP.*,
		V_USERNAME_NAME_TEXT, T001_WAERS AS  RBKP_T001_WAERS,
		T003T_LTEXT, TCURF_COC.TCURF_FFACT,TCURF_COC.TCURF_TFACT,
		RBKP_KURSF * 
		...
		* COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1) 
		AS ZF_EXCHNG_RATIO,
		TCURX_FACTOR,
		AM_COMPANY_CODE.BUKRS_MAPPING AS RSEG_BUKRS_MAPPING
		FROM A_RSEG
...
JOIN AM_COMPANY_CODE ON RSEG_BUKRS=COMPANY_CODE
	-- An update add currency for A_T001

	LEFT JOIN B00_03_IT_T001_RMV_DUP
	ON A_RSEG.RSEG_BUKRS = ZF_COMPANY_CODE
...
ON A.RSEG_EBELN=
	CASE 
		WHEN B09_SS02_EKPO_EBELN IS NOT NULL THEN B09_SS02_EKPO_EBELN ELSE B26_02_MATDOC_EBELN
	END 
AND A.RSEG_EBELP=
	CASE 
		WHEN B09_SS02_EKPO_EBELP IS NOT NULL THEN B09_SS02_EKPO_EBELP ELSE B26_02_MATDOC_EBELP
	END 
AND A.RSEG_BUKRS_MAPPING=
	CASE 
		WHEN B09_SS02_EKPO_BUKRS_MAPPING IS NOT NULL THEN B09_SS02_EKPO_BUKRS_MAPPING ELSE B26_02_MATDOC_BUKRS_MAPPING
	END 
...
B.B04_GL_ACDOCA_CPUDT AS ACDOCA_CPUDT_INV,--Invoice input date
B.B04_GL_ACDOCA_CPUTM AS ACDOCA_CPUTM_INV,
...
EXEC SP_DROPTABLE B27_01_04_TT_PO_GR_INCO_INV

SELECT DISTINCT *
INTO B27_01_04_TT_PO_GR_INCO_INV
FROM
(
	SELECT *
...
DELETE FROM B27_01_04_TT_PO_GR_INCO_INV
WHERE ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI IN
(
SELECT DISTINCT ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI
FROM B27_01_03_TT_INV_PO
WHERE EKPO_EBELN IS NOT NULL AND ACDOCA_BUKRS_INV IS NOT NULL
AND ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI IN
(
	SELECT ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI
	FROM B27_01_02_TT_INCOMING_INV
	WHERE RSEG_BUKRS IS NULL AND ACDOCA_BUKRS_INV IS NOT NULL
)
)
...
(
	SELECT DISTINCT ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI
	FROM B27_01_02_TT_INCOMING_INV
	WHERE RSEG_BUKRS IS NOT NULL AND ACDOCA_BUKRS_INV IS NOT NULL
AND ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI IN
(
	SELECT ZF_ACDOCA_BUKRS_GJAHR_BELNR_BUZEI
	FROM B27_01_03_TT_INV_PO 
	WHERE EKPO_EBELN IS NULL AND ACDOCA_BUKRS_INV IS NOT NULL
)
)
...
C.B11B_SS02_05_ACDOCA_CPUDT AS ACDOCA_CPUDT_PAY,--Payment date
C.B11B_SS02_05_ACDOCA_CPUTM AS ACDOCA_CPUTM_PAY,--Payment date
...
C.B11B_SS02_05_T001_WAERS AS T001_WAERS_PAY
-- Join PR_PO_GR_INV with Payment table,base on AUGBL and AUGDT
INTO B27_01_IT_PTP_GLOBAL_TABLE
FROM B27_01_04_TT_PO_GR_INCO_INV
FULL OUTER JOIN B11_SS02_05_IT_PTP_APA_PAY_GROUP AS C
	ON B27_01_04_TT_PO_GR_INCO_INV.B04_GL_ACDOCA_AUGBL =  C.B11B_SS02_05_ACDOCA_AUGBL 
	AND B27_01_04_TT_PO_GR_INCO_INV.B04_GL_ACDOCA_AUGDT=  C.B11B_SS02_05_ACDOCA_AUGDT 
	AND B27_01_04_TT_PO_GR_INCO_INV.B04_GL_ACDOCA_BUKRS_MAPPING= C.B11B_SS02_05_ACDOCA_BUKRS_MAPPING
	AND (B27_01_04_TT_PO_GR_INCO_INV.B04_GL_ACDOCA_AUGBL <> '' AND  C.B11B_SS02_05_ACDOCA_AUGBL <> '')
