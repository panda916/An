...
GO
CREATE        PROC [dbo].[script_B19_BANK_CHANGE]
...
--Step 1: join the CDHDR with CDPOS and add filters to get relevant changes to LFBK, TIBAN/IBAN table
	...
	EXEC SP_REMOVE_TABLES 'B19%TT%'

	EXEC SP_DROPTABLE 'B19_01_TT_CDHDR_CDPOS'
    ...
    IIF(CDPOS_TABNAME IN ('TIBAN','LFBK', 'LFA1', 'KNA1'), SUBSTRING(CDPOS_TABKEY, 4, 10), NULL) AS ZF_CUSTOMER_SUPPLIER_KEY
	INTO B19_01_TT_CDHDR_CDPOS
	FROM A_CDHDR A
	...
	WHERE CDPOS_TABNAME IN ('LFBK', 'TIBAN',  'LFA1', 'KNA1') AND CDHDR_OBJECTID <> ''


	--Step 2: create a scope table that contain all change cases with both delete and insert.
	EXEC SP_DROPTABLE 'B19_02_TT_CDHDR_CDPOS_LFBK_SCOPE'
	SELECT DISTINCT A.CDHDR_OBJECTID, A.CDHDR_CHANGENR, A.CDHDR_OBJECTCLAS 
	INTO B19_02_TT_CDHDR_CDPOS_LFBK_SCOPE
	FROM B19_01_TT_CDHDR_CDPOS A
	WHERE CDPOS_CHNGIND = 'I'
		AND CDPOS_FNAME = 'KEY'
		AND EXISTS(SELECT * FROM B19_01_TT_CDHDR_CDPOS B WHERE A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
...
*/
	EXEC SP_DROPTABLE 'B19_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE'
	SELECT DISTINCT A.*
	INTO B19_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE
	FROM B19_01_TT_CDHDR_CDPOS A
	INNER JOIN B19_02_TT_CDHDR_CDPOS_LFBK_SCOPE B ON B.CDHDR_CHANGENR = A.CDHDR_CHANGENR
...
-- because when users change or delete key fields, SAP will update and 
	EXEC SP_DROPTABLE 'B19_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED'
	SELECT DISTINCT A.*,
	A.ZF_LAST_KEY_CHANGE ZF_VALUE_NEW,
	B.ZF_LAST_KEY_CHANGE ZF_VALUE_OLD

	INTO B19_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED
	FROM B19_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE A
	LEFT JOIN B19_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE B ON A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
															AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
...
-- Step 4: from the combined result above, perform a normal with the table itself to get flipflop cases (old value of table 1 should be the new change value of table 2)
	EXEC SP_DROPTABLE 'B19_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP'
	SELECT DISTINCT 
	...
	INTO B19_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP
	FROM B19_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED A
	INNER JOIN B19_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED B ON A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS 
														AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
														AND A.CDPOS_TABNAME = B.CDPOS_TABNAME 
...
-- This table will manage many to many relation ship, in case that users want to get all relevant data from original table, including all the lines between two change session.
	EXEC SP_DROPTABLE 'B19_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING'
	CREATE TABLE B19_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING
	(
...
-- Step 6: Insert all result to above table
	INSERT INTO B19_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING
	...
	ZF_CHANGE_ID,
	ZF_LIFNR_1
	FROM B19_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP A
	UNION
	...
	ZF_CHANGE_ID_2,
	ZF_LIFNR_1
	FROM B19_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP B
	UNION
	...
	ZF_LIFNR_1
	FROM B19_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP B
	LEFT JOIN B19_01_TT_CDHDR_CDPOS C ON B.CDHDR_OBJECTID = C.CDHDR_OBJECTID
...
-- Step 4: join the table with itself to get flipflop cases (old value of table 1 should be the new change value of table 2)
	EXEC SP_DROPTABLE 'B19_06_TT_CDHDR_CDPOS_IBAN'
	SELECT DISTINCT
	...
	INTO B19_06_TT_CDHDR_CDPOS_IBAN
	FROM B19_01_TT_CDHDR_CDPOS A
	INNER JOIN B19_01_TT_CDHDR_CDPOS B ON A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
															AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
															AND A.CDPOS_TABNAME = B.CDPOS_TABNAME
															AND A.CDPOS_FNAME = B.CDPOS_FNAME
...
-- Step 6: Insert all result to above table
	...
	ZF_CHANGE_ID,
	ZF_LFBK_LIFNR_OLD_1
	FROM B19_06_TT_CDHDR_CDPOS_IBAN A
	UNION
-- Insert all 2nd change (changing back to the bank account of the original)
	SELECT DISTINCT
	ZF_CASE_TYPE,
...
-- Step 7: join the CDHDR_CDPOS table with itself where the 2nd table has change indicator = D, E
	...
	SELECT DISTINCT
		...
		SUBSTRING(A.CDPOS_TABKEY, 4, 10) ZF_LIFNR_1,
		DBO.TRIM(SUBSTRING(A.CDPOS_TABKEY, 14, LEN(A.CDPOS_TABKEY)- 14)) ZF_BANK_DETAIL
	INTO B19_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE
	FROM B19_01_TT_CDHDR_CDPOS A
	INNER JOIN B19_01_TT_CDHDR_CDPOS B ON B.CDPOS_CHNGIND IN ('D', 'E') AND  A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
															AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
															AND A.CDPOS_TABNAME = B.CDPOS_TABNAME
															AND A.CDHDR_CHANGENR < B.CDHDR_CHANGENR
	...
	SELECT DISTINCT
		...
		ZF_CHANGE_ID,
		ZF_LIFNR_1
	FROM B19_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE A
	UNION
	SELECT DISTINCT
		ZF_CASE_TYPE,
		ZF_CHANGE_ID,
		ZF_CHANGE_ID_2,
		ZF_LIFNR_1
	FROM B19_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE B


								

	EXEC SP_DROPTABLE 'B19_01_IT_CDHDR_CDPOS'
	SELECT * INTO B19_01_IT_CDHDR_CDPOS FROM B19_01_TT_CDHDR_CDPOS
    CREATE NONCLUSTERED INDEX IDX ON B19_01_IT_CDHDR_CDPOS(ZF_CUSTOMER_SUPPLIER_KEY)
	EXEC SP_DROPTABLE 'B19_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP'
	SELECT * INTO B19_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP FROM B19_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP
	EXEC SP_DROPTABLE 'B19_06C_IT_CDHDR_CDPOS_IBAN'
	SELECT * INTO B19_06C_IT_CDHDR_CDPOS_IBAN FROM B19_06_TT_CDHDR_CDPOS_IBAN

	EXEC SP_DROPTABLE 'B19_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE'
	SELECT * INTO B19_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE FROM B19_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE


	EXEC SP_RENAME_FIELD 'B19_', 'B19_01_IT_CDHDR_CDPOS'
	EXEC SP_RENAME_FIELD 'B19A_', 'B19_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP'
	EXEC SP_RENAME_FIELD 'B19B_', 'B19_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING'
	EXEC SP_RENAME_FIELD 'B19C_', 'B19_06C_IT_CDHDR_CDPOS_IBAN'
	EXEC SP_RENAME_FIELD 'B19E_', 'B19_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE'

	DECLARE @CURRENCY NVARCHAR(MAX) = (SELECT GLOBALS_VALUE FROM [AM_GLOBALS] WHERE GLOBALS_PARAMETER = 'currency')

	EXEC SP_DROPTABLE 'B19_10_TT_AP_DETAIL'
	SELECT A.*,
	B.B19A_ZF_CASE_TYPE ZF_CASE_TYPE,
	B.B19A_ZF_CHANGE_ID ZF_CHANGE_ID
	INTO B19_10_TT_AP_DETAIL
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B19_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP B ON B11B_ACDOCA_LIFNR = B.B19A_ZF_LIFNR_1
												AND B11B_ACDOCA_BLDAT BETWEEN B19A_ZF_CDHDR_UDATE_1 AND B.B19A_ZF_CDHDR_UDATE_2
                                                AND B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'

	UNION
	SELECT A.*,
	C.B19E_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B19E_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B19_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE C ON B11B_ACDOCA_LIFNR = C.B19E_ZF_LIFNR_1
												AND B11B_ACDOCA_BLDAT BETWEEN c.B19E_ZF_CDHDR_UDATE_1 AND C.B19E_ZF_CDHDR_UDATE_2
                                                AND B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	C.B19C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_ACDOCA_LIFNR = B19C_ZF_LFBK_LIFNR_NEW_1
												AND B11B_ACDOCA_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
                                                AND B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION                       
	SELECT A.*,
	C.B19C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_ACDOCA_LIFNR = B19C_ZF_LFBK_LIFNR_OLD_1
												AND B11B_ACDOCA_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
                                                AND B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	...
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_ACDOCA_LIFNR = B19C_ZF_LFBK_LIFNR_NEW_2
												AND B11B_ACDOCA_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
                                                AND B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	...
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_ACDOCA_LIFNR = B19C_ZF_LFBK_LIFNR_OLD_2
												AND B11B_ACDOCA_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
                                                AND B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'


	EXEC SP_DROPTABLE 'B19_11_IT_AP_DETAIL'
	SELECT A.*	
	INTO B19_11_IT_AP_DETAIL
	FROM B19_10_TT_AP_DETAIL A

    EXEC SP_UNNAME_FIELD 'B11B_', 'B19_11_IT_AP_DETAIL'
    EXEC SP_RENAME_FIELD 'B19G_', 'B19_11_IT_AP_DETAIL'


	EXEC SP_DROPTABLE 'B19_12_TT_REGUP_DETAIL'
	SELECT A.*,
	B.B19A_ZF_CASE_TYPE ZF_CASE_TYPE,
	B.B19A_ZF_CHANGE_ID ZF_CHANGE_ID
	INTO B19_12_TT_REGUP_DETAIL
	FROM A_REGUP A
	INNER JOIN B19_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP B ON REGUP_LIFNR = B.B19A_ZF_LIFNR_1
												AND REGUP_BLDAT BETWEEN B19A_ZF_CDHDR_UDATE_1 AND B.B19A_ZF_CDHDR_UDATE_2
	UNION
	SELECT A.*,
	C.B19E_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B19E_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B19_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE C ON REGUP_LIFNR = C.B19E_ZF_LIFNR_1
												AND REGUP_BLDAT BETWEEN c.B19E_ZF_CDHDR_UDATE_1 AND C.B19E_ZF_CDHDR_UDATE_2
	...
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B19C_ZF_LFBK_LIFNR_NEW_1
												AND REGUP_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
	UNION
	SELECT A.*,
	C.B19C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B19C_ZF_LFBK_LIFNR_OLD_1
												AND REGUP_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
	UNION
	SELECT A.*,
	C.B19C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B19C_ZF_LFBK_LIFNR_NEW_2
												AND REGUP_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2
	...
	C.B19C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B19_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B19C_ZF_LFBK_LIFNR_OLD_2
												AND REGUP_BLDAT BETWEEN C.B19C_ZF_CDHDR_UDATE_1 AND C.B19C_ZF_CDHDR_UDATE_2


	EXEC SP_DROPTABLE 'B19_14_IT_REGUP_DETAIL'
	SELECT A.*,
	CONVERT(money, REGUP_DMBTR * ISNULL(B00_TCURX.TCURX_FACTOR,1) * (CASE WHEN A.REGUP_SHKZG = 'S' THEN 1 ELSE -1 END) 
	* COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1))  AS ZF_REGUP_DMBTR_S_CUC,
	CONVERT(money, REGUP_DMBTR  * ISNULL(B00_TCURX.TCURX_FACTOR,1) * (CASE WHEN A.REGUP_SHKZG = 'S' THEN 1 ELSE -1 END)) AS ZF_REGUP_DMBTR_S,
	
	A_T001.T001_KTOPL,
	A_T003T.T003T_LTEXT,
	A_TBSLT.TBSLT_LTEXT
	INTO B19_14_IT_REGUP_DETAIL
	FROM B19_12_TT_REGUP_DETAIL A
	LEFT JOIN A_T001
		ON  REGUP_BUKRS = A_T001.T001_BUKRS
	LEFT JOIN A_T003T
		ON T003T_BLART = REGUP_BLART
	LEFT JOIN A_TBSLT
		ON  A_TBSLT.TBSLT_BSCHL = A.REGUP_BSCHL AND
			A_TBSLT.TBSLT_UMSKZ = A.REGUP_UMSKZ
	LEFT JOIN B00_TCURX
	...
	EXEC SP_RENAME_FIELD 'B19H_', 'B19_14_IT_REGUP_DETAIL'

	EXEC SP_DROPTABLE 'B19_15_IT_KNA1'
    SELECT * 
    INTO B19_15_IT_KNA1
    FROM A_KNA1
    WHERE EXISTS(SELECT * FROM A_KNB1 WHERE KNA1_KUNNR = KNB1_KUNNR
                    AND EXISTS(SELECT * FROM A_BKPF WHERE BKPF_BUKRS = KNB1_BUKRS))
        OR EXISTS(SELECT * FROM B19_01_IT_CDHDR_CDPOS WHERE B19_ZF_CUSTOMER_SUPPLIER_KEY = KNA1_KUNNR)

	EXEC SP_DROPTABLE 'B19_16_IT_LFA1'
    SELECT * 
    INTO B19_16_IT_LFA1
    FROM A_LFA1
    WHERE EXISTS(SELECT * FROM A_LFB1 WHERE LFA1_LIFNR = LFB1_LIFNR
                    AND EXISTS(SELECT * FROM A_BKPF WHERE BKPF_BUKRS = LFB1_BUKRS))
        OR EXISTS(SELECT * FROM B19_01_IT_CDHDR_CDPOS WHERE B19_ZF_CUSTOMER_SUPPLIER_KEY = LFA1_LIFNR)
