...
CREATE       PROCEDURE [dbo].[script_B04_FIN_GENERAL_LEDGER_SIE]
...
-- Based on Universal script (line 110)
			,ACDOCA_NETDT --[Z_Due Date] -- Nam: update net due date from ACDOCA
			,CASE
...
-- Nam: update net due date from ACDOCA
...
-- Based on Universal script (line 326)
			,DATEDIFF(day,ACDOCA_BLDAT,ACDOCA_NETDT) AS ZF_ACDOCA_BLDAT_NETDT_DIFF --[Z_Days - Document Date to Due Date]
...
-- Based on Universal script (line 342)
			,DATEDIFF(day,ACDOCA_BUDAT,ACDOCA_NETDT) AS ZF_ACDOCA_BUDAT_NETDT_DIFF --[Z_Days - Posting Date to Due Date]
...
-- Based on Universal script (line 368)
			,DATEDIFF(day,ACDOCA_NETDT,ACDOCA_AUGDT) AS ZF_ACDOCA_NETDT_AUGDT_DIFF --Z_Days - Due Date to Clearing Date
...
ALTER TABLE B04_05_IT_BKPF_ACDOCA ADD ZF_ACDOCA_BLDAT_NETDT_DIFF_BUCKET VARCHAR(50) 
ALTER TABLE B04_05_IT_BKPF_ACDOCA ADD ZF_ACDOCA_BLDAT_AUGDT_DIFF_BUCKET VARCHAR(50)
ALTER TABLE B04_05_IT_BKPF_ACDOCA ADD ZF_ACDOCA_BUDAT_NETDT_DIFF_BUCKET VARCHAR(50)
ALTER TABLE B04_05_IT_BKPF_ACDOCA ADD ZF_ACDOCA_BUDAT_AUGDT_DIFF_BUCKET VARCHAR(50)
ALTER TABLE B04_05_IT_BKPF_ACDOCA ADD ZF_ACDOCA_NETDT_AUGDT_DIFF_BUCKET VARCHAR(50)
...
UPDATE B04_05_IT_BKPF_ACDOCA
SET ZF_ACDOCA_BLDAT_NETDT_DIFF_BUCKET = 
(
	CASE WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF      < -360          THEN '01_-361 or less'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN -360 AND -181 THEN '02_-360 to -181 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN -180 AND  -91 THEN '03_-180 to -91 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN  -90 AND  -61 THEN '04_-90 to -61 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN  -60 AND  -31 THEN '05_-60 to -31 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN  -30 AND   -1 THEN '06_-30 to -1 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF       =    0          THEN '07_0 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN    1 AND   30 THEN '08_1 to 30 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN   31 AND   60 THEN '09_31 to 60 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN   61 AND   90 THEN '10_61 to 90 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN   91 AND  180 THEN '11_91 to 180 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF BETWEEN  181 AND  360 THEN '12_181 to 360 days'
			WHEN ZF_ACDOCA_BLDAT_NETDT_DIFF      >  360          THEN '13_361 days or more'
			ELSE                                                                 '90_Others'
		END
)
...
UPDATE B04_05_IT_BKPF_ACDOCA
SET ZF_ACDOCA_BUDAT_NETDT_DIFF_BUCKET =
(
	CASE WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF      < -360          THEN '01_-361 or less'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN -360 AND -181 THEN '02_-360 to -181 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN -180 AND  -91 THEN '03_-180 to -91 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN  -90 AND  -61 THEN '04_-90 to -61 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN  -60 AND  -31 THEN '05_-60 to -31 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN  -30 AND   -1 THEN '06_-30 to -1 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF       =    0          THEN '07_0 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN    1 AND   30 THEN '08_1 to 30 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN   31 AND   60 THEN '09_31 to 60 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN   61 AND   90 THEN '10_61 to 90 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN   91 AND  180 THEN '11_91 to 180 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF BETWEEN  181 AND  360 THEN '12_181 to 360 days'
			WHEN ZF_ACDOCA_BUDAT_NETDT_DIFF      >  360          THEN '13_361 days or more'
			ELSE                                                                '90_Others'
		END
)
...
UPDATE B04_05_IT_BKPF_ACDOCA
SET ZF_ACDOCA_NETDT_AUGDT_DIFF_BUCKET =
(
	CASE WHEN ZF_ACDOCA_CLEARING_STATUS                        = 'Open'        THEN '14_Not Cleared'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF      < -360          THEN '01_-361 or less'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN -360 AND -181 THEN '02_-360 to -181 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN -180 AND  -91 THEN '03_-180 to -91 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN  -90 AND  -61 THEN '04_-90 to -61 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN  -60 AND  -31 THEN '05_-60 to -31 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN  -30 AND   -1 THEN '06_-30 to -1 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF       =    0          THEN '07_0 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN    1 AND   30 THEN '08_1 to 30 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN   31 AND   60 THEN '09_31 to 60 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN   61 AND   90 THEN '10_61 to 90 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN   91 AND  180 THEN '11_91 to 180 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF BETWEEN  181 AND  360 THEN '12_181 to 360 days'
			WHEN ZF_ACDOCA_NETDT_AUGDT_DIFF      >  360          THEN '13_361 days or more'
			ELSE                                                                 '90_Others'
		END
)
	 ...
	 -- Create a list of Debit account AP clearing flag
                  ...
                  ,ISNULL(STUFF((SELECT '_' + B.ZF_ACC_IS_AP_CLEARING
					...
					-- Create a list of Credit account AR clearing flag
					 ...
					 --KHOI
					 --Debit customer name
					,ISNULL(STUFF((SELECT '_' + ISNULL(B.ACDOCA_KNA1_NAME1,'')
				   FROM B04_08_IT_FIN_GL B
				   WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS
								 AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
								 AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
								 AND    B.ACDOCA_DRCRK = 'S' AND B.ACDOCA_RACCT  <> '')
				   GROUP BY B.ACDOCA_RACCT, B.ACDOCA_KNA1_NAME1 
				   ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
				   FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
				,1,1,''), '_') ZF_ACDOCA_KUNNR_NAME1_DEBIT_LIST
					 --Credit customer name
					 ,ISNULL(STUFF((SELECT '_'+  B.ACDOCA_KNA1_NAME1
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_KUNNR_NAME1_CREDIT_LIST
					 --Debit supplier name
					 ,ISNULL(STUFF((SELECT '_'+ ISNULL(B.ACDOCA_LFA1_NAME1,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ACDOCA_LFA1_NAME1
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_LIFNR_NAME1_DEBIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_LIFNR_NAME1_CREDIT_LIST
                     ...
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           ...
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.MARA_MATKL
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_MATNR_MARA_MATKL_DEBIT_LIST
                     ...
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           ...
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.T023T_WGBEZ
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_MATNR_T023T_WGBEZ_DEBIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_MATNR_MARA_MATKL_CREDIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_MATNR_T023T_WGBEZ_CREDIT_LIST
					 --Debit asset
					 ,ISNULL(STUFF((SELECT '_'  + B.ACDOCA_ANLN1+ '|' + B.ACDOCA_ANLN2
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ACDOCA_ANLN1,B.ACDOCA_ANLN2
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC,  B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_ANLN1_ANLN2_DEBIT_LIST
                     ...
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           ...
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ANLT_TXT50
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_ANLN1_ANLT_DEBIT_LIST
					 --Credit asset
					 ,ISNULL(STUFF((SELECT '_' + B.ACDOCA_ANLN1+ '|' + B.ACDOCA_ANLN2
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC,B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_ANLN1_ANLN2_CREDIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_ACDOCA_ANLN1_ANLT_CREDIT_LIST
					 --Debit LFA1_KUNNR
					 ,ISNULL(STUFF((SELECT '_'  + B.ACDOCA_LFA1_KUNNR
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ACDOCA_LFA1_KUNNR
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_ACDOCA_DEBIT_LIST
					 --Debit LFA1_KUNNR name
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ACDOCA_LFA1_KUNNR_NAME1,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ACDOCA_LFA1_KUNNR_NAME1
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_NAME1_ACDOCA_DEBIT_LIST
					 --Credit LFA1_KUNNR
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ACDOCA_LFA1_KUNNR,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_ACDOCA_CREDIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_NAME1_ACDOCA_CREDIT_LIST
					 --Debit KNA1_LIFNR
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ACDOCA_KNA1_LIFNR,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ACDOCA_KNA1_LIFNR
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_ACDOCA_DEBIT_LIST
					-- Debit KNA1_LIFNR name
					,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ACDOCA_KNA1_LIFNR_NAME1,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
                           AND    B.ACDOCA_DRCRK = 'S')
                     GROUP BY B.ACDOCA_RACCT, B.ACDOCA_KNA1_LIFNR_NAME1
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_NAME1_ACDOCA_DEBIT_LIST
					 --Credit KNA1_LIFNR
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ACDOCA_KNA1_LIFNR,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_ACDOCA_CREDIT_LIST
					--Credit KNA1_LIFNR name
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_NAME1_ACDOCA_CREDIT_LIST


					 --Debit found CSKB flag
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ZF_RACCT_FOUND_IN_CSKB,'')
                     FROM B04_08_IT_FIN_GL B
                     WHERE (B.ACDOCA_RBUKRS = B04_08_IT_FIN_GL.ACDOCA_RBUKRS  
                           AND B.ACDOCA_GJAHR = B04_08_IT_FIN_GL.ACDOCA_GJAHR
                           AND B.ACDOCA_BELNR = B04_08_IT_FIN_GL.ACDOCA_BELNR
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_RACCT_FOUND_IN_CSKB_DEBIT
					--Credit found in CSKB flag
					 ...
					 ORDER BY SUM(ABS(B.ZF_ACDOCA_HSL_S)) DESC, B.ACDOCA_RACCT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_RACCT_FOUND_IN_CSKB_CREDIT
