...
--     Thuan Update spend category level 4 logic in PTP.
--     If GL cube MATNR is null, Link GL with PO cube base on EBELN, EBELP then get MAKTL from PO cube.
--  
	UPDATE B12_01_IT_PTP_GL
	SET SPCAT_SPEND_CAT_LEVEL_4 = 
	IIF(ISNULL(IRGR_GL_ACCT, '') <> '' AND ISNULL(A_T023T.T023T_WGBEZ,'') <> '',
					'P-' + CONCAT(EKPO_MATKL, '-', A_T023T.T023T_WGBEZ),
					SPCAT_SPEND_CAT_LEVEL_4
			   )
	FROM B12_01_IT_PTP_GL
	INNER JOIN A_EKPO ON B04_GL_ACDOCA_EBELN = EKPO_EBELN AND EKPO_EBELP = B04_GL_ACDOCA_EBELP		
	INNER JOIN A_T023T ON T023T_MATKL = EKPO_MATKL AND T023T_SPRAS = 'EN'	
	--Map with GR IRGR account
	INNER JOIN AM_IRGR_GINI_ARDR_MAPPING
		ON DBO.REMOVE_LEADING_ZEROES(B04_GL_ACDOCA_RACCT) = DBO.REMOVE_LEADING_ZEROES(IRGR_GL_ACCT) AND
		IRGR_OR_GINI_OR_ARDR = 'IRGR'
	WHERE B04_GL_ACDOCA_MATNR = ''

--     Thuan Update spend category level 1 - 3 logic in PTP.

	UPDATE B12_01_IT_PTP_GL
	SET SPCAT_SPEND_CAT_LEVEL_1 = 	IIF(ISNULL(IRGR_GL_ACCT, '') <> '' AND ISNULL(AM_IRGR_MATERIAL_GROUP_HIERARCHY.IRGR_SPEND_CAT_LEVEL_1,'') <> '',
									'P-' + AM_IRGR_MATERIAL_GROUP_HIERARCHY.IRGR_SPEND_CAT_LEVEL_1,
									 SPCAT_SPEND_CAT_LEVEL_1
									  ),
	SPCAT_SPEND_CAT_LEVEL_2 =		IIF(ISNULL(IRGR_GL_ACCT, '') <> '' AND ISNULL(AM_IRGR_MATERIAL_GROUP_HIERARCHY.IRGR_SPEND_CAT_LEVEL_2,'') <> '',
									'P-' + AM_IRGR_MATERIAL_GROUP_HIERARCHY.IRGR_SPEND_CAT_LEVEL_2,
									 SPCAT_SPEND_CAT_LEVEL_2
									  ),
	SPCAT_SPEND_CAT_LEVEL_3 =		IIF(ISNULL(IRGR_GL_ACCT, '') <> '' AND ISNULL(AM_IRGR_MATERIAL_GROUP_HIERARCHY.IRGR_SPEND_CAT_LEVEL_3,'') <> '',
									'P-' + AM_IRGR_MATERIAL_GROUP_HIERARCHY.IRGR_SPEND_CAT_LEVEL_3,
									 SPCAT_SPEND_CAT_LEVEL_3
									  )
	FROM B12_01_IT_PTP_GL
	...
	--Map with GR IRGR account
	INNER JOIN AM_IRGR_GINI_ARDR_MAPPING
		ON DBO.REMOVE_LEADING_ZEROES(B04_GL_ACDOCA_RACCT) = DBO.REMOVE_LEADING_ZEROES(IRGR_GL_ACCT) AND IRGR_OR_GINI_OR_ARDR = 'IRGR'
	INNER JOIN AM_IRGR_MATERIAL_GROUP_HIERARCHY
			ON AM_IRGR_MATERIAL_GROUP_HIERARCHY.T023T_MATKL = EKPO_MATKL
	WHERE B04_GL_ACDOCA_MATNR = ''
...
--Update the flag where LFA1_LAND1 is not the same with LFBK_BANKS
	...
	WHERE B11_ACDOCA_LIFNR IN
	(
		SELECT B11_ACDOCA_LIFNR 
		FROM A_LFBK 
		INNER JOIN B12_02_IT_PTP_AP 
			ON B11_ACDOCA_LIFNR=LFBK_LIFNR
...
--Step 6 / Get the detail of Supplier bank details
--Only get the case relate to AP supplier
...
EXEC SP_DROPTABLE B12_07_RT_LFBK_AP_DETALS
		...
		WHERE EXISTS
			(
				...
				WHERE B11_ACDOCA_LIFNR=LFBK_LIFNR
			)
