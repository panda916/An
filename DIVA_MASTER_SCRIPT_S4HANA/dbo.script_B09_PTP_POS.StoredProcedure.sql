...
CREATE    PROCEDURE [dbo].[script_B09_PTP_POS]
...
/* 
	...
	16-04-2024		HL		Changed the history total table to an IT one + add fields from SAP USAGE
*/
...
*/
   ...
   EXEC A_004_CREATE_PROCUREMENT_CATEGORY_STRUCTURE_EXE
/*--Step 2 
...
*/	
	...
	ON A_EKKO.EKKO_BUKRS = ZF_COMPANY_CODE

	-- Add currency conversion factors for company code currency
	LEFT JOIN B00_TCURX TCURX_CC 
	ON B00_03_IT_T001_RMV_DUP.T001_WAERS = TCURX_CC.TCURX_CURRKEY   

	-- Add currency factor from company currency to USD
	LEFT JOIN B00_IT_TCURF
	ON B00_03_IT_T001_RMV_DUP.T001_WAERS = B00_IT_TCURF.TCURF_FCURR
...
-- Add exchange rate from company currency to USD
	LEFT JOIN B00_IT_TCURR
		ON B00_03_IT_T001_RMV_DUP.T001_WAERS = B00_IT_TCURR.TCURR_FCURR
...
EXEC SP_DROPTABLE 'B09_04_TT_IMM_GR'
	...
	SELECT
		...
		,SUM(CASE WHEN EKBE_VGABE = '1' THEN CONVERT(money,EKBE_DMBTR * (CASE WHEN (EKBE_SHKZG = 'S') THEN 1 ELSE -1 END) * COALESCE(TCURX_DOC.TCURX_FACTOR,1) * COALESCE(TCURX_CC.TCURX_FACTOR,1) * COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1)) END) AS ZF_EKBE_WRBTR_IMM_GR_S_CUC
	...
	LEFT JOIN B00_TCURX TCURX_CC 
	on
	   B00_03_IT_T001_RMV_DUP.T001_WAERS = TCURX_CC.TCURX_CURRKEY   
...
--Fields are being calculated as mentioned in SELECT clause below*/
   ...
   EXEC SP_DROPTABLE 'B09_08_TT_IMM_INV'
	...
	SELECT
		...
		,SUM(CASE WHEN EKBE_VGABE IN ('2', '3', '4')  THEN CONVERT(money,EKBE_DMBTR * (CASE WHEN (EKBE_SHKZG = 'S') THEN 1 ELSE -1 END) * COALESCE(TCURX_DOC.TCURX_FACTOR,1) * COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1)) END) AS ZF_EKBE_WRBTR_IMM_INV_S_CUC
...
--Fields are being calculated as mentioned in SELECT clause below*/
   ...
   EXEC SP_DROPTABLE 'B09_09_IT_PO_HIST_TOTALS' -- HL: Changed to an IT table so it can be used in SAP USAGE
	SELECT  
		A_EKBE.EKBE_MANDT,
		A_EKBE.EKBE_EBELN,
		A_EKBE.EKBE_EBELP,
		MAX(IIF(EKBE_VGABE IN ('2', '3', '4'), EKBE_WAERS, NULL)) ZF_INV_EKBE_WAERS,
		MAX(IIF(EKBE_VGABE IN ('2', '3', '4'), T001_WAERS, NULL)) ZF_INV_T001_WAERS, -- HL: Added from SAP USAGE script
		...
		MAX(IIF(EKBE_VGABE IN ('1'), T001_WAERS, NULL)) ZF_GR_T001_WAERS, -- HL: Added from SAP USAGE Script
	...
	--Add the house currency
	LEFT JOIN B00_03_IT_T001_RMV_DUP
	ON A_EKPO.EKPO_BUKRS = ZF_COMPANY_CODE
...
-- Using the exchange rate type m (the average) and tcurf_ffact 1 when multiple currency factor occurs
   ...
   EXEC SP_DROPTABLE 'B09_13_IT_PTP_POS'
	...
	SELECT
		...
		COALESCE (B08_02_IT_PTP_SMD.B08_LFA1_KTOKK,'')	AS LFA1_KTOKK,
		COALESCE (B08_02_IT_PTP_SMD.B08_T077Y_TXT30,'')	AS T077Y_TXT30,
		A_EKKO.EKKO_LIFNR,
		COALESCE (B08_02_IT_PTP_SMD.B08_LFA1_NAME1,'')	AS LFA1_NAME1,
		COALESCE (B08_02_IT_PTP_SMD.B08_LFA1_LAND1,'')	AS LFA1_LAND1,
		-- Country code in three digit ISO format, required for QlikSense maps
		COALESCE (B08_02_IT_PTP_SMD.B08_COUNTRY_MAPPING_DESC,'')		AS COUNTRY_MAPPING_DESC,
		A_EKPO.EKPO_WERKS,
		A_T001W.T001W_NAME1,
		A_EKKO.EKKO_EKORG,
		A_T024E.T024E_EKOTX,
		...
		--COALESCE(B00_T023T.T023T_WGBEZ60, '')	AS T023T_WGBEZ60,
		--Add the DIVISION
		COALESCE(A_MARA.MARA_SPART,'') AS MARA_SPART,

		---AM_DSM.DSM_DIVISION,
		-- Add business segment
		----AM_DSM.DSM_BUSINESS_SEGMENT,
		...
		-- Blanket PO update logic from Jesper (05/05/2021).
		...
		A_EKPO.EKPO_NETPR,
		B00_03_IT_T001_RMV_DUP.T001_WAERS,																									
		...
		CONVERT(money,(A_EKPO.EKPO_NETWR * COALESCE(TCURX_DOC.TCURX_FACTOR,1) * A_EKKO.EKKO_WKURS * COALESCE(A_TCURF_DOC_TO_LOCAL.TCURF_TFACT,1))/COALESCE(A_TCURF_DOC_TO_LOCAL.TCURF_FFACT,1))	AS ZF_EKPO_NETWR_COC, 
		...
		* COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1))	AS ZF_EKPO_NETWR_CUC,
		...
		B09_06_TT_1ST_INV_USR_DT_PER_PO.ZF_EKBE_BLDAT_1ST_INV_DATE											AS ZF_EKBE_BLDAT_1ST_INV_DATE,
		...
		B09_02_TT_1ST_GR_USR_DT_PER_PO.ZF_EKBE_CPUDT_1ST_GR_DATE																		AS ZF_EKBE_CPUDT_1ST_GR_DATE,
		...
		COALESCE(B09_09_IT_PO_HIST_TOTALS_GR.ZF_EKBE_VGABE_IS_A_GR,'')		AS ZF_EKBE_VGABE_IS_A_GR,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_MENGE_GR_S,0)			AS ZF_EKBE_MENGE_GR_S,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_DMBTR_GR_S,0)			AS ZF_EKBE_DMBTR_GR_S,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_DMBTR_GR_S_CUC,0)		AS ZF_EKBE_DMBTR_GR_S_CUC,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_WRBTR_GR_S,0)			AS ZF_EKBE_WRBTR_GR_S,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_WRBTR_GR_S_CUC,0)		AS ZF_EKBE_WRBTR_GR_S_CUC,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_BELNR_NUM_GRS,0)		AS ZF_EKBE_BELNR_NUM_GRS,
		...
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_VGABE_IS_AN_INV,'')	AS ZF_EKBE_VGABE_IS_AN_INV,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_MENGE_INV_S,0)		AS ZF_EKBE_MENGE_INV_S,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_DMBTR_INV_S,0)		AS ZF_EKBE_DMBTR_INV_S,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_DMBTR_INV_S_CUC,0)	AS ZF_EKBE_DMBTR_INV_S_CUC,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_WRBTR_INV_S,0)		AS ZF_EKBE_WRBTR_INV_S,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_WRBTR_INV_S_CUC,0)	AS ZF_EKBE_WRBTR_INV_S_CUC,
		--COALESCE(B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_BELNR_NUM_INVS,0)		AS ZF_EKBE_BELNR_NUM_INVS,
		...
		COALESCE(B08_02_IT_PTP_SMD.B08_INTERCO_TXT,'Unknown ? supplier record not available')		 AS INTERCO_TXT,   
		...
		CASE 
			...
			WHEN   A_EKKO.EKKO_BSTYP='F' THEN 
				CASE 
					WHEN B09_09_IT_PO_HIST_TOTALS_INV.ZF_EKBE_VGABE_IS_AN_INV='X' THEN
						CASE 
							WHEN B09_09_IT_PO_HIST_TOTALS_GR.ZF_EKBE_VGABE_IS_A_GR = 'X' 
		...
		CASE  
			WHEN (CONVERT(money,A_EKPO.EKPO_NETWR * COALESCE(TCURX_DOC.TCURX_FACTOR,1)) - B09_09_IT_PO_HIST_TOTALS_INV.ZF_EKBE_WRBTR_INV_S) <= -1	THEN 'invoice value over PO value'
			WHEN ABS(CONVERT(money,A_EKPO.EKPO_NETWR * COALESCE(TCURX_DOC.TCURX_FACTOR,1)) - B09_09_IT_PO_HIST_TOTALS_INV.ZF_EKBE_WRBTR_INV_S) < 1	THEN 'invoice value equal PO value'
			WHEN (CONVERT(money,A_EKPO.EKPO_NETWR * COALESCE(TCURX_DOC.TCURX_FACTOR,1)) - B09_09_IT_PO_HIST_TOTALS_INV.ZF_EKBE_WRBTR_INV_S) >= 1		THEN 'invoice value under PO value'
			ELSE 'Not invoiced' 
		END	AS ZF_EKBE_WRBTR_UNDER_OVER_INV,
		---- Add flag to show if the immediate invoice is fully reversed
		--CASE 
		--	WHEN B09_08_TT_IMM_INV.ZF_NUM_IMM_INVS>0 AND B09_09_IT_PO_HIST_TOTALS.ZF_EKBE_WRBTR_INV_S = 0 THEN 'YES'
		--	ELSE 'NO'
	...
	A_TCURF_DOC_TO_LOCAL.*,
	A_EKPO.EKPO_INFNR -- Hanh Lam: Added from SAP USAGE
	...
	LEFT JOIN B08_02_IT_PTP_SMD
		ON  A_EKKO.EKKO_LIFNR = B08_02_IT_PTP_SMD.B08_EKKO_LIFNR 
	...
	LEFT JOIN B00_T009B
	ON (B00_03_IT_T001_RMV_DUP.T001_PERIV = T009B_PERIV) AND
	   MONTH(EKKO_AEDAT) = T009B_BUMON
...
-- Related convert from local currency to USD
...
-- Related convert from document currency to local currency
-- Add currency factor from document currency to local currency
	...
	LEFT JOIN B09_12_TT_A_TCURF A_TCURF_DOC_TO_LOCAL
		...
		AND B00_03_IT_T001_RMV_DUP.T001_WAERS = A_TCURF_DOC_TO_LOCAL.TCURF_TCURR
	...
	--OUTER APPLY(SELECT TOP 1 * FROM B09_09_IT_PO_HIST_TOTALS WHERE
	--	A_EKKO.EKKO_EBELN = B09_09_IT_PO_HIST_TOTALS.EKBE_EBELN 
	--	AND	A_EKPO.EKPO_EBELP = B09_09_IT_PO_HIST_TOTALS.EKBE_EBELP) B09_09_IT_PO_HIST_TOTALS
	...
	LEFT JOIN (
		...
		FROM B09_09_IT_PO_HIST_TOTALS AS A
		WHERE A.ZF_EKBE_VGABE_IS_AN_INV LIKE 'X'
		GROUP BY A.EKBE_MANDT, A.EKBE_EBELN, A.EKBE_EBELP, A.ZF_EKBE_VGABE_IS_AN_INV
	) AS B09_09_IT_PO_HIST_TOTALS_INV
	ON  EKPO_EBELN = B09_09_IT_PO_HIST_TOTALS_INV.EKBE_EBELN
	AND EKPO_EBELP = B09_09_IT_PO_HIST_TOTALS_INV.EKBE_EBELP
	...
	) B09_09_IT_PO_HIST_TOTALS_GR
	ON EKPO_EBELN = B09_09_IT_PO_HIST_TOTALS_GR.EKBE_EBELN
	AND EKPO_EBELP = B09_09_IT_PO_HIST_TOTALS_GR.EKBE_EBELP
	...
	LEFT JOIN B00_SKAT
		...
		AND B00_03_IT_T001_RMV_DUP.T001_KTOPL = B00_SKAT.SKAT_KTOPL
	...
	LEFT JOIN B09_12_TT_TCURR_TO_HSE_CURR 
		...
		AND B00_03_IT_T001_RMV_DUP.T001_WAERS = B09_12_TT_TCURR_TO_HSE_CURR.TCURR_TCURR
				
	-- Add the business segment
		---LEFT JOIN AM_DSM
		-----ON A_MARA.MARA_SPART = AM_DSM.DSM_DIVISION
...
EXEC sp_RENAME_FIELD 'B09_09_', 'B09_09_IT_PO_HIST_TOTALS'
