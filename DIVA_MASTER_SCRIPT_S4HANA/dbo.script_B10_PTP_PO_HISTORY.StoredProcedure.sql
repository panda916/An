...
/*Change history comments*/
  ...
  /*
	...
	05-08-2019		VL		Update with S4HANA
...
--Fields are being calculated as mentioned in SELECT clause below*/
	...
	SELECT 
		...
		,B09_13_IT_PTP_POS.B09_EKKO_BUKRS AS EKKO_BUKRS
        ...
        ,A_T163C.T163C_BEWTL
        ,A_T163C.T163C_BEWTK
        ,IIF(A_EKBE.EKBE_WAERS = B09_13_IT_PTP_POS.B09_EKKO_WAERS, 'X', '') AS ZF_EKKO_EKBE_SAME_DOC_CURRENCY_FLAG
		,CASE 
		...
		,CONVERT(money,A_EKBE.EKBE_DMBTR * (CASE WHEN (A_EKBE.EKBE_SHKZG = 'S') THEN 1 ELSE -1 END) * COALESCE(TCURX_CC.TCURX_Factor,1) * COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1)) AS ZF_EKBE_AREWR_S_CUC
		...
		-- Thuan updated : 20240201 Remove link AM_COMPANY_CODE from EKKO.

		LEFT JOIN B00_03_IT_T001_RMV_DUP
		ON B09_EKKO_BUKRS = ZF_COMPANY_CODE
...
-- Thuan 28/07/2021 Add field to show "With 1 EKBE_BELNR VGABE are invoice and have many POs"
ALTER TABLE B10_02_IT_PTP_PO_HISTORY ADD B10_ZF_INV_MANY_POS NVARCHAR(3); 

UPDATE B10_02_IT_PTP_PO_HISTORY
SET B10_ZF_INV_MANY_POS = IIF(B.B10_EKBE_BELNR IS NULL, 'No', 'Yes')
FROM B10_02_IT_PTP_PO_HISTORY AS A 
LEFT JOIN 
(
	SELECT DISTINCT B10_EKBE_BELNR FROM B10_02_IT_PTP_PO_HISTORY
	WHERE B10_EKBE_VGABE in ('2','3','4')
	GROUP BY B10_EKBE_BELNR
	HAVING COUNT(DISTINCT B10_EKKO_EBELN) > 1
) AS B ON A.B10_EKBE_BELNR = B.B10_EKBE_BELNR


/*Remove temparory table*/
