...
CREATE     PROCEDURE [dbo].[script_B20_SOD_OTC]
...
-- Add exchange rate from document currency to USD
	...
	*/
	...
	/*
		Step 8: Get all material documents from MATDOC table base on VFBA sale document flow table
	*/
		...
		SELECT 
			...
			MATDOC_AEDAT,
			MATDOC_USNAM,
			IIF(A_V_USERNAME.V_USERNAME_NAME_TEXT IS NULL, MATDOC_USNAM, CONCAT(MATDOC_USNAM, ' - ', A_V_USERNAME.V_USERNAME_NAME_TEXT)) AS MATDOC_USNAM_AND_NAME_TEXT,
			MATDOC_TCODE,
			MATDOC_XBLNR,
			MATDOC_BKTXT,
			MATDOC_BWART,
			...
			@CURRENCY ZF_CUSTOM_CURRENCY,
			T001_WAERS,
			(MATDOC_DMBTR)*ISNULL(TCURX_FACTOR,1) ZF_MATDOC_DMBTR_S, -- Amount in local currency
			(MATDOC_DMBTR)*ISNULL(TCURX_FACTOR,1)*COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1) ZF_MATDOC_DMBTR_S_CUC, -- Amount in local currency,
			MATDOC_MENGE,
			MATDOC_SGTXT,
			VBFA_VBELV,
			VBFA_POSNV,
		    CONCAT(MATDOC_MANDT,'|',MATDOC_MJAHR,'|',MATDOC_MBLNR,'|',CAST(MATDOC_ZEILE AS INT)) ZF_SALE_MATERIAL_DOC_KEY
		INTO B20_08_IT_MATERIAL_DOCS
		FROM B00_MATDOC
		--Get material desc
		LEFT JOIN A_MAKT
		ON  MAKT_SPRAS IN ('E', 'EN')
		AND MATDOC_MATNR = MAKT_MATNR 

		--Get customer desc
		LEFT JOIN A_KNA1
		ON  KNA1_KUNNR = MATDOC_KUNNR

		--Get auto generate indicator desc
		LEFT JOIN B00_DD07T_MB_XAUTO
		ON MATDOC_XAUTO = B00_DD07T_MB_XAUTO.DD07T_DOMVALUE_L  COLLATE SQL_Latin1_General_CP1_CS_AS

		--Transaction type desc
		LEFT JOIN A_T158W
		ON T158W_SPRAS IN ('E', 'EN')
		AND T158W_VGART = MATDOC_VGART

		--Document type desc
		LEFT JOIN A_T003T
		ON  T003T_SPRAS IN ('E', 'EN')
		AND MATDOC_BLART = T003T_BLART

		--Get comapany info
		LEFT JOIN A_T001
		ON  T001_BUKRS = MATDOC_BUKRS
...
-- Add currency factor from company currency to USD
		...
		AND SKAT_SAKNR = MATDOC_SAKTO 

		LEFT JOIN A_V_USERNAME
		ON  A_V_USERNAME.V_USERNAME_BNAME = MATDOC_USNAM

		--Get cost center desc
		LEFT JOIN A_CSKT
		ON  CSKT_KOKRS = MATDOC_KOKRS
		AND CSKT_SPRAS IN ('E', 'EN')
		AND CSKT_KOSTL = MATDOC_KOSTL
		AND CSKT_DATBI = (
			SELECT TOP 1 CSKT_DATBI
			FROM A_CSKT
			WHERE  CSKT_KOKRS = MATDOC_KOKRS
			AND CSKT_SPRAS IN ('E', 'EN')
			AND CSKT_KOSTL =MATDOC_KOSTL
			AND CSKT_DATBI >= MATDOC_BLDAT
			ORDER BY CSKT_DATBI ASC
		)

		--Get profit center center desc
		LEFT JOIN A_CEPCT
		ON  CEPCT_KOKRS = MATDOC_KOKRS
		AND CEPCT_SPRAS IN ('E', 'EN')
		AND CEPCT_PRCTR = MATDOC_PRCTR
		AND CEPCT_DATBI = (
			SELECT TOP 1 CEPCT_DATBI
			FROM A_CEPCT
			WHERE  CEPCT_KOKRS = MATDOC_KOKRS
			AND CEPCT_SPRAS IN ('E', 'EN')
			AND CEPCT_PRCTR = MATDOC_PRCTR
			AND CEPCT_DATBI >= MATDOC_BLDAT
		...
		LEFT JOIN A_VBFA
		ON MATDOC_MJAHR = YEAR(VBFA_ERDAT)
		AND MATDOC_MBLNR = VBFA_VBELN
		AND CAST(MATDOC_ZEILE AS INT) = CAST(VBFA_POSNN AS INT)
		...
		EXISTS(
			SELECT TOP 1 1 FROM A_VBFA WHERE  VBFA_VBELN = MATDOC_MBLNR AND CAST(VBFA_POSNN AS INT) = CAST(MATDOC_ZEILE AS INT)
		)
	...
	*/
		...
		SELECT 
            ...
            (VBRP_NETWR)*ISNULL(TCURX_FACTOR,1) ZF_VBRP_NETWR_S, -- Item net value
             (VBRP_NETWR)*ISNULL(TCURX_FACTOR,1)* VBRP_KURSK * COALESCE(TCURF_COC.TCURF_TFACT,1)/COALESCE(TCURF_COC.TCURF_FFACT,1) 
			* COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1) ZF_VBRP_NETWR_S_CUC, -- Item net value
            VBRP_VGBEL,
            VBRP_VGPOS,
            VBRP_VGTYP,
...
-- Related to convert from local currency to USD
-- Add currency factor from company currency to USD
		LEFT JOIN B00_IT_TCURF TCURF_CUC
		ON A_T001.T001_WAERS = TCURF_CUC.TCURF_FCURR
		AND TCURF_CUC.TCURF_TCURR  = @currency  
		AND TCURF_CUC.TCURF_GDATU = (
			...
			WHERE A_T001.T001_WAERS = B00_IT_TCURF.TCURF_FCURR AND 
					B00_IT_TCURF.TCURF_TCURR  = @currency  AND
					B00_IT_TCURF.TCURF_GDATU <= VBRP_ERDAT
			ORDER BY B00_IT_TCURF.TCURF_GDATU DESC
			)
-- Add exchange rate from company currency to USD
		LEFT JOIN B00_IT_TCURR TCURR_CUC
			...
			AND TCURR_CUC.TCURR_GDATU = (
				...
				WHERE A_T001.T001_WAERS = B00_IT_TCURR.TCURR_FCURR AND 
						B00_IT_TCURR.TCURR_TCURR  = @currency  AND
						B00_IT_TCURR.TCURR_GDATU <= VBRP_ERDAT
				ORDER BY B00_IT_TCURR.TCURR_GDATU DESC
				) 
...
-- Add currency factor from document currency to local currency
	...
	*/
		...
		SELECT
			  B07_03_IT_FIN_AR_INV_PAY_FLAGS.*,
			  IIF(A_V_USERNAME.V_USERNAME_NAME_TEXT IS NULL, B07D_ACDOCA_USNAM, CONCAT(B07D_ACDOCA_USNAM, ' - ', A_V_USERNAME.V_USERNAME_NAME_TEXT)) AS ZF_ACDOCA_USNAM_AND_NAME_TEXT,
			  KNA1_ERNAM,
			  IIF(KNA1_ERNAM_INFO.V_USERNAME_NAME_TEXT IS NULL, KNA1_ERNAM, CONCAT(KNA1_ERNAM, ' - ', KNA1_ERNAM_INFO.V_USERNAME_NAME_TEXT)) AS ZF_KNA1_ERNAM_AND_NAME_TEXT,
			  T001_BUTXT,
			  DD07T_DDTEXT,
			  SKAT_TXT50,
			  CSKT_LTEXT,
			  CEPCT_LTEXT,
			  (B07D_ACDOCA_WSL)*ISNULL(B00_TCURX.TCURX_FACTOR,1) ZF_ACDOCA_WSL_S,
			  CONCAT(B07D_ACDOCA_RCLNT,'|',ACDOCA_AWREF) ZF_SALE_INVOICE_TO_ACCOUNTING_DOC_KEY
		INTO B20_10_IT_INVOICE_ACCOUTING_DOCS
		FROM B07_03_IT_FIN_AR_INV_PAY_FLAGS
		
		LEFT JOIN B00_ACDOCA
		ON B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RBUKRS = B00_ACDOCA.ACDOCA_RBUKRS AND
		   B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_GJAHR = B00_ACDOCA.ACDOCA_GJAHR AND
		   B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_BELNR = B00_ACDOCA.ACDOCA_BELNR AND
		   B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_DOCLN = B00_ACDOCA.ACDOCA_DOCLN AND
		   B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_BUZEI = B00_ACDOCA.ACDOCA_BUZEI

		LEFT JOIN A_KNA1
		ON B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_KUNNR = A_KNA1.KNA1_KUNNR

		LEFT JOIN A_T001
		ON B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RBUKRS = A_T001.T001_BUKRS

		LEFT JOIN B00_DD07T_BSTAT
		ON DD07T_DOMVALUE_L = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_BSTAT

		LEFT JOIN A_SKAT
		ON  A_SKAT.SKAT_SPRAS IN ('E', 'EN')
		AND A_T001.T001_KTOPL = A_SKAT.SKAT_KTOPL
		AND B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RACCT = A_SKAT.SKAT_SAKNR

		LEFT JOIN B00_TCURX
		ON B00_TCURX.TCURX_CURRKEY = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RWCUR

		LEFT JOIN A_V_USERNAME
		ON A_V_USERNAME.V_USERNAME_BNAME = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_USNAM
		...
		--Get cost center desc
		LEFT JOIN A_CSKT
		ON  CSKT_KOKRS = B00_ACDOCA.ACDOCA_KOKRS
		AND CSKT_SPRAS IN ('E', 'EN')
		AND CSKT_KOSTL = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RCNTR
		AND CSKT_DATBI = (
			SELECT TOP 1 CSKT_DATBI
			FROM A_CSKT
			WHERE  CSKT_KOKRS = B00_ACDOCA.ACDOCA_KOKRS
			AND CSKT_SPRAS IN ('E', 'EN')
			AND CSKT_KOSTL = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RCNTR
			AND CSKT_DATBI >= B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_BLDAT
			ORDER BY CSKT_DATBI ASC
		)

		--Get profit center center desc
		LEFT JOIN A_CEPCT
		ON  CEPCT_KOKRS = B00_ACDOCA.ACDOCA_KOKRS
		AND CEPCT_SPRAS IN ('E', 'EN')
		AND CEPCT_PRCTR = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_PRCTR
		AND CEPCT_DATBI = (
			SELECT TOP 1 CEPCT_DATBI
			FROM A_CEPCT
			WHERE CEPCT_KOKRS = B00_ACDOCA.ACDOCA_KOKRS
			AND CEPCT_SPRAS IN ('E', 'EN')
			AND CEPCT_PRCTR = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_PRCTR
			AND CEPCT_DATBI >= B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_BLDAT
			ORDER BY CEPCT_DATBI ASC
		)

		WHERE B07D_ACDOCA_AWTYP = 'VBRK' AND EXISTS(
			SELECT TOP 1 1 FROM B20_09_IT_INVOICE_DOCS WHERE B07D_ACDOCA_AWREF = VBRK_VBELN
		)
	...
	*/
		...
		SELECT B07_03_IT_FIN_AR_INV_PAY_FLAGS.*,
			   IIF(A_V_USERNAME.V_USERNAME_NAME_TEXT IS NULL, B07D_ACDOCA_USNAM, CONCAT(B07D_ACDOCA_USNAM, ' - ', A_V_USERNAME.V_USERNAME_NAME_TEXT)) AS ZF_ACDOCA_USNAM_AND_NAME_TEXT,
			   KNA1_ERNAM,
			   T001_BUTXT,
			   DD07T_DDTEXT,
			   ...
			   CEPCT_LTEXT,
			   (B07D_ACDOCA_WSL)*ISNULL(B00_TCURX.TCURX_FACTOR,1) ZF_ACDOCA_WSL_S,
			   CONCAT(B07D_ACDOCA_RCLNT,'|',ACDOCA_RBUKRS,'|',B07D_ACDOCA_GJAHR,'|',B07D_ACDOCA_BELNR) ZF_CLEARING_INVOICE_DOC_KEY
		...
		WHERE EXISTS (
			...
			WHERE  B20_10_IT_INVOICE_ACCOUTING_DOCS.B07D_ACDOCA_RBUKRS = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_RBUKRS
			AND B20_10_IT_INVOICE_ACCOUTING_DOCS.B07D_ACDOCA_AUGBL = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_BELNR
			AND B20_10_IT_INVOICE_ACCOUTING_DOCS.B07D_ACDOCA_AUGGJ = B07_03_IT_FIN_AR_INV_PAY_FLAGS.B07D_ACDOCA_GJAHR
		)
	...
	*/
		...
		IIF(B20_08_IT_MATERIAL_DOCS.MATDOC_MANDT IS NULL, NULL, 
			CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_MANDT,'|',B20_08_IT_MATERIAL_DOCS.MATDOC_MJAHR,'|',B20_08_IT_MATERIAL_DOCS.MATDOC_MBLNR,'|',CAST(B20_08_IT_MATERIAL_DOCS.MATDOC_ZEILE AS INT))) B20_08_ZF_SALE_MATERIAL_DOC_KEY,

		--Add material entry user
		B20_08_IT_MATERIAL_DOCS.MATDOC_MBLNR AS B20_20_MATDOC_MBLNR,
		B20_08_IT_MATERIAL_DOCS.MATDOC_ZEILE AS B20_20_MATDOC_ZEILE,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_VGART, ' - ', B20_08_IT_MATERIAL_DOCS.T158W_LTEXT) AS B20_20_MATDOC_VGART_AND_LTEXT,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_MATNR, ' - ', B20_08_IT_MATERIAL_DOCS.MAKT_MAKTX) AS B20_20_MATDOC_MATNR_AND_MAKTX,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_KUNNR, ' - ', B20_08_IT_MATERIAL_DOCS.KNA1_NAME1) AS B20_20_MATDOC_KUNNR_AND_NAME1,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_BUKRS, ' - ', B20_08_IT_MATERIAL_DOCS.T001_BUTXT) AS B20_20_MATDOC_BUKRS_AND_BUTXT,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_SAKTO, ' - ', B20_08_IT_MATERIAL_DOCS.SKAT_TXT20) AS B20_20_MATDOC_SAKTO_AND_TXT20,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_PRCTR, ' - ', B20_08_IT_MATERIAL_DOCS.CEPCT_LTEXT) AS B20_20_MATDOC_PRCTR_AND_LTEXT,
		CONCAT(B20_08_IT_MATERIAL_DOCS.MATDOC_KOSTL, ' - ', B20_08_IT_MATERIAL_DOCS.CSKT_LTEXT) AS B20_20_MATDOC_KOSTL_AND_LTEXT,
		B20_08_IT_MATERIAL_DOCS.MATDOC_SHKZG AS B20_20_MATDOC_SHKZG,
		B20_08_IT_MATERIAL_DOCS.MATDOC_WAERS AS B20_20_MATDOC_WAERS,
		B20_08_IT_MATERIAL_DOCS.MATDOC_MENGE AS B20_20_MATDOC_MENGE,
		B20_08_IT_MATERIAL_DOCS.ZF_MATDOC_DMBTR_S AS B20_20_ZF_MATDOC_DMBTR_S,
		B20_08_IT_MATERIAL_DOCS.MATDOC_USNAM AS B20_20_MATDOC_USNAM,
		B20_08_IT_MATERIAL_DOCS.MATDOC_CPUDT AS B20_20_MATDOC_CPUDT,
		...
		IIF(B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_RCLNT IS NULL, NULL, 
			CONCAT(B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_RCLNT,'|',B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_RBUKRS,'|',B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_GJAHR,'|',B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_BELNR)) B20_10_ZF_INVOICE_ACCOUNTING_DOC_KEY
		
		,B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_USNAM AS B20_20_INV_ACDOCA_USNAM
		,B20_10_IT_INVOICE_ACCOUTING_DOCS.KNA1_ERNAM AS B20_20_INV_KNA1_ERNAM
		
		--Add invoice accounting keys
		,IIF(B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_RCLNT IS NULL, NULL, 
			CONCAT(B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_RCLNT,'|',B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_RBUKRS,'|',B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_GJAHR,'|',B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_BELNR)) B20_11_ZF_CLEARING_INVOICE_DOC_KEY
		,B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_USNAM AS B20_20_PAYMENT_ACDOCA_USNAM
		...
		ON  B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_AWTYP = 'VBRK'
		AND B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_AWREF = B20_09_IT_INVOICE_DOCS.VBRK_VBELN

		-- --Get clearing document of VBRK invoice accounting document
		LEFT JOIN B20_11_IT_OTC_CLEARING_DOCS
		ON  B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_RBUKRS = B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_RBUKRS
		AND B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_GJAHR = B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_AUGGJ
		AND B20_11_IT_OTC_CLEARING_DOCS.ACDOCA_BELNR = B20_10_IT_INVOICE_ACCOUTING_DOCS.ACDOCA_AUGBL
		...
		*/
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_23_ZF_KNA1_ERNAM_SAME_VBAK_VBAP_ERNAM_SAME_MATDOC_USNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_23_ZF_KNA1_ERNAM_SAME_VBAK_VBAP_ERNAM_SAME_LIKP_LIPS_SAME_MATDOC_USNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_23_ZF_VBAK_VBAP_ERNAM_SAME_LIKP_LIPS_SAME_MATDOC_USNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_23_ZF_VBAK_VBAP_ERNAM_SAME_MATDOC_USNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		*/
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_24_ZF_KNA1_ERNAM_SAME_VBAK_VBAP_ERNAM_SAME_MATDOC_USNAM_SAME_VBRK_VBRP_ERNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_24_ZF_VBAK_VBAP_ERNAM_SAME_MATDOC_USNAM_SAME_VBRK_VBRP_ERNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_24_ZF_VBAK_VBAP_ERNAM_SAME_LIKP_LIPS_ERNAM_SAME_MATDOC_USNAM_SAME_VBRK_VBRP_ERNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		ALTER TABLE B20_20_CENTER_FACT_TABLE ADD B20_24_ZF_KNA1_ERNAM_SAME_VBAK_VBAP_ERNAM_SAME_LIKP_LIPS_ERNAM_SAME_MATDOC_USNAM_SAME_VBRK_VBRP_ERNAM_FLAG NVARCHAR(3) DEFAULT 'No' WITH VALUES;
		...
		EXEC SP_DROPTABLE 'B20_01_TT_SALE_DOCUMENTS'
END
