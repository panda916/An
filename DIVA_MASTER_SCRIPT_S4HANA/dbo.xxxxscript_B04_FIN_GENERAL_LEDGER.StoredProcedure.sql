...
CREATE   PROCEDURE [dbo].[xxxxscript_B04_FIN_GENERAL_LEDGER]
...
EXEC SP_REMOVE_TABLES  'B04_08_IT_FIN_GL'
            ...
            SELECT
                    ...
                    CONVERT(money,B04_05_IT_BKPF_ACDOCA.ACDOCA_HSL * AM_EXCHNG.EXCHNG_RATIO * ISNULL(TCURX_COC.TCURX_factor,1)) AS ZF_ACDOCA_HSL_S_CUC,
       ...
       -- Additional fields that are required for manual journal entries consideration
        ...
        -- Additional fields that are interesting for manual journal entry analysis
         ...
         ,(CONVERT(MONEY,B04_05_IT_BKPF_ACDOCA.ACDOCA_HSL * AM_EXCHNG.EXCHNG_RATIO * ISNULL(TCURX_COC.TCURX_factor,1)))
        ...
        LEFT JOIN AM_EXCHNG
        ON     (B04_05_IT_BKPF_ACDOCA.ACDOCA_RHCUR = AM_EXCHNG.EXCHNG_FROM) AND
...
EXEC SP_REMOVE_TABLES    'B04_09_IT_ACDOCA_BKPF_ACC_SCH'
					...
					FROM B04_05_IT_BKPF_ACDOCA B
					WHERE (B.ACDOCA_RBUKRS = B04_05_IT_BKPF_ACDOCA.ACDOCA_RBUKRS  
						AND B.ACDOCA_GJAHR = B04_05_IT_BKPF_ACDOCA.ACDOCA_GJAHR
						AND B.ACDOCA_BELNR = B04_05_IT_BKPF_ACDOCA.ACDOCA_BELNR
						AND    B.ACDOCA_DRCRK = 'S')
					GROUP BY B.ACDOCA_RACCT, B.ACDOCA_KOART
					ORDER BY SUM(ABS(B.ACDOCA_HSL)) DESC
					FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
					,1,1,''), '') ZF_DEBIT_ACCOUNT_TYPES
					...
					ORDER BY SUM(ABS(B.ACDOCA_HSL)) DESC
					FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
					,1,1,''), '') ZF_DEBIT_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG_LIST
					...
					ORDER BY SUM(ABS(B.ACDOCA_HSL)) DESC
					FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
					,1,1,''), '') ZF_CREDIT_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG_LIST
					...
					ORDER BY SUM(ABS(B.ACDOCA_HSL)) DESC
					FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
					,1,1,''), '') ZF_DEBIT_SKA1_GVTYP_FLAG_LIST
					...
					ORDER BY SUM(ABS(B.ACDOCA_HSL)) DESC
					FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
					,1,1,''), '') ZF_CREDIT_SKA1_GVTYP_FLAG_LIST
...
EXEC SP_REMOVE_TABLES    'B04_11_IT_AP_ACC_SCH'
       ...
       ,CONVERT(MONEY,ACDOCA_HSL * COALESCE(TCURX_CC.TCURX_FACTOR,1)  * EXCHNG_RATIO) AS ZF_ACDOCA_HSL_S_CUC
...
EXEC SP_REMOVE_TABLES 'B04_12_TT_AR_DOCUMENT'
       ...
       ,@currency      AS GLOBALS_CURRENCY
	   -- Signed amount in custom currency - values are inversed so that total spend shows as positive on the dashboard                                                                                                                                                                                AS AM_GLOBALS_CURRENCY
       ,CONVERT(MONEY,B04_12_TT_AR_DOCUMENT.ACDOCA_HSL * COALESCE(TCURX_CC.TCURX_FACTOR,1) * EXCHNG_RATIO)     AS ZF_ACDOCA_HSL_S_CUC
	   -- Document type description
       ,B00_T003T.T003T_LTEXT
	   --Posting key description
	   ,B00_TBSLT.TBSLT_LTEXT
