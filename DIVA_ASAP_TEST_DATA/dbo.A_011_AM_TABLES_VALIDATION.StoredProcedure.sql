...
-- =============================================
CREATE           PROCEDURE [dbo].[A_011_AM_TABLES_VALIDATION] 
AS
...
BEGIN
	...
	*/
	IF EXISTS (SELECT * FROM AM_SPEND_CATEGORY WHERE SPCAT_SPEND_CAT_LEVEL_1 IS NULL OR LEN(SPCAT_SPEND_CAT_LEVEL_1) = 0)
	BEGIN
		RAISERROR('***SOME G/L ACCOUNTS HAVE NOT BEEN UPDATED IN AM_SPEND_CATEGORY***',	20,	1) WITH LOG
	END
	...
	*/
	IF EXISTS (SELECT * FROM AM_IRGR_MATERIAL_GROUP_HIERARCHY WHERE IRGR_SPEND_CAT_LEVEL_1 IS NULL OR LEN(IRGR_SPEND_CAT_LEVEL_1) = 0)
	BEGIN
