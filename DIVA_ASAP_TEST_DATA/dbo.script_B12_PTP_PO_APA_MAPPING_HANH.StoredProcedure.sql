...
CREATE     PROC [dbo].[script_B12_PTP_PO_APA_MAPPING_HANH]
...
SELECT DB_NAME(),OBJECT_NAME(@@PROCID),'P',SYSTEM_USER,CONVERT(DATE,GETDATE()),CONVERT(TIME,GETDATE()),'Procedure started',NULL,NULL
	...
	SELECT DISTINCT
		...
		IIF(ISNULL(IRGR_GL_ACCT, '') <> '' AND ISNULL(A_T023T.T023T_WGBEZ,'') <> '',
				'P-' + CONCAT(A_EKPO.EKPO_MATKL, '-', A_T023T.T023T_WGBEZ),
	...
	FROM B04_11_IT_FIN_GL
		...
		--Get Material group from A_EKPO (PO) table
		LEFT JOIN A_EKPO
			ON EKPO_EBELN = B04_BSEG_EBELN
			AND EKPO_EBELP = B04_BSEG_EBELP
		
		--Get Material description

		LEFT JOIN B00_MAKT
			ON MAKT_MATNR = A_EKPO.EKPO_MATNR -- Change BSEG_MATNR to EKPO_MATNR 

		--Get material group description
		LEFT JOIN A_T023T
			ON T023T_MATKL = EKPO_MATKL
			AND T023T_SPRAS = 'EN'
		...
		LEFT JOIN AM_IRGR_MATERIAL_GROUP_HIERARCHY
			ON AM_IRGR_MATERIAL_GROUP_HIERARCHY.T023T_MATKL = A_EKPO.EKPO_MATKL
