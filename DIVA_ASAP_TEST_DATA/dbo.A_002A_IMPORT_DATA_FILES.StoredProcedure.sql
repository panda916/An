...
IF @DebugMsg = 'Y'
	...
	*/
	...
	SELECT * FROM AM_DD03L_STRUCTURE
...
--	Load staging results into stageDataLog
 ...
 -- HdG more informative error messages
    ...
    ******************************************************************/    
	...
	UPDATE #fielddefs
    SET
		[fielddef] = 'nvarchar(10)'
	WHERE  fieldname = 'SN'

	-- 2025-05-20 : Thuan update for all fields name have data type are date convert to nvachar(30) then will hanlde again in A_04E

	-- Step 1 / Create #fielddefs temp to store update datatype for number and date columns.

	SELECT *
	INTO #fielddefs_temp
	FROM #fielddefs

	-- Step 2 / With table have columns are datatype date need to update to nvarchar(30)

	UPDATE #fielddefs_temp
	SET [fielddef] = 'nvarchar(30)'
	WHERE fielddef LIKE 'date%'
	/*
		A_TCURR
		A_REGUH
		A_ANEP
		...
		A_LFC1
		A_KNC1
		A_CE1AM00 : COPA TABLE
		A_KNKK
	*/

	IF @tbl = 'TCURR' OR @tbl = 'REGUH' OR @tbl = 'ANEP' 
		OR @tbl = 'ANLC' OR @tbl = 'RSEG'
		OR @tbl = 'FAGLFLEXT' OR @tbl = 'GLT0'
		OR @tbl = 'LFC1' OR @tbl = 'KNC1'
		OR @tbl = 'KNKK' OR @tbl = 'BKPF'
	BEGIN
		PRINT 'The list of tables you are importing maybe have negative value ---- '
		-- Step 1. For field column related to number we need to update to nvarchar(30)
		UPDATE #fielddefs_temp
		SET [fielddef] = 'nvarchar(30)'
		WHERE fielddef IN ('money','int')
		OR fielddef LIKE 'decimal%'
