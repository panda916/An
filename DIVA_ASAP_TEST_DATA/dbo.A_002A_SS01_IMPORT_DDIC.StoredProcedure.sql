...
CREATE     PROCEDURE [dbo].[A_002A_SS01_IMPORT_DDIC] (
...
IF OBJECT_ID('TempDB..#ALLFILES_DDIC','U') IS NOT NULL DROP TABLE #ALLFILES_DDIC
CREATE TABLE #ALLFILES_DDIC ([filename] VARCHAR(255),Depth INT, isfile int)
INSERT #ALLFILES_DDIC EXEC xp_dirtree @path,1,1

IF @filename <> '' 
	DELETE FROM #ALLFILES_DDIC WHERE [filename] <> @filename
...
/*****************************************************************
CREATE TABLE FOR KPMG_LOGFILE IF KPMG_LOGFILE IN #ALLFILES_DDIC
******************************************************************/
...
/*****************************************************************
LOOP THROUGH EACH TABLE IN #ALLFILES_DDIC
...
IF @DebugMsg = 'Y'
	...
	ALTER TABLE #ALLFILES_DDIC Add TableNameIndex varchar(255)
	ALTER TABLE #ALLFILES_DDIC Add RowNo Int

	--UPDATE #ALLFILES_DDIC SET TableNameIndex  = left([filename],PATINDEX('%_[0][0][0-9][0-9]%.txt',[filename])-1)
	UPDATE #ALLFILES_DDIC 
	SET TableNameIndex  = REPLACE(SUBSTRING([filename], CHARINDEX('_', [filename]) + 1, LEN([filename])),'.txt','')
	WHERE
	...
	UPDATE #ALLFILES_DDIC SET Rowno = B.RowNo
	FROM #ALLFILES_DDIC A
	INNER JOIN (SELECT FileName,ROW_NUMBER() OVER(Partition by TableNameIndex order by TableNameIndex) RowNo FROM #ALLFILES_DDIC
	...
	*/
	UPDATE AM_DD03L_STRUCTURE
	SET INTLEN = INTLEN*3, LENG = LENG*3
	WHERE DATATYPE IN ('RAW', 'LRAW', 'FLTP', 'NUMC')
...
--	Load staging results into stageDataLog
  ...
  IF @tbl not like 'KPMG_LOG_RECORDCOUNT%'
	BEGIN
		...
		SET @tbl = 	REPLACE(SUBSTRING(@tbl, CHARINDEX('_', @tbl) + 1, LEN(@tbl)),'.txt','')
	END
  IF @tbl like '%_RSEG' 
  BEGIN
 ...
 -- HdG more informative error messages
   ...
   SET @SAPTable = (CASE WHEN CHARINDEX('-',@tbl) <> 0 THEN SUBSTRING(@tbl,0,CHARINDEX('-',@tbl)) ELSE @tbl END) 
    ...
    INTO #fielddefs
    FROM #fields t1 
    LEFT JOIN AM_DD03L_STRUCTURE t2
  ...
  IF @DebugMsg = 'Y'
	...
	BEGIN
	  ...
	  --SET @tbl = REPLACE(@tbl, '','-tmp') --HdG corrected into:
	    ...
	    EXEC dbo.[A_002A_SS02_REMOVE_DUPLICATES_DDIC] @Tbl
