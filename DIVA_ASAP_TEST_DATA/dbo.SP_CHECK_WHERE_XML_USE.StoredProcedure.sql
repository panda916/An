...
GO
CREATE PROCEDURE [dbo].[SP_CHECK_WHERE_XML_USE]
...
BEGIN
	...
	DELETE dbo.B_WHERE_XML_USE_RESULT
		...
		BEGIN
			DECLARE XML_TABNAME_CURSOR CURSOR LOCAL FOR 
				SELECT DISTINCT XML_NAME, TABLE_NAME, FIELD_NAME
					...
					ORDER BY XML_NAME, TABLE_NAME, FIELD_NAME
		END
	...
	FETCH NEXT FROM XML_TABNAME_CURSOR INTO @XML_NAME, @TABLE_NAME, @FIELD_NAME
	WHILE (@@FETCH_STATUS = 0)
	BEGIN
		SET @SEARCH_TEXT = @TABLE_NAME + '_' + @FIELD_NAME;
	...
	IF (@SearchXMLName = 'ALL')
		INSERT INTO dbo.B_WHERE_XML_USE_RESULT( XML_NAME, TABLE_NAME, FIELD_NAME, SEARCH_TEXT, SP_NAME)
			SELECT A.XML_NAME, A.TABLE_NAME, A.FIELD_NAME, A.TABLE_NAME + '_' + A.FIELD_NAME AS SEARCH_TEXT, B.SP_NAME
			FROM dbo.A_XML_TABLE_FIELD_DATA AS A LEFT JOIN dbo.B_WHERE_XML_USE_RESULT_TEMP AS B
			ON A.XML_NAME = B.XML_NAME AND A.TABLE_NAME = B.TABLE_NAME AND A.TABLE_NAME + '_' + A.FIELD_NAME = B.SEARCH_TEXT
			ORDER BY A.XML_NAME, A.TABLE_NAME, A.FIELD_NAME
	ELSE
		INSERT INTO dbo.B_WHERE_XML_USE_RESULT( XML_NAME, TABLE_NAME, FIELD_NAME, SEARCH_TEXT, SP_NAME)
			SELECT A.XML_NAME, A.TABLE_NAME, A.FIELD_NAME, A.TABLE_NAME + '_' + A.FIELD_NAME AS SEARCH_TEXT, B.SP_NAME
			FROM dbo.A_XML_TABLE_FIELD_DATA AS A LEFT JOIN dbo.B_WHERE_XML_USE_RESULT_TEMP AS B
	...
	SELECT *FROM dbo.B_WHERE_XML_USE_RESULT
	ORDER BY XML_NAME, TABLE_NAME, FIELD_NAME, SP_NAME
