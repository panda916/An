...
CREATE         PROCEDURE [dbo].[A_004H_UPDATE_NEGATIVE_SIGN_ASAP] 
...
-- Step 2/ For each table update the date format
  ...
  DECLARE c CURSOR FOR
		SELECT sys.objects.name as SAP_Table, sys.columns.name as SAP_Field ,
		CASE ECC.INTTYPE
			WHEN 'D' then 'date'
			WHEN 'I' then 'int'
			WHEN 'P' THEN CASE WHEN ECC.DATATYPE = 'CURR' THEN 'money' ELSE 'decimal(' + ECC.LENG + ', ' + ECC.DECIMALS + ' )' END
		...
		WHERE sys.objects.type = 'U' AND ECC.INTTYPE in ('I', 'P')
    ...
    OPEN c
    FETCH NEXT FROM c INTO @TableName, @FieldName,@Fielddef
    ...
    BEGIN
        ...
        BEGIN TRY
		  ...
		  PRINT '------------------- Step 1 -- Convert the negative value to corect values --------------------------------'
			...
			SET @UpdateStatement = 
				...
				SET [' + @FieldName + '] = 
					CASE 
						WHEN ['+ @FieldName +'] LIKE ''%-'' 
						THEN ''-'' + LEFT(   ['+ @FieldName +'], LEN(['+ @FieldName +']) - 1)
						ELSE ['+ @FieldName +']
					END
		  ...
		  PRINT '------------------- Step 2 -- Convert the date type to date--------------------------------'
			 ...
			 SET @UpdateStatement_type = 
				...
				'ALTER TABLE [' + @TableName + '] ALTER COLUMN [' + @FieldName + '] ' + @Fielddef;
