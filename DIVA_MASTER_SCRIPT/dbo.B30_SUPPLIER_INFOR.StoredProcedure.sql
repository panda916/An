...
GO
CREATE     PROC [dbo].[B30_SUPPLIER_INFOR]
AS
...
into z_databases
	FROM master.dbo.sysdatabases db inner join DIVA_MASTER_SCRIPT..AM_SUPPLIER_MAPPING on db.[name] = [database]
...
BEGIN
	...
	BEGIN
		...
		IF (@database_name LIKE 'USCULCAASQL03%')
			BEGIN
				EXEC script_B30_GPW_BSAIK_ECC_SUPPLIER_INFO @database_name
			END
		ELSE IF (not @database_name LIKE '%SIE%') AND (not @database_name LIKE '%SEV%')
			BEGIN
				EXEC SP_REMOVE_TABLES_MASTER @database_name, 'B30_%'
				EXEC SP_EXEC_DYNAMIC @database_name, 'script_B30_BSAIK_ECC_SUPPLIER_INFO'
			END
		ELSE
			BEGIN
				EXEC SP_REMOVE_TABLES_MASTER @database_name, 'B30_%'
				EXEC SP_EXEC_DYNAMIC @database_name, 'script_B30_ACDOCA_SUPPLIER_INFO'
			END
	END
