...
GO
CREATE   PROC [dbo].[script_B28_BANK_CHANGE]
...
--Step 1: join the CDHDR with CDPOS and add filters to get relevant changes to LFBK, TIBAN/IBAN table
	...
	EXEC SP_REMOVE_TABLES 'B34%TT%'

	EXEC SP_DROPTABLE 'B34_01_TT_CDHDR_CDPOS'
	...
	EXEC SP_DROPTABLE 'B34_02_TT_CDHDR_CDPOS_LFBK_SCOPE'
	SELECT DISTINCT A.CDHDR_OBJECTID, A.CDHDR_CHANGENR, A.CDHDR_OBJECTCLAS 
	INTO B34_02_TT_CDHDR_CDPOS_LFBK_SCOPE
	FROM B34_01_TT_CDHDR_CDPOS A
	WHERE CDPOS_CHNGIND = 'I'
		AND CDPOS_FNAME = 'KEY'
		AND EXISTS(SELECT * FROM B34_01_TT_CDHDR_CDPOS B WHERE A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
...
*/
	EXEC SP_DROPTABLE 'B34_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE'
	SELECT DISTINCT A.*
	INTO B34_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE
	FROM B34_01_TT_CDHDR_CDPOS A
	INNER JOIN B34_02_TT_CDHDR_CDPOS_LFBK_SCOPE B ON B.CDHDR_CHANGENR = A.CDHDR_CHANGENR
...
-- because when users change or delete key fields, SAP will update and 
	EXEC SP_DROPTABLE 'B34_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED'
	SELECT DISTINCT A.*,
	A.ZF_LAST_KEY_CHANGE ZF_VALUE_NEW,
	B.ZF_LAST_KEY_CHANGE ZF_VALUE_OLD

	INTO B34_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED
	FROM B34_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE A
	LEFT JOIN B34_03_TT_CDHDR_CDPOS_LFBK_FLIPFLOP_SCOPE B ON A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
															AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
...
-- Step 4: from the combined result above, perform a normal with the table itself to get flipflop cases (old value of table 1 should be the new change value of table 2)
	EXEC SP_DROPTABLE 'B34_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP'
	SELECT DISTINCT 
	...
	INTO B34_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP
	FROM B34_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED A
	INNER JOIN B34_04_TT_CDHDR_CDPOS_SCOPE_VALUE_OLD_NEW_COMBINED B ON A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS 
														AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
														AND A.CDPOS_TABNAME = B.CDPOS_TABNAME 
...
-- This table will manage many to many relation ship, in case that users want to get all relevant data from original table, including all the lines between two change session.
	EXEC SP_DROPTABLE 'B34_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING'
	CREATE TABLE B34_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING
	(
...
-- Step 6: Insert all result to above table
	INSERT INTO B34_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING
	...
	ZF_CHANGE_ID,
	ZF_LIFNR_1
	FROM B34_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP A
	UNION
	...
	ZF_CHANGE_ID_2,
	ZF_LIFNR_1
	FROM B34_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP B
	UNION
	...
	ZF_LIFNR_1
	FROM B34_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP B
	LEFT JOIN B34_01_TT_CDHDR_CDPOS C ON B.CDHDR_OBJECTID = C.CDHDR_OBJECTID
...
-- Step 4: join the table with itself to get flipflop cases (old value of table 1 should be the new change value of table 2)
	EXEC SP_DROPTABLE 'B34_06_TT_CDHDR_CDPOS_IBAN'
	SELECT DISTINCT
	...
	INTO B34_06_TT_CDHDR_CDPOS_IBAN
	FROM B34_01_TT_CDHDR_CDPOS A
	INNER JOIN B34_01_TT_CDHDR_CDPOS B ON A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
															AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
															AND A.CDPOS_TABNAME = B.CDPOS_TABNAME
															AND A.CDPOS_FNAME = B.CDPOS_FNAME
...
-- Step 6: Insert all result to above table
	...
	ZF_CHANGE_ID,
	ZF_LFBK_LIFNR_OLD_1
	FROM B34_06_TT_CDHDR_CDPOS_IBAN A
	UNION
-- Insert all 2nd change (changing back to the bank account of the original)
	SELECT DISTINCT
	ZF_CASE_TYPE,
...
-- Step 7: join the CDHDR_CDPOS table with itself where the 2nd table has change indicator = D, E
	...
	SELECT DISTINCT
		...
		SUBSTRING(A.CDPOS_TABKEY, 4, 10) ZF_LIFNR_1,
		DBO.TRIM(SUBSTRING(A.CDPOS_TABKEY, 14, LEN(A.CDPOS_TABKEY)- 14)) ZF_BANK_DETAIL
	INTO B34_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE
	FROM B34_01_TT_CDHDR_CDPOS A
	INNER JOIN B34_01_TT_CDHDR_CDPOS B ON B.CDPOS_CHNGIND IN ('D', 'E') AND  A.CDHDR_OBJECTCLAS = B.CDHDR_OBJECTCLAS
															AND A.CDHDR_OBJECTID = B.CDHDR_OBJECTID
															AND A.CDPOS_TABNAME = B.CDPOS_TABNAME
															AND A.CDHDR_CHANGENR < B.CDHDR_CHANGENR
	...
	SELECT DISTINCT
		...
		ZF_CHANGE_ID,
		ZF_LIFNR_1
	FROM B34_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE A
	UNION
	SELECT DISTINCT
		ZF_CASE_TYPE,
		ZF_CHANGE_ID,
		ZF_CHANGE_ID_2,
		ZF_LIFNR_1
	FROM B34_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE B


								

	EXEC SP_DROPTABLE 'B34_01_IT_CDHDR_CDPOS'
	SELECT * INTO B34_01_IT_CDHDR_CDPOS FROM B34_01_TT_CDHDR_CDPOS
    CREATE NONCLUSTERED INDEX IDX ON B34_01_IT_CDHDR_CDPOS(ZF_CUSTOMER_SUPPLIER_KEY)
	EXEC SP_DROPTABLE 'B34_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP'
	SELECT * INTO B34_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP FROM B34_05_TT_CDHD_CDPOS_LFBK_FLIPFLOP
	EXEC SP_DROPTABLE 'B34_06C_IT_CDHDR_CDPOS_IBAN'
	SELECT * INTO B34_06C_IT_CDHDR_CDPOS_IBAN FROM B34_06_TT_CDHDR_CDPOS_IBAN

	EXEC SP_DROPTABLE 'B34_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE'
	SELECT * INTO B34_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE FROM B34_08_TT_CDHDR_CDPOS_LFBK_BANK_DELETE


	EXEC SP_RENAME_FIELD 'B34_', 'B34_01_IT_CDHDR_CDPOS'
	EXEC SP_RENAME_FIELD 'B34A_', 'B34_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP'
	EXEC SP_RENAME_FIELD 'B34B_', 'B34_05B_IT_CDHD_CDPOS_RELEVANT_CASES_MAPPING'
	EXEC SP_RENAME_FIELD 'B34C_', 'B34_06C_IT_CDHDR_CDPOS_IBAN'
	EXEC SP_RENAME_FIELD 'B34E_', 'B34_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE'

	DECLARE @CURRENCY NVARCHAR(MAX) = (SELECT GLOBALS_VALUE FROM [AM_GLOBALS] WHERE GLOBALS_PARAMETER = 'currency')

	EXEC SP_DROPTABLE 'B34_10_TT_BSAIK_DETAIL'
	SELECT A.*,
	B.B34A_ZF_CASE_TYPE ZF_CASE_TYPE,
	B.B34A_ZF_CHANGE_ID ZF_CHANGE_ID
	INTO B34_10_TT_BSAIK_DETAIL
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B34_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP B ON B11B_BSAIK_LIFNR = B.B34A_ZF_LIFNR_1
												AND B11B_BSAIK_BLDAT BETWEEN B34A_ZF_CDHDR_UDATE_1 AND B.B34A_ZF_CDHDR_UDATE_2
	WHERE B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	C.B34E_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B34E_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B34_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE C ON B11B_BSAIK_LIFNR = C.B34E_ZF_LIFNR_1
												AND B11B_BSAIK_BLDAT BETWEEN c.B34E_ZF_CDHDR_UDATE_1 AND C.B34E_ZF_CDHDR_UDATE_2

	WHERE B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	C.B34C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_BSAIK_LIFNR = B34C_ZF_LFBK_LIFNR_NEW_1
												AND B11B_BSAIK_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	WHERE B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	C.B34C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_BSAIK_LIFNR = B34C_ZF_LFBK_LIFNR_OLD_1
												AND B11B_BSAIK_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	WHERE B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	...
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_BSAIK_LIFNR = B34C_ZF_LFBK_LIFNR_NEW_2
												AND B11B_BSAIK_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	WHERE B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	UNION
	SELECT A.*,
	...
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM B11_04_IT_PTP_APA A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON B11B_BSAIK_LIFNR = B34C_ZF_LFBK_LIFNR_OLD_2
												AND B11B_BSAIK_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	WHERE B11B_ZF_FLAG_SUMMARY like '%PAYMENT%'
	
	EXEC SP_DROPTABLE 'B34_11_IT_BSAIK_DETAIL'
	SELECT A.*
	INTO B34_11_IT_BSAIK_DETAIL
	FROM B34_10_TT_BSAIK_DETAIL A

	EXEC SP_UNNAME_FIELD 'B11B_', 'B34_11_IT_BSAIK_DETAIL'
    EXEC SP_RENAME_FIELD 'B34G_', 'B34_11_IT_BSAIK_DETAIL'


	EXEC SP_DROPTABLE 'B34_12_TT_REGUP_DETAIL'
	SELECT A.*,
	B.B34A_ZF_CASE_TYPE ZF_CASE_TYPE,
	B.B34A_ZF_CHANGE_ID ZF_CHANGE_ID
	INTO B34_12_TT_REGUP_DETAIL
	FROM A_REGUP A
	INNER JOIN B34_04A_IT_CDHD_CDPOS_LFBK_FLIPFLOP B ON REGUP_LIFNR = B.B34A_ZF_LIFNR_1
												AND REGUP_BLDAT BETWEEN B34A_ZF_CDHDR_UDATE_1 AND B.B34A_ZF_CDHDR_UDATE_2
	UNION
	SELECT A.*,
	C.B34E_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B34E_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B34_08E_IT_CDHDR_CDPOS_LFBK_BANK_DELETE C ON REGUP_LIFNR = C.B34E_ZF_LIFNR_1
												AND REGUP_BLDAT BETWEEN c.B34E_ZF_CDHDR_UDATE_1 AND C.B34E_ZF_CDHDR_UDATE_2
	...
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B34C_ZF_LFBK_LIFNR_NEW_1
												AND REGUP_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	UNION
	SELECT A.*,
	C.B34C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B34C_ZF_LFBK_LIFNR_OLD_1
												AND REGUP_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	UNION
	SELECT A.*,
	C.B34C_ZF_CASE_TYPE ZF_CASE_TYPE,
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B34C_ZF_LFBK_LIFNR_NEW_2
												AND REGUP_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2
	...
	C.B34C_ZF_CHANGE_ID ZF_CHANGE_ID
	FROM A_REGUP A
	INNER JOIN B34_06C_IT_CDHDR_CDPOS_IBAN C ON REGUP_LIFNR = B34C_ZF_LFBK_LIFNR_OLD_2
												AND REGUP_BLDAT BETWEEN C.B34C_ZF_CDHDR_UDATE_1 AND C.B34C_ZF_CDHDR_UDATE_2

	EXEC SP_DROPTABLE 'B34_14_IT_REGUP_DETAIL'
	SELECT A.*,
	...
	A_T001.T001_KTOPL
	INTO B34_14_IT_REGUP_DETAIL
	FROM B34_12_TT_REGUP_DETAIL A
	LEFT JOIN A_T001
	...
	LEFT JOIN B00_IT_TCURF TCURF_CUC
		...
		WHERE A_T001.T001_WAERS = B00_IT_TCURF.TCURF_FCURR AND 
				...
				B00_IT_TCURF.TCURF_GDATU <= REGUP_LAUFD
	...
	LEFT JOIN B00_IT_TCURR TCURR_CUC
		...
		AND TCURR_CUC.TCURR_GDATU = (
			...
			WHERE A_T001.T001_WAERS = B00_IT_TCURR.TCURR_FCURR AND 
					...
					B00_IT_TCURR.TCURR_GDATU <= REGUP_LAUFD
	...
	EXEC SP_RENAME_FIELD 'B34H_', 'B34_14_IT_REGUP_DETAIL'
	
	EXEC SP_DROPTABLE 'B34_15_IT_KNA1'
    SELECT * 
    INTO B34_15_IT_KNA1
    FROM A_KNA1
    WHERE EXISTS(SELECT * FROM A_KNB1 WHERE KNA1_KUNNR = KNB1_KUNNR
                    AND EXISTS(SELECT * FROM A_BKPF WHERE BKPF_BUKRS = KNB1_BUKRS))
        OR EXISTS(SELECT * FROM B34_01_IT_CDHDR_CDPOS WHERE B34_ZF_CUSTOMER_SUPPLIER_KEY = KNA1_KUNNR)

	EXEC SP_DROPTABLE 'B34_16_IT_LFA1'
    SELECT * 
    INTO B34_16_IT_LFA1
    FROM A_LFA1
    WHERE EXISTS(SELECT * FROM A_LFB1 WHERE LFA1_LIFNR = LFB1_LIFNR
                    AND EXISTS(SELECT * FROM A_BKPF WHERE BKPF_BUKRS = LFB1_BUKRS))
        OR EXISTS(SELECT * FROM B34_01_IT_CDHDR_CDPOS WHERE B34_ZF_CUSTOMER_SUPPLIER_KEY = LFA1_LIFNR)
GO
