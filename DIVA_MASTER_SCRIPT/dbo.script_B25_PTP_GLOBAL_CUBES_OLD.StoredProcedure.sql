...
--
-- =============================================
CREATE     PROCEDURE [dbo].[script_B25_PTP_GLOBAL_CUBES_OLD]
...
B24_02_MKPF_BLDAT AS MKPF_BLDAT,--Good receipt date
...
B.B04_BKPF_BLDAT AS BSEG_BLDAT_INV,--Invoice document date
...
C.B11B_SS02_05_BSAIK_BLDAT AS BSAIK_BLDAT_PAY,--Payment document date
...
INTO B25_01_IT_PTP_GLOBAL_TABLE
...
FULL OUTER JOIN 
(SELECT A_RSEG.*,A_RBKP.*,V_USERNAME_NAME_TEXT, T003T_LTEXT, RBKP_KURSF * COALESCE(TCURF_COC.TCURF_TFACT,1)/COALESCE(TCURF_COC.TCURF_FFACT,1) * COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1) AS ZF_EXCHNG_RATIO,TCURX_FACTOR FROM A_RSEG
...
ON 	
	B.B04_BSEG_EBELN=RSEG_EBELN
AND B.B04_BSEG_EBELP=RSEG_EBELP
AND B.B04_ZF_BKPF_AWKEY_DOC_NUM=RSEG_BELNR 
...
FULL OUTER JOIN B11_SS02_05_IT_PTP_APA_PAY_GROUP AS C
	ON B.B04_BSEG_AUGBL =  C.B11B_SS02_05_BSAIK_AUGBL 
	AND B.B04_BSEG_AUGDT=  C.B11B_SS02_05_BSAIK_AUGDT 
	AND B.B04_BSEG_BUKRS= C.B11B_SS02_05_BSAIK_BUKRS
	AND (B.B04_BSEG_AUGBL<>'' AND B11B_SS02_05_BSAIK_AUGBL<>'')
