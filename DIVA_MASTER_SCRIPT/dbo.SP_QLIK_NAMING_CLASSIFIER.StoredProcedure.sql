...
-- 3.8.1 DV01:  For RTR Trial Balance, Pivot table 1: (RTRTB_PIV1_)
		...
		IF (CHARINDEX('GL account', @TABLE_FIELD) > 0) SET @HASRTRTB_PIV1_GL = 'Y'
		IF (CHARINDEX('opening balance', @TABLE_FIELD) > 0) SET @HASRTRTB_PIV1_OPENING = 'Y'
		IF (CHARINDEX('tb debit movements', @TABLE_FIELD) > 0) SET @HASRTRTB_PIV1_DEBIT = 'Y'
		IF (CHARINDEX('tb credit movements', @TABLE_FIELD) > 0) SET @HASRTRTB_PIV1_CREDIT = 'Y'
		IF (CHARINDEX('bal value', @TABLE_FIELD) > 0) SET @HASRTRTB_PIV1_BAL = 'Y'
		--IF (CHARINDEX('tb', @TABLE_FIELD) > 0) SET @HASRTRTB_PIV1_TB = 'Y'
