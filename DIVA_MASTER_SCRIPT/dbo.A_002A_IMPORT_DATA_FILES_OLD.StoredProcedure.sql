...
CREATE PROCEDURE [dbo].[A_002A_IMPORT_DATA_FILES_OLD] (
...
IF @DebugMsg = 'Y'
	...
	--UPDATE #allfiles SET TableNameIndex  = left([filename],PATINDEX('%_[0][0][0-9][0-9]%.txt',[filename])-1)
	UPDATE #allfiles 
	SET TableNameIndex  = CASE WHEN PATINDEX('%_[0][0][0-9][0-9]_ARCHIVE%.txt',[filename]) > 0 THEN left([filename],PATINDEX('%_[0][0][0-9][0-9]_ARCHIVE%.txt',[filename])-1)
							WHEN PATINDEX('%_RSEG_[0][0][0-9][0-9]%.txt',[filename]) > 0 THEN left([filename],PATINDEX('%_RSEG_[0][0][0-9][0-9]%.txt',[filename])-1)
							WHEN PATINDEX('%_VBRP_[0][0][0-9][0-9]%.txt',[filename]) > 0 THEN left([filename],PATINDEX('%_VBRP_[0][0][0-9][0-9]%.txt',[filename])-1)
							WHEN PATINDEX('%AGR_1016%',[filename]) > 0 OR PATINDEX('%AGR_1251%',[filename]) > 0 THEN left([filename], 8)
							WHEN PATINDEX('%PA0000%',[filename]) > 0 OR PATINDEX('%PA0001%',[filename]) > 0 THEN left([filename], 6)
							ELSE left([filename],PATINDEX('%_[0][0][0-9][0-9]%.txt',[filename])-1) END
	WHERE
  ...
  SELECT @MaxID = Max(RowNO) FROM #allfiles WHERE TableNameIndex= @TableIndex
...
--	Load staging results into stageDataLog
  ...
  IF @tbl not like 'KPMG_LOG_RECORDCOUNT%'
	BEGIN
		...
		SET @tbl = CASE WHEN PATINDEX('%_[0][0][0-9][0-9]_ARCHIVE%.txt',@tbl) > 0 THEN left(@tbl,PATINDEX('%_[0][0][0-9][0-9]_ARCHIVE%.txt',@tbl)-1)
							WHEN PATINDEX('%_RSEG_[0][0][0-9][0-9]%.txt',@tbl) > 0 THEN left(@tbl,PATINDEX('%_RSEG_[0][0][0-9][0-9]%.txt',@tbl)-1)
							WHEN PATINDEX('%_VBRP_[0][0][0-9][0-9]%.txt',@tbl) > 0 THEN left(@tbl,PATINDEX('%_VBRP_[0][0][0-9][0-9]%.txt',@tbl)-1)
							WHEN PATINDEX('%AGR_1016%',@tbl) > 0 OR PATINDEX('%AGR_1251%',@tbl) > 0 THEN left(@tbl, 8)
							WHEN PATINDEX('%PA0000%',@tbl) > 0 OR PATINDEX('%PA0001%',@tbl) > 0 THEN left(@tbl, 6)
								ELSE left(@tbl,PATINDEX('%_[0][0][0-9][0-9]%.txt',@tbl)-1) END
	END
  IF @tbl like '%_RSEG' 
  BEGIN
 ...
 -- HdG more informative error messages
   ...
   SET @SAPTable = (CASE WHEN CHARINDEX('-',@tbl) <> 0 THEN SUBSTRING(@tbl,0,CHARINDEX('-',@tbl)) ELSE @tbl END) 
    ...
    INTO #fielddefs
    FROM #fields t1 
    LEFT JOIN zzSAP_DDIC..TURKEY_DD03L t2
