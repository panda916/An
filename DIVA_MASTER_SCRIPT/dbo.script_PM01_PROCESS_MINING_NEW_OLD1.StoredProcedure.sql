...
GO
CREATE       PROCEDURE [dbo].[script_PM01_PROCESS_MINING_NEW_OLD1]
AS
...
SELECT  ZF_DOCUMENT_FLOW_KEY AS ZF_DOCUMENT_FLOW_KEY,
		...
		MKPF_BLDAT AS  'ZF_DATE_USED_IN_MODEL',
...
FROM PM_01_TT_ADD_FLAG_GLOBAL
WHERE ISNULL(MKPF_BLDAT,'')<>''

UNION
SELECT  ZF_DOCUMENT_FLOW_KEY AS ZF_DOCUMENT_FLOW_KEY,
		ZF_RSEG_BELNR_GJAHR_BUZEI AS ZF_DOCUMENT_NUMBER,
		RBKP_BLDAT AS  'ZF_DATE_USED_IN_MODEL',
...
FROM PM_01_TT_ADD_FLAG_GLOBAL
WHERE ISNULL(RBKP_BLDAT,'')<>''
UNION 
SELECT  ZF_DOCUMENT_FLOW_KEY AS ZF_DOCUMENT_FLOW_KEY,
		ZF_BSEG_BUKRS_GJAHR_BELNR_BUZEI AS ZF_DOCUMENT_NUMBER,
		BSEG_BLDAT_INV AS  'ZF_DATE_USED_IN_MODEL',
...
FROM PM_01_TT_ADD_FLAG_GLOBAL
WHERE ISNULL(BSEG_BLDAT_INV,'')<>''
UNION 
SELECT  ZF_DOCUMENT_FLOW_KEY AS ZF_DOCUMENT_FLOW_KEY,
		ZF_BSAIK_BUKRS_GJAHR_BELNR_BUZEI_PAY AS ZF_DOCUMENT_NUMBER,
		BSAIK_BLDAT_PAY AS  'ZF_DATE_USED_IN_MODEL',
...
--Step 6 Sorting the the table base on the document_flow_key, dates, and stage
	...
	*,'' AS ZF_PO_HAS_PRICE_CHANGE INTO PM_06_TT_GLOBAL_VERTICAL_SORT FROM PM_05_TT_VERTICAL_ADD_CHANGE
	...
	ON PM_06_TT_GLOBAL_VERTICAL_SORT(ZF_DOCUMENT_FLOW_KEY, ZF_DATE_USED_IN_MODEL, ZF_STAGE)
...
CREATE CLUSTERED INDEX PM_05_TT_PROCESS_MINING ON PM_08_TT_NODE_FROM_TO_SORT(ZF_DOCUMENT_FLOW_KEY, ZF_DATE_USED_IN_MODEL,ZF_STAGE)
...
FROM B25_01_IT_PTP_GLOBAL_TABLE
 WHERE BSEG_BLDAT_INV IS NOT NULL
...
FROM B25_01_IT_PTP_GLOBAL_TABLE
 WHERE BSAIK_BLDAT_PAY IS NOT NULL
...
SELECT DISTINCT ZF_INTEGER_FOR_USER,
'' AS ZF_OTC_FLAG,
