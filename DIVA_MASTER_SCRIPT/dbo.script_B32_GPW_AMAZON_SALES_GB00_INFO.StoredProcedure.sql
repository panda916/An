...
CREATE     PROCEDURE [dbo].[script_B32_GPW_AMAZON_SALES_GB00_INFO](@database_name NVARCHAR(1000))
...
-- Step 1: check if target database has required field and tables.
	...
	DECLARE @SQLCMD NVARCHAR(1000) ='SELECT TOP 1 @RESULT = 1 FROM ' + @database_name + '.DBO.[_CUBE_OTC-07-COPA-transactions]'
...
BEGIN 
	...
	EXEC SP_DROPTABLE 'B32_GPW_COPA_TRAN'
	SET @SQLCMD = 
	'
		SELECT   A.*, WAERS
			INTO B32_GPW_COPA_TRAN 
		FROM ' + @database_name + '.DBO.[_CUBE_OTC-07-COPA-transactions] A
