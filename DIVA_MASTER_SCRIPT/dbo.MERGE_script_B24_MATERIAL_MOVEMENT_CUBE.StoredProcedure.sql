...
-- =============================================
CREATE     PROCEDURE [dbo].[MERGE_script_B24_MATERIAL_MOVEMENT_CUBE] 
...
SELECT DISTINCT
   ...
   CASE WHEN DBO.TRIM(MSEG_SHKZG)='S' THEN MSEG_SALK3*COALESCE(TCURX_FACTOR,1)
	ELSE MSEG_SALK3*-1*COALESCE(TCURX_FACTOR,1) END AS ZF_MSEG_SALK3_S,  -- Added from SAP USAGE
   CASE WHEN DBO.TRIM(MSEG_SHKZG)='S' THEN MSEG_SALK3*COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1)*COALESCE(TCURX_FACTOR,1)
	ELSE MSEG_SALK3*-1*COALESCE(CAST(TCURR_CUC.TCURR_UKURS AS FLOAT),1) * COALESCE(TCURF_CUC.TCURF_TFACT,1) / COALESCE(TCURF_CUC.TCURF_FFACT,1)*COALESCE(TCURX_FACTOR,1) END AS ZF_MSEG_SALK3_S_CUC,--signed amount caculation  -- Added from SAP USAGE
...
-- Added from SAP USAGE - Get customer name from KNA1 table
LEFT JOIN A_KNA1
...
-- Added from SAP USAGE - Get profit center description
LEFT JOIN A_CEPCT
...
--Added from SAP USAGE - Get movement type - text
LEFT JOIN A_T156HT
...
-- Added from SAP USAGE- Get material group - desription
LEFT JOIN A_T023T
