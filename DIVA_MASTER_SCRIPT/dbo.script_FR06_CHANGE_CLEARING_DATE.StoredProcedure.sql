...
----------------------------------------------------------------------------------------------------------------------------
			...
			,@custom_currency_field NVARCHAR(10)  = (SELECT GLOBALS_VALUE FROM [AM_GLOBALS] WHERE GLOBALS_PARAMETER = 'custom_currency_field')
...
-- Restrict to those that are only for the company codes in scope
	...
	SELECT DISTINCT
		...
		CDPOS_CHANGENR,
		CDPOS_OBJECTCLAS,
		CONVERT(DATETIME, CDPOS_VALUE_OLD, 112) CDPOS_VALUE_OLD,
		CDPOS_VALUE_NEW, --IIF(CDPOS_VALUE_NEW='00000000','',CDPOS_VALUE_NEW) CDPOS_VALUE_NEW, --CONVERT(DATETIME, CDPOS_VALUE_NEW, 112) CDPOS_VALUE_NEW,
		CDPOS_CHNGIND,
		CDPOS_TABNAME,
		CDPOS_FNAME,
		CDHDR_OBJECTID,
...
--          - that are within the date range from AM_GLOBALS
    ...
    SELECT
		...
		,CAST(YEAR(CDHDR_UDATE) + ISNULL(B00_T009B_A.T009B_RELJR, B00_T009B_B.T009B_RELJR) AS VARCHAR (4)) ZF_CDHDR_UDATE_RELJR_FY  
		-- Fiscal period relating to the change
		,CASE
			WHEN ISNULL(B00_T009B_A.T009B_PERIV, B00_T009B_B.T009B_PERIV) <> '' THEN DBO.ADD_LEADING_ZEROES(ISNULL(B00_T009B_A.T009B_POPER, B00_T009B_B.T009B_POPER))
			ELSE DBO.ADD_LEADING_ZEROES(MONTH(CDHDR_UDATE))
		END ZF_T009B_POPER_CDHDR_UDATE_FP
		-- Fiscal quarter relating to the change 
		,CASE 
			WHEN ISNULL(B00_T009B_A.T009B_POPER, B00_T009B_B.T009B_POPER) IN ('1','2','3') THEN 'Q1'
			WHEN ISNULL(B00_T009B_A.T009B_POPER, B00_T009B_B.T009B_POPER) IN ('4','5','6') THEN 'Q2'
			WHEN ISNULL(B00_T009B_A.T009B_POPER, B00_T009B_B.T009B_POPER) IN ('7','8','9') THEN 'Q3'
			WHEN ISNULL(B00_T009B_A.T009B_POPER, B00_T009B_B.T009B_POPER) IN ('10','11','12') THEN 'Q4'
	...
	ON (A_T001.T001_PERIV = B00_T009B_A.T009B_PERIV) AND
	   MONTH(CDHDR_UDATE) = B00_T009B_A.T009B_BUMON AND
	   YEAR(CDHDR_UDATE) = B00_T009B_A.T009B_BDATJ

	LEFT JOIN B00_T009B B00_T009B_B
	ON (A_T001.T001_PERIV = B00_T009B_B.T009B_PERIV) AND
	   MONTH(CDHDR_UDATE) = B00_T009B_B.T009B_BUMON AND
...
---       Total AP value
	...
	SELECT
		...
		,B11B_BSAIK_GJAHR
		,B11B_BSAIK_BELNR
		,SUM(B11B_ZF_BSAIK_DMBTR_CUC) B11B_ZF_BSAIK_DMBTR_CUC
		,SUM(B11B_ZF_BSAIK_DMBE2_S) B11B_ZF_BSAIK_DMBE2_S
		,SUM(B11B_ZF_BSAIK_DMBE3_S) B11B_ZF_BSAIK_DMBE3_S
		,SUM(B11B_ZF_BSAIK_DMBTR_S) B11B_ZF_BSAIK_DMBTR_S
		,SUM(CASE
			WHEN @custom_currency_field = 'DMBTR' THEN B11B_ZF_BSAIK_DMBTR_S_CUC
			WHEN @custom_currency_field = 'DMBE2' THEN B11B_ZF_BSAIK_DMBE2_S
			WHEN @custom_currency_field = 'DMBE3' THEN B11B_ZF_BSAIK_DMBE3_S
		END) B11B_ZF_DMBTR_S_CUC
	...
	EXEC SP_CREATE_INDEX 'B11B_BSAIK_MANDT,B11B_BSAIK_BUKRS,B11B_BSAIK_GJAHR,B11B_BSAIK_BELNR', 'FR06_02_TT_B11_TOT_PER_JE'
...
---       Total AR value
	...
	SELECT
		...
		,B14_BSAID_GJAHR
		,B14_BSAID_BELNR
		,SUM(B14_ZF_BSAID_DMBTR_CUC) B14_ZF_BSAID_DMBTR_CUC
		,SUM(B14_BSAID_DMBE2) B14_BSAID_DMBE2
		,SUM(B14_BSAID_DMBE3) B14_BSAID_DMBE3
		,SUM(B14_ZF_BSAID_DMBTR_COC) B14_ZF_BSAID_DMBTR_COC
		,SUM(CASE
			WHEN @custom_currency_field = 'DMBTR' THEN B14_ZF_BSAID_DMBTR_CUC
			WHEN @custom_currency_field = 'DMBE2' THEN B14_BSAID_DMBE2
			WHEN @custom_currency_field = 'DMBE3' THEN B14_BSAID_DMBE3
		END) B14_ZF_DMBTR_S_CUC
	...
	-- Create indexes to boost performance in the join
	EXEC SP_CREATE_INDEX 'B14_BSAID_MANDT,B14_BSAID_BUKRS,B14_BSAID_GJAHR,B14_BSAID_BELNR', 'FR06_03_TT_B14_TOT_PER_JE'
...
---       Total AP value
	...
	SELECT
		...
		,B04_BSEG_GJAHR
		,B04_BSEG_BELNR
		,MAX(B04_ZF_ENTRY_TYPE) B04_ZF_ENTRY_TYPE
		,SUM(B04_ZF_BSEG_DMBTR_S_CUC) B04_ZF_BSEG_DMBTR_S_CUC
		,SUM(B04_ZF_BSEG_DMBE2_S) B04_ZF_BSEG_DMBE2_S
		,SUM(B04_ZF_BSEG_DMBE3_S) B04_ZF_BSEG_DMBE3_S
		,SUM(B04_ZF_BSEG_DMBTR_S) B04_ZF_BSEG_DMBTR_S
		,SUM(CASE
			WHEN @custom_currency_field = 'DMBTR' THEN B04_ZF_BSEG_DMBTR_S_CUC
			WHEN @custom_currency_field = 'DMBE2' THEN B04_ZF_BSEG_DMBE2_S
			WHEN @custom_currency_field = 'DMBE3' THEN B04_ZF_BSEG_DMBE3_S
		END) B04_ZF_DMBTR_S_CUC
	...
	-- Create indexes to boost performance in the join
	EXEC SP_CREATE_INDEX 'B04_BKPF_MANDT,B04_BSEG_BUKRS,B04_BSEG_GJAHR,B04_BSEG_BELNR', 'FR06_04_TT_B04_TOT_PER_JE'
...
EXEC SP_DROPTABLE 'FR06_05_TT_B04_MJE_NUMS'
	...
	SELECT DISTINCT B04_BSEG_BUKRS, B04_BSEG_GJAHR, B04_BSEG_BELNR 
			INTO FR06_05_TT_B04_MJE_NUMS FROM B04_11_IT_FIN_GL WHERE B04_ZF_ENTRY_TYPE = 'manual' AND (B04_BSEG_KOART in ('K','D'))
...
--Step 6 Add the AP, AR, GL and Manual journal entry information to the list of changes 
	...
	SELECT FR06_01_TT_CDHDR_CDPOS.*
		...
		,@custom_currency_field CUSTOM_CURRENCY_FIELD
		,T001_WAERS
		-- Add a flag to indicate the type of journal entry
		,CASE
			WHEN B11B_BSAIK_BELNR IS NOT NULL THEN 'AP'
			WHEN B14_BSAID_BELNR IS NOT NULL THEN 'AR'
			WHEN B04_BSEG_BELNR IS NOT NULL THEN 'GL'
		END ZF_JOURNAL_TYPE
		,CASE
			WHEN B11B_BSAIK_BELNR IS NOT NULL THEN B11B_ZF_BSAIK_DMBTR_S
			WHEN B14_BSAID_BELNR IS NOT NULL THEN B14_ZF_BSAID_DMBTR_COC
			WHEN B04_BSEG_BELNR IS NOT NULL THEN B04_ZF_BSEG_DMBTR_S
		END ZF_DMBTR_COC
		,CASE
			WHEN B11B_BSAIK_BELNR IS NOT NULL THEN B11B_ZF_DMBTR_S_CUC
			WHEN B14_BSAID_BELNR IS NOT NULL THEN B14_ZF_DMBTR_S_CUC
			WHEN B04_BSEG_BELNR IS NOT NULL THEN B04_ZF_DMBTR_S_CUC
		END ZF_DMBTR_CUC
	...
	B11B_BSAIK_BUKRS = ZF_CDHDR_OBJID_COMP_CODE AND
	B11B_BSAIK_GJAHR = ZF_CDHDR_OBJID_FISCAL_YEAR AND
    B11B_BSAIK_BELNR = ZF_CDHDR_OBJID_DOC_NR  

	-- Get indicator of GL journal entry and value per journal entry
	LEFT JOIN FR06_04_TT_B04_TOT_PER_JE  ON
	B04_BSEG_BUKRS = ZF_CDHDR_OBJID_COMP_CODE AND
	B04_BSEG_GJAHR = ZF_CDHDR_OBJID_FISCAL_YEAR AND
    B04_BSEG_BELNR = ZF_CDHDR_OBJID_DOC_NR 

	-- Get indicator of AR journal entry and value per journal entry
    LEFT JOIN FR06_03_TT_B14_TOT_PER_JE ON 
    B14_BSAID_BUKRS = ZF_CDHDR_OBJID_COMP_CODE AND
	B14_BSAID_GJAHR = ZF_CDHDR_OBJID_FISCAL_YEAR AND
    B14_BSAID_BELNR = ZF_CDHDR_OBJID_DOC_NR

	DELETE FR06_04_TT_B04_TOT_PER_JE
	WHERE NOT EXISTS(SELECT * FROM FR06_06_TT_ADD_AP_AR_MJE_INFO
						WHERE 	B04_BSEG_BUKRS = ZF_CDHDR_OBJID_COMP_CODE AND
								B04_BSEG_GJAHR = ZF_CDHDR_OBJID_FISCAL_YEAR AND
								B04_BSEG_BELNR = ZF_CDHDR_OBJID_DOC_NR AND
								ZF_JOURNAL_TYPE = 'GL')
								
	DELETE FR06_02_TT_B11_TOT_PER_JE
	WHERE NOT EXISTS(SELECT * FROM FR06_06_TT_ADD_AP_AR_MJE_INFO
						WHERE 	B11B_BSAIK_BUKRS = ZF_CDHDR_OBJID_COMP_CODE AND
								B11B_BSAIK_GJAHR = ZF_CDHDR_OBJID_FISCAL_YEAR AND
								B11B_BSAIK_BELNR = ZF_CDHDR_OBJID_DOC_NR  AND
								ZF_JOURNAL_TYPE = 'AP')
								
	DELETE FR06_03_TT_B14_TOT_PER_JE
	WHERE NOT EXISTS(SELECT * FROM FR06_06_TT_ADD_AP_AR_MJE_INFO
						WHERE 	B14_BSAID_BUKRS = ZF_CDHDR_OBJID_COMP_CODE AND
								B14_BSAID_GJAHR = ZF_CDHDR_OBJID_FISCAL_YEAR AND
								B14_BSAID_BELNR = ZF_CDHDR_OBJID_DOC_NR AND
								ZF_JOURNAL_TYPE = 'AR')
--step 7: Add:
...
--          (Sometimes more than one suppleir per journal entry number)
	...
	WHERE EXISTS(SELECT * FROM FR06_06_TT_ADD_AP_AR_MJE_INFO
					...
					AND ZF_CDHDR_OBJID_COMP_CODE + ZF_CDHDR_OBJID_FISCAL_YEAR + ZF_CDHDR_OBJID_DOC_NR = B11B_BSAIK_BUKRS + B11B_BSAIK_GJAHR + B11B_BSAIK_BELNR)
...
--          (Sometimes more than one customer per journal entry number)
	...
	WHERE EXISTS(SELECT * FROM FR06_06_TT_ADD_AP_AR_MJE_INFO
					...
					AND ZF_CDHDR_OBJID_COMP_CODE + ZF_CDHDR_OBJID_FISCAL_YEAR + ZF_CDHDR_OBJID_DOC_NR = B14_BSAID_BUKRS + B14_BSAID_GJAHR + B14_BSAID_BELNR)
...
--          (Sometimes more than one customer per journal entry number)
	...
	WHERE EXISTS(SELECT * FROM FR06_06_TT_ADD_AP_AR_MJE_INFO
					...
					AND ZF_CDHDR_OBJID_COMP_CODE + ZF_CDHDR_OBJID_FISCAL_YEAR + ZF_CDHDR_OBJID_DOC_NR = B04_BSEG_BUKRS + B04_BSEG_GJAHR + B04_BSEG_BELNR)
