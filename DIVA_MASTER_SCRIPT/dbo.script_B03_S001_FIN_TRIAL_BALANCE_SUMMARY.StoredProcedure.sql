...
--Step0 Add some field in AM 20F account mapping. This is request from Jesper ( 27-11-2020)
	/* Create AM_TANGO_20F_MAPPING table.  */

	EXEC SP_REMOVE_TABLES 	'AM_TANGO_20F_MAPPING' 
	SELECT A.*
		,
				CASE 
	...
	FROM AM_TANGO AS A 
	LEFT JOIN  DIVA_MASTER_SCRIPT..AM_20F_ACCOUNT AS B
...
SELECT 
	...
	B03_TB_RYEAR, 
	B03_TB_MONAT,
	SUM(B03_TB_HSL_TOT_MOV) AS B03_TB_HSL_TOT_MOV,
	SUM(B03_TB_HSL_TOT_MOV_CUC) AS B03_TB_HSL_TOT_MOV_CUC,
	SUM(B03_TB_KSL_TOT_MOV) AS B03_TB_KSL_TOT_MOV
INTO B03_02_TT_TB_SUMMARY_TEMP
FROM B03_03_IT_FIN_TB
GROUP BY 	B03_T001_BUKRS, 
			B03_TB_RACCT,   
			B03_TB_RYEAR, 
			B03_TB_MONAT
	...
	(
		SELECT 
			DISTINCT 
				...
				B03_TB_RYEAR, 
				B.B03_TB_MONAT
			FROM B03_02_TT_TB_SUMMARY_TEMP A , 
			( 
				SELECT DISTINCT B03_TB_MONAT FROM B03_02_TT_TB_SUMMARY_TEMP
			) AS B -- Get all monat value in ACDOCA table
...
(
	SELECT 
		TOP 1 1 
	FROM B03_02_TT_TB_SUMMARY_TEMP
	WHERE  A.B03_TB_RYEAR+A.B03_T001_BUKRS+A.B03_TB_RACCT+A.B03_TB_MONAT =
		   B03_TB_RYEAR+B03_T001_BUKRS+B03_TB_RACCT+B03_TB_MONAT
)
...
SELECT 
	...
	B03_TB_MONAT, 
	CASE
		WHEN B03_TB_MONAT  = '00'                     THEN 'Opening'
		WHEN B03_TB_MONAT  = '01'                     THEN 'Apr'
		WHEN B03_TB_MONAT  = '02'                     THEN 'May'
		WHEN B03_TB_MONAT  = '03'                     THEN 'Jun'
		WHEN B03_TB_MONAT  = '04'                     THEN 'Jul'
		WHEN B03_TB_MONAT  = '05'                     THEN 'Aug'
		WHEN B03_TB_MONAT  = '06'                     THEN 'Sep'
		WHEN B03_TB_MONAT  = '07'                     THEN 'Oct'
		WHEN B03_TB_MONAT  = '08'                     THEN 'Nov'
		WHEN B03_TB_MONAT  = '09'                     THEN 'Dec'
		WHEN B03_TB_MONAT  = '10'                     THEN 'Jan'
		WHEN B03_TB_MONAT  = '11'                     THEN 'Feb'
		WHEN B03_TB_MONAT  = '12'                     THEN 'Mar'
		WHEN B03_TB_MONAT IN ('13','14','15','16') THEN 'Adj'
		ELSE                                                             '90_Unknown'
	END AS ZF_TB_POPER_MONTH,
	CASE	
				WHEN CAST(B03_TB_MONAT AS INT) = 0 THEN 'Open'
				WHEN CAST(B03_TB_MONAT AS INT) IN (1, 2, 3) THEN 'Q1'
				WHEN CAST(B03_TB_MONAT AS INT) IN (4, 5, 6) THEN 'Q2'
				WHEN CAST(B03_TB_MONAT AS INT) IN (7, 8, 9) THEN 'Q3'
				WHEN CAST(B03_TB_MONAT AS INT) IN (10, 11, 12) THEN 'Q4'
				ELSE 'Special Quater' 
	END AS ZF_TB_MONAT_FQ,
	B03_TB_RACCT+' - '+B02_SKAT_TXT50 as ZF_TB_RACCT_TEXT,
	B02_TANGO_ACCT AS TANGO_ACCT,
	B02_TANGO_ACCT_TXT AS TANGO_ACCT_TXT,
	TANGO_ACCT_MAP_TXT,
	ZF_TANGO_20F_GROUP,
	...
	T001_BUTXT,
	B03_TB_HSL_TOT_MOV,
	B03_TB_HSL_TOT_MOV_CUC,
	B03_TB_KSL_TOT_MOV,
	SKA1_XBILK,SKA1_GVTYP,
	SUM(B03_TB_HSL_TOT_MOV) OVER (PARTITION BY B03_TB_RACCT, B03_TB_RYEAR, B03_T001_BUKRS 
	ORDER BY B03_TB_RACCT, B03_TB_RYEAR, B03_T001_BUKRS , B03_TB_MONAT) AS B03_ZF_TB_HSL_TOT_END,
	SUM(B03_TB_HSL_TOT_MOV_CUC) OVER (PARTITION BY B03_TB_RACCT, B03_TB_RYEAR, B03_T001_BUKRS 
	ORDER BY B03_TB_RACCT, B03_TB_RYEAR, B03_T001_BUKRS , B03_TB_MONAT) AS B03_ZF_TB_HSL_TOT_END_CUC,
	SUM(B03_TB_KSL_TOT_MOV) OVER (PARTITION BY B03_TB_RACCT, B03_TB_RYEAR, B03_T001_BUKRS 
	ORDER BY B03_TB_RACCT, B03_TB_RYEAR, B03_T001_BUKRS , B03_TB_MONAT) AS B03_ZF_TB_KSL_TOT_END
INTO B03_04_IT_TB_SUMMARY_FULL
FROM B03_03_TT_TB_SUMMARY_FULL_MONAT
LEFT JOIN  B02_04_IT_FIN_COA -- Get GL test
ON  B02_04_IT_FIN_COA.B02_SKB1_SAKNR = B03_TB_RACCT AND
	B02_04_IT_FIN_COA.B02_T001_BUKRS = B03_T001_BUKRS
LEFT JOIN AM_TANGO_20F_MAPPING ON TANGO_GL_ACCT = B03_TB_RACCT
INNER JOIN AM_SCOPE ON SCOPE_CMPNY_CODE = B03_T001_BUKRS
INNER JOIN A_T001 ON B03_T001_BUKRS = T001_BUKRS
LEFT JOIN A_SKA1 ON T001_KTOPL = A_SKA1.SKA1_KTOPL
AND B03_TB_RACCT = A_SKA1.SKA1_SAKNR
