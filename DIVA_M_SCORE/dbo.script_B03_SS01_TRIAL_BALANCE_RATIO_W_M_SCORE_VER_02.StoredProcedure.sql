...
GO
CREATE   PROCEDURE [dbo].[script_B03_SS01_TRIAL_BALANCE_RATIO_W_M_SCORE_VER_02]
AS
...
-- Net income = Revenue + Other revenue + Discounts/reductions/adjustments + COGS + Expenditure + SGA expenses (because SGA expenses used to be a subset of expenditure)
UPDATE  B03_SS01_03_RT_CACULATED_NET_MOV_BALANCE_MAPPING_YEAR_MONTH_PIVOT
SET 
	ZF_NET_INCOME_NET_HSL		= (ZF_REVENUE_GROSS_SALE_MOV_HSL)+ ZF_OTHER_REVENUE_MOV_HSL + ZF_COGS_MOV_HSL + ZF_EXPENDITURE_MOV_HSL + ZF_SGA_EXPS_MOV_HSL,
	ZF_NET_INCOME_NET_HSL_CUC	= ZF_REVENUE_GROSS_SALE_MOV_CUC  + ZF_OTHER_REVENUE_MOV_CUC + ZF_COGS_MOV_CUC + ZF_EXPENDITURE_MOV_CUC + ZF_SGA_EXPS_MOV_CUC,
	ZF_NET_INCOME_NET_OSL		= (ZF_REVENUE_GROSS_SALE_MOV_OSL)+ ZF_OTHER_REVENUE_MOV_OSL + ZF_COGS_MOV_OSL + ZF_EXPENDITURE_MOV_OSL + ZF_SGA_EXPS_MOV_OSL,
	ZF_NET_INCOME_NET_KSL		= (ZF_REVENUE_GROSS_SALE_MOV_KSL)+ ZF_OTHER_REVENUE_MOV_KSL + ZF_COGS_MOV_KSL + ZF_EXPENDITURE_MOV_KSL + ZF_SGA_EXPS_MOV_KSL,
	ZF_NET_INCOME_BAL_HSL		= (ZF_REVENUE_GROSS_SALE_BAL_HSL)+ ZF_OTHER_REVENUE_BAL_HSL + ZF_COGS_BAL_HSL + ZF_EXPENDITURE_BAL_HSL + ZF_SGA_EXPS_BAL_HSL,
	ZF_NET_INCOME_BAL_HSL_CUC	= (ZF_REVENUE_GROSS_SALE_BAL_CUC)+ ZF_OTHER_REVENUE_BAL_CUC + ZF_COGS_BAL_CUC + ZF_EXPENDITURE_BAL_CUC + ZF_SGA_EXPS_BAL_CUC,
	ZF_NET_INCOME_BAL_OSL		= (ZF_REVENUE_GROSS_SALE_BAL_OSL)+ ZF_OTHER_REVENUE_BAL_OSL + ZF_COGS_BAL_OSL + ZF_EXPENDITURE_BAL_OSL + ZF_SGA_EXPS_BAL_OSL,
	ZF_NET_INCOME_BAL_KSL		= (ZF_REVENUE_GROSS_SALE_BAL_KSL)+ ZF_OTHER_REVENUE_BAL_KSL + ZF_COGS_BAL_KSL + ZF_EXPENDITURE_BAL_KSL + ZF_SGA_EXPS_BAL_KSL
...
--Step 8 Remove temporary tables
	...
	SELECT
	  ...
	  ZF_REVENUE_GROSS_SALE_MOV_HSL,
	  ISNULL(LAG(ZF_REVENUE_GROSS_SALE_MOV_HSL) OVER (PARTITION BY B03_04_ACDOCA_RYEAR, B03_04_ACDOCA_RBUKRS ORDER BY B03_04_ACDOCA_RYEAR, B03_04_ACDOCA_MONAT, B03_04_ACDOCA_RBUKRS, B03_04_ZF_ACDOCA_POPER_MONTH),0) AS ZF_REVENUE_GROSS_SALE_BAL_HSL_PREV,
	  ...
	  ZF_NET_INCOME_NET_HSL,
	  LAG(ZF_NET_INCOME_NET_HSL) OVER (PARTITION BY B03_04_ACDOCA_RYEAR, B03_04_ACDOCA_RBUKRS ORDER BY B03_04_ACDOCA_RYEAR, B03_04_ACDOCA_MONAT, B03_04_ACDOCA_RBUKRS, B03_04_ZF_ACDOCA_POPER_MONTH) AS ZF_NET_INCOME_NET_HSL_PREV,
	  ZF_NET_INCOME_NET_HSL_CUC,
	  LAG(ZF_NET_INCOME_NET_HSL_CUC) OVER (PARTITION BY B03_04_ACDOCA_RYEAR, B03_04_ACDOCA_RBUKRS ORDER BY B03_04_ACDOCA_RYEAR, B03_04_ACDOCA_MONAT, B03_04_ACDOCA_RBUKRS, B03_04_ZF_ACDOCA_POPER_MONTH) AS ZF_NET_INCOME_NET_HSL_CUC_PREV,
