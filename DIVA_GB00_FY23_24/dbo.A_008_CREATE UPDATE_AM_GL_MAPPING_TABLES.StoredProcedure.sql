...
-- =============================================
CREATE    PROCEDURE [dbo].[A_008_CREATE/UPDATE_AM_GL_MAPPING_TABLES]
AS
BEGIN
	/*
	...
	*/
	...
	EXEC SP_REMOVE_TABLES 'A08_07_TT_IRGR_MATERIAL_GROUP'
	SELECT DISTINCT A_MARA.MARA_MATKL, A_T023T.T023T_WGBEZ
	-- INTO A08_07_TT_IRGR_MATERIAL_GROUP
	FROM A_BSEG
	--Only keep records relate to AP documents
	INNER JOIN (
		SELECT DISTINCT BSEG_BUKRS, BSEG_GJAHR, BSEG_BELNR
		FROM A_BSEG
		WHERE BSEG_KOART = 'K'
	) A_BSEG_AP
	ON A_BSEG_AP.BSEG_BUKRS = A_BSEG.BSEG_BUKRS AND
		A_BSEG_AP.BSEG_GJAHR = A_BSEG.BSEG_GJAHR AND
		A_BSEG_AP.BSEG_BELNR = A_BSEG.BSEG_BELNR

	--Only keep records relate to IRGR accounts
	...
	--Get material groups
	LEFT JOIN A_MARA
	ON A_MARA.MARA_MATNR = A_BSEG.BSEG_MATNR

	--Get material group descriptions
	LEFT JOIN A_T023T
	ON A_T023T.T023T_MATKL = A_MARA.MARA_MATKL
	WHERE A_BSEG.BSEG_MATNR <> '' AND A_MARA.MARA_MATKL IS NOT NULL
	AND BSEG_HKONT = 'A212100'
	...
	-- MATERIAL GROUP  START A,B, ETC not number
	...
	FROM A_BSEG
		INNER JOIN A_BKPF 
			ON BKPF_BELNR = BSEG_BELNR
			AND BKPF_BUKRS = BSEG_BUKRS
			AND BKPF_GJAHR=  BSEG_GJAHR

	WHERE BSEG_HKONT = 'A212100'
	AND BKPF_BLART = 'RE'
	...
	ON A.BSEG_EBELN = B.EKKO_EBELN
	AND A.BSEG_EBELP = B.EKPO_EBELP

-- 4200607142

SELECT MIN(EKKO_AEDAT)
FROM A_EKKO
WHERE EKKO_EBELN = '4200607142'
...
WHERE BSEG_EBELN <> ''
	...
	*/
	...
	SELECT DISTINCT MARA_MATKL, T023T_WGBEZ, T023T_WGBEZ
	FROM A08_07_TT_IRGR_MATERIAL_GROUP
	WHERE NOT EXISTS (SELECT TOP 1 1 FROM AM_IRGR_MATERIAL_GROUP_HIERARCHY WHERE A08_07_TT_IRGR_MATERIAL_GROUP.MARA_MATKL = AM_IRGR_MATERIAL_GROUP_HIERARCHY.T023T_MATKL)
