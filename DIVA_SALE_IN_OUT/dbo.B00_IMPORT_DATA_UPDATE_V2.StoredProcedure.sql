...
CREATE         PROCEDURE [dbo].[B00_IMPORT_DATA_UPDATE_V2]
...
FROM 
	...
	FROM (
		...
		FROM 
			(
				SELECT 
					SPWOC, MATNR, ZZSV_CMPTR, SUM(FKIMG) AS FKIMG
		...
		FULL  JOIN 
			(
				SELECT 
					SPWOC, MATNR, ZZSV_CMPTR, SUM(ZZSV_QTYTC) AS ZZSV_QTYTC, SUM(ZZSV_CHINV) AS ZZSV_CHINV
				FROM B00_02_IT_SALE_OUT
				GROUP BY SPWOC, MATNR, ZZSV_CMPTR

			)	
		
		B ON A.SPWOC = B.SPWOC AND A.MATNR = B.MATNR AND A.ZZSV_CMPTR = B.ZZSV_CMPTR
...
------------------------------------------------------ TASK from Jesper  Compare ROH / Sale in with Sale out ---------------------------------------

-- Step 5 /  Store table Sale in, Sale out and Report on hand stock
DROP TABLE TEMP1
...
SELECT 
	TABLE1.*, TABLE2.[Reported on hand stock],TABLE3.[Sale in]
INTO TEMP1
FROM TABLE1 INNER JOIN TABLE2
	...
	INNER JOIN TABLE3
		ON TABLE1.MATNR = TABLE3.MATNR
		AND TABLE1.SPWOC = TABLE3.SPWOC
		AND TABLE1.ZZSV_CMPTR = TABLE3.ZZSV_CMPTR

-- Step 6 / Calculated ROH/ sale in / avg sale out.

DROP  TABLE  B00_07_IT_ROH_SALE_IN_SALES_OUT
...
INTO B00_07_IT_ROH_SALE_IN_SALES_OUT
FROM (
SELECT 
    MATNR,
	ZZSV_CMPTR,
...
-- Step 6 / Update add 3 more field for B06 table.

ALTER TABLE B00_06_IT_SALE_IN_OUT_APPEND ADD NAME1 NVARCHAR(1000);
ALTER TABLE B00_06_IT_SALE_IN_OUT_APPEND ADD MATKL NVARCHAR(1000);
ALTER TABLE B00_06_IT_SALE_IN_OUT_APPEND ADD WGBEZ NVARCHAR(1000);
...
EXEC SP_UNNAME_FIELD 'B00_02' , 'B00_07_IT_ROH_SALE_IN_SALES_OUT'
...
EXEC sp_RENAME_FIELD 'B00_07_', 'B00_07_IT_ROH_SALE_IN_SALES_OUT' 


SELECT 
	RIGHT(SPWOC,4), LEFT(SPWOC,2),
	FKIMG,
	*
FROM [21 22 sell in data]
WHERE MATNR = 'KD65X85J'
AND ZZSV_CMPTR = '1004386'
ORDER BY RIGHT(SPWOC,4), LEFT(SPWOC,2)



SELECT 
	RIGHT(SPWOC,4), LEFT(SPWOC,2),
	ZZSV_QTYTC,	ZZSV_CHINV,
