...
--Get only the lines have delivery
		...
		AND B28_VBAK_VBELN+B28_VBAP_POSNR IN
		(
	
			SELECT DISTINCT B29_LIPS_VGBEL+B29_LIPS_VGPOS FROM B29_01_IT_DELIVERY_DOCS
		)
...
-- Step 4: Create cube for SD document: Delivery: Item data, get only the line relate to sale orders
		...
		ON B29_LIPS_MATNR = MARA_MATNR
		-- Get delivery item category description
		LEFT JOIN A_TVPT
		ON B29_LIPS_PSTYV = TVPT_PSTYV
		-- Get name text of person who created the object
		LEFT JOIN A_V_USERNAME
		ON B29_LIKP_ERNAM = V_USERNAME_BNAME
		WHERE B29_LIPS_VGBEL+B29_LIPS_VGPOS IN
		(
...
--Step 5.1 Summarize LIKP-LIPS base on LIPS_VGBEL,LIPS_VGPOS
		...
		SELECT
			B29_LIPS_VGBEL,B29_LIPS_VGPOS,
			SUM(B29_LIPS_LFIMG) AS ZF_TOTAL_LIPS_LFIMG
...
--Get only the case where actual quantity higher than target quantity
		...
		ON B29_LIPS_VGBEL=B28_VBAK_VBELN AND
		   B29_LIPS_VGPOS=B28_VBAP_POSNR
		WHERE ZF_TOTAL_LIPS_LFIMG>B28_VBAP_ZMENG
...
--Step 5.3 Get the delivery where actual quantity higher than target quantity
		...
		WHERE B29_LIPS_VGBEL+B29_LIPS_VGPOS IN
		(
			SELECT DISTINCT B29_LIPS_VGBEL+B29_LIPS_VGPOS FROM SU19_06_TT_LIKP_LIPS_COMPARE_VBAK_VBAP
		)
...
--Step 5.4 Get the sale document where actual quantity higher than target quantity
		...
		WHERE B28_VBAP_POSNR+B28_VBAK_VBELN IN 
		(
			SELECT DISTINCT B28_VBAP_POSNR+B28_VBAK_VBELN FROM SU19_06_TT_LIKP_LIPS_COMPARE_VBAK_VBAP
...
-- Unname the fields
EXEC SP_UNNAME_FIELD 'B28_', 'SU19_08_XT_VBAP_SALE_ORDER_LFIMG_HIGHER_ZMENG'
EXEC SP_UNNAME_FIELD 'B29_', 'SU19_07_XT_LIKP_DELIVERY_DOCS_LFIMG_HIGHER_ZMENG'
EXEC SP_UNNAME_FIELD 'B29_', 'SU19_08_XT_VBAP_SALE_ORDER_LFIMG_HIGHER_ZMENG'
-- Rename the fiedls
