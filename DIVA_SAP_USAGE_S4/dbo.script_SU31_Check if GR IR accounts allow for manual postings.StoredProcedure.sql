...
INNER JOIN (
		SELECT 
			DISTINCT B04_GL_ACDOCA_BELNR, B04_GL_ACDOCA_GJAHR, B04_GL_ACDOCA_RBUKRS
		FROM B04_08_IT_FIN_GL A
		WHERE EXISTS 
		(
			SELECT TOP 1 1 
			FROM SU31_01_XT_T030_KTOSL_EQ_WRX
			WHERE A.B04_GL_ACDOCA_RACCT = SU31_01_XT_T030_KTOSL_EQ_WRX.T030_KONTS
		)
) AS A ON 		 A.B04_GL_ACDOCA_BELNR =  B.B04_GL_ACDOCA_BELNR
		AND A.B04_GL_ACDOCA_GJAHR = B.B04_GL_ACDOCA_GJAHR
		AND A.B04_GL_ACDOCA_RBUKRS = B.B04_GL_ACDOCA_RBUKRS

LEFT JOIN A_T001 
ON B.B04_GL_ACDOCA_RBUKRS=T001_BUKRS
LEFT JOIN A_V_USERNAME
ON B.B04_GL_ACDOCA_USNAM=V_USERNAME_BNAME
LEFT JOIN A_SKB1 
ON A_SKB1.SKB1_BUKRS = B.B04_GL_ACDOCA_RBUKRS 
	AND A_SKB1.SKB1_SAKNR = B.B04_GL_ACDOCA_RACCT 
LEFT JOIN A_T003T
...
EXISTS
	(
		SELECT TOP 1 1 
		FROM SU31_01_XT_T030_KTOSL_EQ_WRX
		WHERE B04_GL_ACDOCA_RACCT = SU31_01_XT_T030_KTOSL_EQ_WRX.T030_KONTS
	)
...
CONCAT('Debit: ', B04_09_ZF_DEBIT_ACCOUNTS, 'Credit: ', B04_09_ZF_CREDIT_ACCOUNTS) AS ZF_DB_CR_ACDOCA_RACCT_LIST_COMBINE,
...
EXEC SP_UNNAME_FIELD 'B04_GL_','SU31_02_IT_JEs_RELATED_GRIR'
EXEC SP_RENAME_FIELD 'SU31_02_','SU31_02_IT_JEs_RELATED_GRIR'

EXEC SP_UNNAME_FIELD 'B04_GL_','SU31_03_IT_GRIR_MAN_POSTING_ACC_SCHEMES'
EXEC SP_UNNAME_FIELD 'B04_09_','SU31_03_IT_GRIR_MAN_POSTING_ACC_SCHEMES'
