...
SELECT DISTINCT 
	 ...
	 CASE B29_TVLK_AUFER 
		  ...
		  IIF(LEN(B29_TVLK_AUFER) = 0,'No','Yes') AS ZF_TVLK_AUFER_BLANK_OR_NOT
...
SELECT DISTINCT *
	...
	FROM B28_01_IT_SALE_DOCUMENTS A
	JOIN SU29A_01_XT_LIPS_VGBEL_NOT_IN_VBFA_VBELV B
	ON B.B29_LIPS_VGBEL = A.B28_VBAK_VBELN AND B.B29_LIPS_VGPOS = A.B28_VBAP_POSNR
WHERE LEN(B.B29_TVLK_AUFER) = 0 
...
SELECT C.*,
	...
	FROM B30_01_IT_INVOICE_DOCS C
	JOIN SU29A_01_XT_LIPS_VGBEL_NOT_IN_VBFA_VBELV D
	ON D.B29_LIKP_VBELN = C.B30_VBRP_VGBEL AND D.B29_LIPS_POSNR = C.B30_VBRP_VGPOS
WHERE LEN(D.B29_TVLK_AUFER) = 0
...
SELECT DISTINCT A_TVLK.TVLK_LFART,
		CASE 
			WHEN LEN(SU29A_01_XT_LIPS_VGBEL_NOT_IN_VBFA_VBELV.B29_LIKP_LFART) > 0 THEN 'Yes' 
			ELSE 'No' 
...
ON A_TVLK.TVLK_LFART = SU29A_01_XT_LIPS_VGBEL_NOT_IN_VBFA_VBELV.B29_LIKP_LFART	

EXEC SP_UNNAME_FIELD 'B29_','SU29A_01_XT_LIPS_VGBEL_NOT_IN_VBFA_VBELV';
EXEC SP_RENAME_FIELD 'SU29A_01_','SU29A_01_XT_LIPS_VGBEL_NOT_IN_VBFA_VBELV';

EXEC SP_UNNAME_FIELD 'B28_','SU29A_02_XT_VBAP_VBELN_POSNR';
EXEC SP_UNNAME_FIELD 'B29_','SU29A_02_XT_VBAP_VBELN_POSNR';
EXEC SP_RENAME_FIELD 'SU29A_02_','SU29A_02_XT_VBAP_VBELN_POSNR';

EXEC SP_UNNAME_FIELD 'B29_','SU29A_03_XT_VBRP_VGBEL_VGPOS';
EXEC SP_UNNAME_FIELD 'B30_','SU29A_03_XT_VBRP_VGBEL_VGPOS';
