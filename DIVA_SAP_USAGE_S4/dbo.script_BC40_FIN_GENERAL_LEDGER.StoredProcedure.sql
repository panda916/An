...
GO
CREATE PROCEDURE [dbo].[script_BC40_FIN_GENERAL_LEDGER]
AS
     

--DYNAMIC_SCRIPT_START

EXEC SP_REMOVE_TABLES 'BC40%'
...
EXEC sp_droptable    'BC40_01_TT_CSKT'
...
EXEC SP_DROPTABLE   'BC40_02_TT_CSKT_DATBI' 
SELECT * INTO BC40_02_TT_CSKT_DATBI FROM BC40_01_TT_CSKT WHERE #ROW = 1
EXEC SP_CREATE_INDEX 'CSKT_MANDT, CSKT_KOSTL', 'BC40_02_TT_CSKT_DATBI'

/*--Step 3
--Create a list of latest information per profit center
*/
	EXEC SP_DROPTABLE 'BC40_03_TT_CEPCT'
	SELECT 
		A_CEPCT.*
		,ROW_NUMBER() OVER( PARTITION BY CEPCT_MANDT, CEPCT_PRCTR, CEPCT_KOKRS ORDER BY CEPCT_DATBI DESC) AS ROW_NR
	INTO BC40_03_TT_CEPCT
	FROM A_CEPCT
	WHERE CEPCT_SPRAS = @language1
	AND CEPCT_DATBI> = @downloaddate

	EXEC SP_DROPTABLE 'BC40_04_TT_CEPCT_UNIQUE'
	SELECT * INTO BC40_04_TT_CEPCT_UNIQUE FROM BC40_03_TT_CEPCT WHERE ROW_NR = 1 
...
EXEC sp_droptable    'BC40_05_TT_BKPF_BSEG'
		...
		SELECT
			...
			A_SKB1.SKB1_HBKID,
			B00_SKAT.SKAT_TXT20,               
			BC40_02_TT_CSKT_DATBI.CSKT_KTEXT, 
			-- Add cost center description       
			CASE
			WHEN ISNULL(BC40_02_TT_CSKT_DATBI.CSKT_MCTXT,'') = '' THEN 'Not assigned'
			ELSE BC40_02_TT_CSKT_DATBI.CSKT_MCTXT
			END AS ZF_CSKT_MCTXT,	
			BC40_04_TT_CEPCT_UNIQUE.CEPCT_MCTXT,
			A_T001.T001_KTOPL,
			A_T001.T001_BUTXT
		INTO BC40_05_TT_BKPF_BSEG
       ...
       -- $$ move CSKT join up from BC40_04
       LEFT JOIN BC40_02_TT_CSKT_DATBI                   
       ON A_BSEG.BSEG_KOKRS = BC40_02_TT_CSKT_DATBI.CSKT_KOKRS AND 
          A_BSEG.BSEG_KOSTL = BC40_02_TT_CSKT_DATBI.CSKT_KOSTL      

		-- Include profit centers descriptions
		LEFT JOIN BC40_04_TT_CEPCT_UNIQUE
		ON	A_BSEG.BSEG_BUKRS = BC40_04_TT_CEPCT_UNIQUE.CEPCT_KOKRS AND
			A_BSEG.BSEG_KOKRS = BC40_04_TT_CEPCT_UNIQUE.CEPCT_KOKRS AND
			A_BSEG.BSEG_PRCTR = BC40_04_TT_CEPCT_UNIQUE.CEPCT_PRCTR 
	   ...
	   EXEC SP_CREATE_INDEX 'BSEG_BUKRS,BSEG_GJAHR,BSEG_BELNR,BSEG_SHKZG,SKA1_XBILK,SKA1_GVTYP,BSEG_KOART,BSEG_KOSTL,CSKT_KTEXT,CEPCT_MCTXT', 'BC40_05_TT_BKPF_BSEG'
...
*/
	...
	ALTER TABLE BC40_05_TT_BKPF_BSEG ADD ZF_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG NVARCHAR(1) DEFAULT '' WITH VALUES;
	
	UPDATE BC40_05_TT_BKPF_BSEG
	...
	EXISTS(
		SELECT TOP 1 1
		FROM A_T012K
		WHERE BC40_05_TT_BKPF_BSEG.BSEG_BUKRS = A_T012K.T012K_BUKRS AND
			  BC40_05_TT_BKPF_BSEG.BSEG_HKONT = A_T012K.T012K_HKONT
	)
	...
	OR	EXISTS(
				SELECT TOP 1 1
				FROM A_FEBKO
				WHERE BC40_05_TT_BKPF_BSEG.BSEG_BUKRS = A_FEBKO.FEBKO_BUKRS AND
					  BC40_05_TT_BKPF_BSEG.BSEG_HKONT = A_FEBKO.FEBKO_HKONT
	)
	...
	OR EXISTS (
		SELECT TOP 1 1
		FROM A_SKB1
		WHERE BC40_05_TT_BKPF_BSEG.BSEG_BUKRS = A_SKB1.SKB1_BUKRS AND
			  BC40_05_TT_BKPF_BSEG.BSEG_HKONT = A_SKB1.SKB1_SAKNR AND
	...
	*/
	...
	EXEC sp_droptable    'BC40_06_TT_FEBKO_EP'
...
EXEC SP_DROPTABLE    'BC40_07_TT_INDIC_PER_JE_NUM'
   
      SELECT
           BC40_05_TT_BKPF_BSEG.BKPF_MANDT
          ,BC40_05_TT_BKPF_BSEG.BSEG_BUKRS
          ,BC40_05_TT_BKPF_BSEG.BSEG_GJAHR
          ,BC40_05_TT_BKPF_BSEG.BSEG_BELNR
          ,BC40_05_TT_BKPF_BSEG.BKPF_BKTXT
          ,CASE
             WHEN COUNT(DISTINCT(BC40_05_TT_BKPF_BSEG.BSEG_HKONT)) = 1 THEN 'X'
             ELSE ''
           END AS ZF_BSEG_HKONT_1_ACCNT_THIS_JE
          ,CASE
             WHEN COUNT(NULLIF('', BC40_05_TT_BKPF_BSEG.BSEG_XAUTO)) = Count(BC40_05_TT_BKPF_BSEG.BKPF_MANDT) THEN 'X'
             ELSE ''
           END AS ZF_BSEG_XAUTO_ALL_LINES_AUTO
          ,CASE
             WHEN (BC40_05_TT_BKPF_BSEG.BKPF_BKTXT LIKE '%auto%' AND BC40_05_TT_BKPF_BSEG.BKPF_BKTXT LIKE '%clear%')
                               THEN 'X'
             ELSE ''
           END AS ZF_BKPF_BKTXT_AUTO_CLEAR
      
         INTO BC40_07_TT_INDIC_PER_JE_NUM
     
         FROM BC40_05_TT_BKPF_BSEG
...
EXEC sp_droptable  'BC40_11_IT_FIN_GL'
 
            SELECT
					BC40_05_TT_BKPF_BSEG.BSEG_MWSKZ,
                    BC40_05_TT_BKPF_BSEG.BKPF_MANDT,
                    BC40_05_TT_BKPF_BSEG.BSEG_BUKRS,
					BC40_05_TT_BKPF_BSEG.BSEG_GJAHR,
                    BC40_05_TT_BKPF_BSEG.BSEG_BELNR,
                    BC40_05_TT_BKPF_BSEG.BSEG_BUZEI,
                    --BC40_05_TT_BKPF_BSEG.T001_BUTXT,
                    -- Add fiscal year + posting period
                    BC40_05_TT_BKPF_BSEG.BSEG_GJAHR + '-' + BC40_05_TT_BKPF_BSEG.BKPF_MONAT AS ZF_BKPF_GJAHR_MONAT,
					BC40_05_TT_BKPF_BSEG.BKPF_MONAT,
                    CAST(YEAR(BC40_05_TT_BKPF_BSEG.BKPF_BUDAT) AS VARCHAR(4)) + '-' +
                        RIGHT('0' + CAST(MONTH(BC40_05_TT_BKPF_BSEG.BKPF_BUDAT) AS VARCHAR(2)),2) AS ZF_BKPF_BUDAT_YEAR_MNTH,
                    --BC40_05_TT_BKPF_BSEG.T001_KTOPL,
                    BC40_05_TT_BKPF_BSEG.BSEG_KOART,
                    -- Add acount type description
                    CASE
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'A' THEN 'Assets'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'D' THEN 'Customers'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'K' THEN 'Vendors'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'M' THEN 'Material'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'S' THEN 'G/L accounts'
                        ELSE ''
                    END                                                                  AS ZF_BKPF_KOART_DESC,
                    BC40_05_TT_BKPF_BSEG.BSEG_HKONT SPCAT_GL_ACCNT,
                    BC40_05_TT_BKPF_BSEG.BSEG_BSCHL,
                    BC40_05_TT_BKPF_BSEG.BKPF_BLART,
                    BC40_05_TT_BKPF_BSEG.BKPF_BKTXT,
                    BC40_05_TT_BKPF_BSEG.BSEG_SGTXT,
                    BC40_05_TT_BKPF_BSEG.BKPF_BLDAT,
                    BC40_05_TT_BKPF_BSEG.BKPF_CPUDT,      
                    BC40_05_TT_BKPF_BSEG.BKPF_CPUTM,
                    BC40_05_TT_BKPF_BSEG.BKPF_AEDAT,
                    BC40_05_TT_BKPF_BSEG.BKPF_UPDDT,
                    BC40_05_TT_BKPF_BSEG.BKPF_USNAM,
                    BC40_05_TT_BKPF_BSEG.BKPF_TCODE,
                    BC40_05_TT_BKPF_BSEG.BKPF_BUDAT,
                    ...
                    -- Add flag to show if it is posted in the period
					,CASE
                        WHEN BC40_05_TT_BKPF_BSEG.BKPF_BUDAT >= @date1 AND BC40_05_TT_BKPF_BSEG.BKPF_BUDAT <= @date2 THEN 'X'
                        ELSE ''
                    END AS ZF_BKPF_BUDAT_IN_PERIOD,
                    BC40_05_TT_BKPF_BSEG.BSEG_AUGDT,
                    BC40_05_TT_BKPF_BSEG.BSEG_AUGBL,
                    BC40_05_TT_BKPF_BSEG.BSEG_BWKEY,
                    BC40_05_TT_BKPF_BSEG.BSEG_LIFNR,
                    BC40_05_TT_BKPF_BSEG.BSEG_KUNNR,
                    BC40_05_TT_BKPF_BSEG.BSEG_MATNR,
                    BC40_05_TT_BKPF_BSEG.BSEG_ANLN1,
                    BC40_05_TT_BKPF_BSEG.BSEG_ANLN2,
                    BC40_05_TT_BKPF_BSEG.BSEG_PROJK,
                    BC40_05_TT_BKPF_BSEG.BSEG_EBELN,
                    BC40_05_TT_BKPF_BSEG.BSEG_EBELP,
                    BC40_05_TT_BKPF_BSEG.BSEG_KOSTL,
					BC40_05_TT_BKPF_BSEG.BSEG_KOKRS,
					BC40_05_TT_BKPF_BSEG.BSEG_AUGGJ,
					BC40_05_TT_BKPF_BSEG.BSEG_HKONT,
					BC40_05_TT_BKPF_BSEG.BSEG_UMSKZ,
					BC40_05_TT_BKPF_BSEG.BSEG_AUFNR,
					BC40_05_TT_BKPF_BSEG.BSEG_ZUONR,
					BC40_05_TT_BKPF_BSEG.BSEG_GSBER,
                    BC40_05_TT_BKPF_BSEG.BSEG_XBILK,
                    BC40_05_TT_BKPF_BSEG.BSEG_GVTYP,
                    BC40_05_TT_BKPF_BSEG.BSEG_XAUTO,
                    BC40_05_TT_BKPF_BSEG.BSEG_XHKOM,
                    BC40_05_TT_BKPF_BSEG.BSEG_XNEGP,
                    BC40_05_TT_BKPF_BSEG.BSEG_SHKZG,
                    -- Add Debit/Credit description
                    CASE BC40_05_TT_BKPF_BSEG.BSEG_SHKZG
                    WHEN 'S'  THEN 'Debit'
                    WHEN 'H'  THEN 'Credit'
                    END                                                                        AS ZF_BSEG_SHKZG_DESC,
                    BC40_05_TT_BKPF_BSEG.BKPF_WAERS                                             AS BKPF_WAERS,
 
                    -- Add value (document currency)
                    CONVERT(money,BC40_05_TT_BKPF_BSEG.BSEG_WRBTR * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S' THEN 1 ELSE -1 END) * ISNULL(TCURX_DOC.TCURX_factor,1)) AS ZF_BSEG_WRBTR_S_DOC, 
			
                    BC40_05_TT_BKPF_BSEG.BKPF_HWAER,
 
                    -- Add Value(company currency)
                    CONVERT(money,BC40_05_TT_BKPF_BSEG.BSEG_DMBTR * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S' THEN 1 ELSE -1 END) * ISNULL(TCURX_COC.TCURX_factor,1)) AS ZF_BSEG_DMBTR_S,
                    CONVERT(money,BC40_05_TT_BKPF_BSEG.BSEG_DMBTR * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S' THEN 1 ELSE 0 END) * ISNULL(TCURX_COC.TCURX_factor,1)) AS ZF_BSEG_DMBTR_DB,
                    CONVERT(money,BC40_05_TT_BKPF_BSEG.BSEG_DMBTR * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'H' THEN 1 ELSE 0 END) * ISNULL(TCURX_COC.TCURX_factor,1)) AS ZF_BSEG_DMBTR_CR,
                    ABS(CONVERT(money,BC40_05_TT_BKPF_BSEG.BSEG_DMBTR * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S' THEN 1 ELSE -1 END) * ISNULL(TCURX_COC.TCURX_factor,1))) AS ZF_BSEG_DMBTR_S_ABS,
                    
                    @currency AS AM_GLOBALS_CURRENCY,
                    -- Add Value(custom currency)
                    CONVERT(money,BC40_05_TT_BKPF_BSEG.BSEG_DMBTR * (CASE WHEN (BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S') THEN 1 ELSE -1 END) * COALESCE(CAST(B00_IT_TCURR.TCURR_UKURS AS FLOAT),1) * COALESCE(B00_IT_TCURF.TCURF_TFACT,1) / COALESCE(B00_IT_TCURF.TCURF_FFACT,1) * ISNULL(TCURX_COC.TCURX_factor,1)) AS ZF_BSEG_DMBTR_S_CUC,
					BC40_05_TT_BKPF_BSEG.BSEG_DMBE2 * (CASE WHEN (BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S') THEN 1 ELSE -1 END) ZF_BSEG_DMBE2_S,
					BC40_05_TT_BKPF_BSEG.BSEG_DMBE3 * (CASE WHEN (BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S') THEN 1 ELSE -1 END) ZF_BSEG_DMBE3_S,
					BC40_05_TT_BKPF_BSEG.BKPF_AWSYS,
                    BC40_05_TT_BKPF_BSEG.BSEG_KTOSL,  
                    BC40_05_TT_BKPF_BSEG.BKPF_GLVOR,
                    BC40_05_TT_BKPF_BSEG.BKPF_XBLNR,
                    BC40_05_TT_BKPF_BSEG.BKPF_AWTYP,
 
                    -- Add Reference document
                    Left(BC40_05_TT_BKPF_BSEG.BKPF_AWKEY, 10)                             AS ZF_BKPF_AWKEY_DOC_NUM,
 
                    -- Add Reference year
                    RIGHT(BC40_05_TT_BKPF_BSEG.BKPF_AWKEY,4)                              AS ZF_BKPF_AWKEY_YEAR,
                    BC40_05_TT_BKPF_BSEG.BKPF_STJAH,
                    BC40_05_TT_BKPF_BSEG.BKPF_STBLG,
                    BC40_05_TT_BKPF_BSEG.BKPF_STGRD, 
                    BC40_05_TT_BKPF_BSEG.BKPF_BSTAT,
                    A_T074U.T074U_MERKP,
                    BC40_05_TT_BKPF_BSEG.BKPF_GRPID,
                    BC40_05_TT_BKPF_BSEG.BKPF_BVORG,
                    BC40_05_TT_BKPF_BSEG.BKPF_DBBLG,
                    BC40_05_TT_BKPF_BSEG.BSEG_HWMET,
 
                    -- Add indicator to show if it is an electronic bank payment
                    CASE WHEN
                        BC40_06_TT_FEBKO_EP.FEBEP_BELNR IS NULL THEN ''
                        ELSE 'X'
                ...
                -- Add cost center description
                    ,BC40_05_TT_BKPF_BSEG.ZF_CSKT_MCTXT
				-- add profit center desc
				    ,BC40_05_TT_BKPF_BSEG.BSEG_PRCTR
				    ,BC40_05_TT_BKPF_BSEG.CEPCT_MCTXT
				-- Concatenate codes and descriptions
                    ,BC40_05_TT_BKPF_BSEG.BSEG_KOART + ' - '  + CASE
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'A' THEN 'Assets'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'D' THEN 'Customers'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'K' THEN 'Vendors'
                        WHEN BC40_05_TT_BKPF_BSEG.BSEG_KOART = 'M' THEN 'Material'
                    ...
                    ,BC40_05_TT_BKPF_BSEG.BSEG_KOSTL + ' - '  + BC40_05_TT_BKPF_BSEG.ZF_CSKT_MCTXT AS BSEG_KOSTL_CSKT_MCTXT  -- [BSEG_KOSTL] + ' - '  +  [ZF_CSKT_MCTXT] AS [BSEG_KOSTL_CSKT_MCTXT]
					--,BC40_05_TT_BKPF_BSEG.BSEG_KOKRS
       ...
       -- Additional fields that are required for manual journal entries consideration
        ...
        -- Additional fields that are interesting for manual journal entry analysis
         ...
         ,(CONVERT(MONEY,BC40_05_TT_BKPF_BSEG.BSEG_DMBTR * COALESCE(CAST(B00_IT_TCURR.TCURR_UKURS AS FLOAT),1) * COALESCE(B00_IT_TCURF.TCURF_TFACT,1) / COALESCE(B00_IT_TCURF.TCURF_FFACT,1) * ISNULL(TCURX_COC.TCURX_factor,1)))
                AS ZF_BSEG_DMBTR_CUC_ABS
		-- Fields required for debit-credit refresh
		,BC40_05_TT_BKPF_BSEG.BSEG_ZFBDT,
		CASE WHEN
			...
			AND ISNULL(BC40_06_TT_FEBKO_EP.FEBEP_BELNR, '') = ''
	   ...
	   A_KNA1_B.KNA1_NAME1 AS BSEG_LFA1_KUNNR_NAME1,
		...
		LEFT JOIN B00_T009B B00_T009B_A
			ON T001_PERIV = B00_T009B_A.T009B_PERIV AND
			   ...
			   B00_T009B_A.T009B_BDATJ = YEAR(BC40_05_TT_BKPF_BSEG.BKPF_BUDAT)
		...
		-- Get customer group
				LEFT JOIN A_KNA1 ON BC40_05_TT_BKPF_BSEG.BSEG_KUNNR = KNA1_KUNNR
		...
		-- Get supplier group number
				LEFT JOIN A_LFA1 ON BC40_05_TT_BKPF_BSEG.BSEG_LIFNR = LFA1_LIFNR
		...
		LEFT JOIN A_USR02
              ON  (BC40_05_TT_BKPF_BSEG.BKPF_USNAM = A_USR02.USR02_BNAME)
		...
		AND B00_IT_TCURF.TCURF_GDATU = (
			...
			WHERE A_T001.T001_WAERS = B00_IT_TCURF.TCURF_FCURR AND 
					...
					B00_IT_TCURF.TCURF_GDATU <= BC40_05_TT_BKPF_BSEG.BKPF_BUDAT
	...
	LEFT JOIN B00_IT_TCURR
		...
		AND B00_IT_TCURR.TCURR_GDATU = (
			...
			WHERE A_T001.T001_WAERS = B00_IT_TCURR.TCURR_FCURR AND 
					...
					B00_IT_TCURR.TCURR_GDATU <= BC40_05_TT_BKPF_BSEG.BKPF_BUDAT
			ORDER BY B00_IT_TCURR.TCURR_GDATU DESC
			) 
           -- Add currency factor for house currency
		LEFT JOIN B00_TCURX TCURX_COC
		ON BC40_05_TT_BKPF_BSEG.BKPF_HWAER = TCURX_COC.TCURX_CURRKEY

       -- Add currency factor for document currency   
		LEFT JOIN B00_TCURX TCURX_DOC
		ON     
				BC40_05_TT_BKPF_BSEG.BKPF_WAERS = TCURX_DOC.TCURX_CURRKEY
             
		-- Add account type info and Special GL's
		LEFT JOIN A_T074U
		ON   (BC40_05_TT_BKPF_BSEG.BSEG_KOART = A_T074U.T074U_KOART) AND
				(BC40_05_TT_BKPF_BSEG.BSEG_UMSKZ = A_T074U.T074U_UMSKZ)

        -- Add information about electronic bank payments    
              LEFT JOIN BC40_06_TT_FEBKO_EP
        ON (BC40_05_TT_BKPF_BSEG.BSEG_BUKRS = BC40_06_TT_FEBKO_EP.FEBKO_BUKRS) AND
            (BC40_05_TT_BKPF_BSEG.BSEG_GJAHR = BC40_06_TT_FEBKO_EP.FEBEP_GJAHR) AND
            (BC40_05_TT_BKPF_BSEG.BSEG_BELNR = BC40_06_TT_FEBKO_EP.FEBEP_BELNR)

        ---- Add information indicators from GL itself
        LEFT JOIN BC40_07_TT_INDIC_PER_JE_NUM
        ON  BC40_05_TT_BKPF_BSEG.BSEG_BUKRS = BC40_07_TT_INDIC_PER_JE_NUM.BSEG_BUKRS
            AND BC40_05_TT_BKPF_BSEG.BSEG_GJAHR = BC40_07_TT_INDIC_PER_JE_NUM.BSEG_GJAHR
            AND BC40_05_TT_BKPF_BSEG.BSEG_BELNR = BC40_07_TT_INDIC_PER_JE_NUM.BSEG_BELNR
		...
		LEFT JOIN A_ANLT
			ON ANLT_BUKRS=BC40_05_TT_BKPF_BSEG.BSEG_BUKRS AND
			   ANLT_ANLN1=BC40_05_TT_BKPF_BSEG.BSEG_ANLN1 AND
			   ANLT_ANLN2=BC40_05_TT_BKPF_BSEG.BSEG_ANLN2
		...
		LEFT JOIN A_LFA1 AS A_LFA1_B
			...
			WHERE  ISNULL(BC40_05_TT_BKPF_BSEG.BKPF_BSTAT, '')    = ''
			AND           ISNULL(A_T074U.T074U_MERKP,'')           = ''
			AND           BC40_05_TT_BKPF_BSEG.BKPF_BUDAT    >= @date1
			AND           BC40_05_TT_BKPF_BSEG.BKPF_BUDAT    <= @date2
			  

--UPDATE BC40_11_IT_FIN_GL
...
EXEC SP_DROPTABLE    'BC40_11_TT_JE_ACCOUNT_TEMP'

SELECT 
	BC40_11_IT_FIN_GL.BSEG_BUKRS,
	BC40_11_IT_FIN_GL.BSEG_GJAHR,
	BC40_11_IT_FIN_GL.BSEG_BELNR,
	BC40_11_IT_FIN_GL.BSEG_KOART,
	BC40_11_IT_FIN_GL.BSEG_SHKZG,
	BC40_05_TT_BKPF_BSEG.ZF_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG,
	BC40_05_TT_BKPF_BSEG.SKA1_GVTYP,
	BC40_11_IT_FIN_GL.BSEG_HKONT,
	BC40_05_TT_BKPF_BSEG.SKAT_TXT20,
	BC40_05_TT_BKPF_BSEG.T001_BUTXT, 
	BC40_11_IT_FIN_GL.BKPF_MONAT,
	BC40_11_IT_FIN_GL.BKPF_BLART,
	T003T_LTEXT,
	BC40_11_IT_FIN_GL.BKPF_BUDAT,
	BC40_11_IT_FIN_GL.BKPF_BLDAT,
	BC40_11_IT_FIN_GL.BKPF_CPUDT,
	BC40_11_IT_FIN_GL.BKPF_TCODE,
	TSTCT_TTEXT,
	BC40_11_IT_FIN_GL.BKPF_AWTYP,
	T001_WAERS,
	BC40_11_IT_FIN_GL.BKPF_USNAM,
	BC40_11_IT_FIN_GL.ZF_ENTRY_TYPE
	TANGO_ACCT, -- Get tango account
	TANGO_ACCT_TXT -- Get tango account description.              
	,CASE 
			
			WHEN BC40_11_IT_FIN_GL.BSEG_KOART <> 'S' THEN BC40_11_IT_FIN_GL.BSEG_KOART
			WHEN BC40_11_IT_FIN_GL.BSEG_KOART = 'S' AND  
				(
					ISNULL(SKB1_HBKID,'') <> '' OR ISNULL(SKB1_XGKON,'') <> ''

				)	THEN 'S: Bank'
			WHEN BC40_11_IT_FIN_GL.BSEG_KOART = 'S'  AND  ( ISNULL(SKA1_GVTYP,'') <> ''  )	
					THEN  
						CASE WHEN LEN(BC40_11_IT_FIN_GL.BSEG_PRCTR)>0 THEN 'S: P&L : Profit'
							 WHEN LEN(BC40_11_IT_FIN_GL.BSEG_KOSTL)>0 THEN 'S: P&L : Loss'
							ELSE 'S: P&L ' END
			WHEN BC40_11_IT_FIN_GL.BSEG_KOART = 'S' AND  
				(
	 ...
	 END AS ZF_BSEG_KOART,
	  ...
	  ZF_BSEG_DMBTR_S * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S' THEN 1 ELSE 0 END) AS ZF_BSEG_DMBTR_S_DB,
	  ZF_BSEG_DMBTR_S * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'H' THEN 1 ELSE 0 END) AS ZF_BSEG_DMBTR_S_CR,
	  ZF_BSEG_DMBTR_S,
	  ZF_BSEG_DMBTR_S_CUC * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'S' THEN 1 ELSE 0 END) AS ZF_BSEG_DMBTR_S_CUC_DB,
	  ZF_BSEG_DMBTR_S_CUC * (CASE WHEN BC40_05_TT_BKPF_BSEG.BSEG_SHKZG = 'H' THEN 1 ELSE 0 END) AS ZF_BSEG_DMBTR_S_CUC_CR,
	  V_USERNAME_NAME_TEXT,
	  ZF_ENTRY_TYPE,
	  ZF_BKPF_KOART_DESC,
	  BC40_11_IT_FIN_GL.BSEG_LIFNR,
	  BC40_11_IT_FIN_GL.LFA1_KTOKK,
	   --Supplier account group description
      B00_T077Y.T077Y_TXT30,
	  BC40_11_IT_FIN_GL.BSEG_KUNNR,
	  BC40_11_IT_FIN_GL.KNA1_KTOKD,
	  BC40_11_IT_FIN_GL.T077X_TXT30
--	 , ZF_HKONT_FOUND_IN_CSKB
		BSEG_KNA1_NAME1,
		BSEG_LFA1_NAME1,
		MARA_MATKL,
		T023T_WGBEZ,
		BC40_11_IT_FIN_GL.BSEG_ANLN1,
		BC40_11_IT_FIN_GL.BSEG_ANLN2,
		ANLT_TXT50,
		BSEG_LFA1_KUNNR,
		...
		T077X_TXT30,
		BSEG_KNA1_LIFNR_NAME1
INTO BC40_11_TT_JE_ACCOUNT_TEMP
FROM BC40_11_IT_FIN_GL
LEFT JOIN BC40_05_TT_BKPF_BSEG 
	ON BC40_11_IT_FIN_GL.BSEG_BELNR = BC40_05_TT_BKPF_BSEG.BSEG_BELNR
	AND BC40_11_IT_FIN_GL.BSEG_BUKRS = BC40_05_TT_BKPF_BSEG.BSEG_BUKRS
	AND BC40_11_IT_FIN_GL.BSEG_GJAHR = BC40_05_TT_BKPF_BSEG.BSEG_GJAHR
	AND BC40_11_IT_FIN_GL.BSEG_BUZEI	 = BC40_05_TT_BKPF_BSEG.BSEG_BUZEI

-- Add document type description
LEFT JOIN B00_T003T
    ON BC40_11_IT_FIN_GL.BKPF_BLART = B00_T003T.T003T_BLART

-- Obtain company description and house currency
LEFT JOIN A_T001
        ON  BC40_11_IT_FIN_GL.BSEG_BUKRS = A_T001.T001_BUKRS
LEFT JOIN A_V_USERNAME
          ON BC40_11_IT_FIN_GL.BKPF_USNAM = A_V_USERNAME.V_USERNAME_BNAME
LEFT JOIN AM_TANGO
		ON RIGHT(CONCAT('0000000000',BC40_11_IT_FIN_GL.BSEG_HKONT), 10) = RIGHT(CONCAT('0000000000',TANGO_GL_ACCT), 10)
-- Add supplier information
LEFT JOIN A_LFA1
        ON BC40_11_IT_FIN_GL.BSEG_LIFNR = A_LFA1.LFA1_LIFNR
...
LEFT JOIN A_KNA1 
	ON KNA1_KUNNR = BC40_11_IT_FIN_GL.BSEG_KUNNR
-- Add customer account group
LEFT JOIN B00_T077X
    ON A_KNA1.KNA1_KTOKD = B00_T077X.T077X_KTOKD


-- Step 6.3 Need to create JE accounting cube

EXEC SP_DROPTABLE    'BC40_07_IT_BSEG_BKPF_ACC_SCH'
...
SELECT 
 ...
 --Create a list of debit account types
    ...
    ISNULL(STUFF((SELECT '_' + B.BSEG_KOART
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.BSEG_KOART
...
--Create a list indicator of BSEG_HKONT for debit
	ISNULL(STUFF((SELECT '_' +   B.BSEG_HKONT
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
...
--Create a list indicator of BSEG_HKONT for debit
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_SKAT_TXT20_LIST,
...
--Create a list indicator of BSEG_HKONT for credit
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_CREDIT_SKAT_TXT20_LIST,

 --Create a list of debit account numbers + descriptions

 	ISNULL(STUFF((SELECT '_' + B.BSEG_HKONT +' : '+B.SKAT_TXT20
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_HKONT_TXT20_LIST,
 ...
 --Create a list of credit account numbers + descriptions
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_CREDIT_HKONT_TXT20_LIST

--Debit customer name
					,ISNULL(STUFF((SELECT '_' + ISNULL(B.BSEG_KNA1_NAME1,'')
				   FROM BC40_11_TT_JE_ACCOUNT_TEMP B
				   WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS
								 AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
								 AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
								 AND    B.BSEG_SHKZG = 'S' AND B.BSEG_HKONT  <> '')
				   GROUP BY B.BSEG_HKONT, B.BSEG_KNA1_NAME1 
				   ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
				   FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
				,1,1,''), '_') ZF_BSEG_KUNNR_NAME1_DEBIT_LIST
					 --Credit customer name
					 ,ISNULL(STUFF((SELECT '_'+  B.BSEG_KNA1_NAME1
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_KUNNR_NAME1_CREDIT_LIST
					 --Debit supplier name
					 ,ISNULL(STUFF((SELECT '_'+ ISNULL(B.BSEG_LFA1_NAME1,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.BSEG_LFA1_NAME1
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_LIFNR_NAME1_DEBIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_LIFNR_NAME1_CREDIT_LIST
					 --Debit material group
					 ,ISNULL(STUFF((SELECT '_'+ ISNULL(B.MARA_MATKL,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.MARA_MATKL
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_MATNR_MARA_MATKL_DEBIT_LIST
					 --Debit material name group
					 ,ISNULL(STUFF((SELECT '_' +ISNULL(B.T023T_WGBEZ,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.T023T_WGBEZ
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_MATNR_T023T_WGBEZ_DEBIT_LIST
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_MATNR_MARA_MATKL_CREDIT_LIST
					 --Credit material group desciption
					 ,ISNULL(STUFF((SELECT '_' + ISNULL(B.T023T_WGBEZ,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_MATNR_T023T_WGBEZ_CREDIT_LIST
					 --Debit asset
					 ,ISNULL(STUFF((SELECT '_'  + B.BSEG_ANLN1+ '|' + B.BSEG_ANLN2
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.BSEG_ANLN1,B.BSEG_ANLN2
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC,  B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_ANLN1_ANLN2_DEBIT_LIST
					 --Debit asset description
					 ,ISNULL(STUFF((SELECT '_'   + ISNULL(B.ANLT_TXT50,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.ANLT_TXT50
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_ANLN1_ANLT_DEBIT_LIST
					 --Credit asset
					 ,ISNULL(STUFF((SELECT '_' + B.BSEG_ANLN1+ '|' + B.BSEG_ANLN2
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC,B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_ANLN1_ANLN2_CREDIT_LIST
					 --Credit asset description
					 ,ISNULL(STUFF((SELECT '_'    + ISNULL(B.ANLT_TXT50,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_BSEG_ANLN1_ANLT_CREDIT_LIST
					 --Debit LFA1_KUNNR
					 ,ISNULL(STUFF((SELECT '_'  + B.BSEG_LFA1_KUNNR
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.BSEG_LFA1_KUNNR
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_BSEG_DEBIT_LIST
					 --Debit LFA1_KUNNR name
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_LFA1_KUNNR_NAME1,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.BSEG_LFA1_KUNNR_NAME1
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_NAME1_BSEG_DEBIT_LIST
					 --Credit LFA1_KUNNR
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_LFA1_KUNNR,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_BSEG_CREDIT_LIST
					 --Credit LFA1_KUNNR name
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_LFA1_KUNNR_NAME1,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_LFA1_KUNNR_NAME1_BSEG_CREDIT_LIST
					 --Debit KNA1_LIFNR
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_KNA1_LIFNR,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.BSEG_KNA1_LIFNR
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_BSEG_DEBIT_LIST
					-- Debit KNA1_LIFNR name
					,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_KNA1_LIFNR_NAME1,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
                           AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
                           AND    B.BSEG_SHKZG = 'S')
                     GROUP BY B.BSEG_HKONT, B.BSEG_KNA1_LIFNR_NAME1
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_NAME1_BSEG_DEBIT_LIST
					 --Credit KNA1_LIFNR
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_KNA1_LIFNR,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_BSEG_CREDIT_LIST
					--Credit KNA1_LIFNR name
					 ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.BSEG_KNA1_LIFNR_NAME1,'')
                     FROM BC40_11_TT_JE_ACCOUNT_TEMP B
                     WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
                           AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
					 ...
					 ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
                     FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
                     ,1,1,''), '_') ZF_KNA1_LIFNR_NAME1_BSEG_CREDIT_LIST
					-- --Debit found CSKB flag
					-- ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ZF_HKONT_FOUND_IN_CSKB,'')
     --                FROM BC40_11_TT_JE_ACCOUNT_TEMP B
     --                WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
     --                      AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
     --                      AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
     --                      AND    B.BSEG_SHKZG = 'S')
     --                GROUP BY B.BSEG_HKONT, B.ZF_HKONT_FOUND_IN_CSKB
					-- ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
     --                FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
     --                ,1,1,''), '_') ZF_HKONT_FOUND_IN_CSKB_DEBIT
					----Credit found in CSKB flag
					-- ,ISNULL(STUFF((SELECT '_'  + ISNULL(B.ZF_HKONT_FOUND_IN_CSKB,'')
     --                FROM BC40_11_TT_JE_ACCOUNT_TEMP B
     --                WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
     --                      AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
     ...
     --                GROUP BY B.BSEG_HKONT, B.ZF_HKONT_FOUND_IN_CSKB
					-- ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
     --                FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
     --                ,1,1,''), '_') ZF_HKONT_FOUND_IN_CSKB_CREDIT
...
-- Add account type detail (blank, P&L, non-bank) request from Claire DEBIT
	...
	ISNULL(STUFF((SELECT '_' +    B.ZF_BSEG_KOART
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_BSEG_KOART_CUSTOM_LIST,
...
-- Add account type detail (blank, P&L, non-bank) request from Claire Credit
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_CREDIT_BSEG_KOART_CUSTOM_LIST,

-- Add account type desc debit
	ISNULL(STUFF((SELECT '_' +    B.ZF_BKPF_KOART_DESC
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.ZF_BKPF_KOART_DESC
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_ZF_BSEG_KOART_DESC_LIST,

-- Add account type desc credit
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_CREDIT_ZF_BSEG_KOART_DESC_LIST,

-- Supplier number debit
	ISNULL(STUFF((SELECT '_' +    B.BSEG_LIFNR
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.BSEG_LIFNR
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_BSEG_LIFNR_LIST,

-- Supplier number credit
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_CREDIT_BSEG_LIFNR_LIST,


-- Supplier group debit
    ...
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        ...
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.LFA1_KTOKK
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
...
-- Supplier group desc debit
    ...
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        ...
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.T077Y_TXT30
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_T077Y_TXT30_LIST,

-- Supplier group desc credit
...
-- Customer number debit
	ISNULL(STUFF((SELECT '_' +    B.BSEG_KUNNR
    FROM BC40_11_TT_JE_ACCOUNT_TEMP B
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        AND B.BSEG_GJAHR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_GJAHR
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.BSEG_KUNNR
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_BSEG_KUNNR_LIST,

-- Customer number credit
	...
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_CREDIT_BSEG_KUNNR_LIST,
...
-- Customer group  debit
    ...
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        ...
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.KNA1_KTOKD
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
...
-- Customer group desc debit
    ...
    WHERE (B.BSEG_BUKRS = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BUKRS  
        ...
        AND B.BSEG_BELNR = BC40_11_TT_JE_ACCOUNT_TEMP.BSEG_BELNR
        AND    B.BSEG_SHKZG = 'S')
    GROUP BY B.BSEG_HKONT, B.T077X_TXT30
	ORDER BY SUM(ABS(B.ZF_BSEG_DMBTR_S)) DESC, B.BSEG_HKONT
    FOR XML PATH(''),TYPE).value('(./text())[1]','VARCHAR(MAX)')
    ,1,1,''), '_') ZF_DEBIT_T077X_TXT30_LIST,

-- Customer group desc credit
...
INTO BC40_07_IT_BSEG_BKPF_ACC_SCH
FROM BC40_11_TT_JE_ACCOUNT_TEMP
GROUP BY BSEG_BELNR, BSEG_GJAHR, BSEG_BUKRS



DROP TABLE BC40_11_TT_JE_ACCOUNT_TEMP



EXEC SP_RENAME_FIELD 'BC40_', 'BC40_11_IT_FIN_GL'
...
EXEC sp_droptable 'BC40_08_TT_BSIK_BSAK'
...
EXEC SP_DROPTABLE    'BC40_10_IT_BSAK_BSIK_AP_ACC_SCH'
 
       SELECT BC40_08_TT_BSIK_BSAK.*
	   -- Header information
	   ,A_BKPF.BKPF_AWTYP
       ...
       ,CONVERT(MONEY,BSAIK_DMBTR * COALESCE(TCURX_CC.TCURX_FACTOR,1) * IIF(BSAIK_SHKZG = 'H', -1,1)  * COALESCE(CAST(B00_IT_TCURR.TCURR_UKURS AS FLOAT),1) * COALESCE(B00_IT_TCURF.TCURF_TFACT,1) / COALESCE(B00_IT_TCURF.TCURF_FFACT,1)) AS ZF_BSAIK_DMBTR_S_CUC	   
	   ,BSAIK_DMBE2 * IIF(BSAIK_SHKZG = 'H',-1,1) AS ZF_BSAIK_DMBE2_S
	   ...
	   --Accounting schemes:
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.BKPF_TCODE, '_') BKPF_TCODE
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_DEBIT_ACCOUNT_TYPES, '_') ZF_DEBIT_ACCOUNT_TYPES
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_CREDIT_ACCOUNT_TYPES, '_') ZF_CREDIT_ACCOUNT_TYPES
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_DEBIT_HKONT_LIST, '_') ZF_DEBIT_HKONT_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_CREDIT_HKONT_LIST, '_') ZF_CREDIT_HKONT_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_DEBIT_SKAT_TXT20_LIST, '_') ZF_DEBIT_SKAT_TXT20_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_CREDIT_SKAT_TXT20_LIST, '_') ZF_CREDIT_SKAT_TXT20_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_DEBIT_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG_LIST, '_') AS ZF_DEBIT_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_CREDIT_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG_LIST, '_') AS ZF_CREDIT_T012K_FEBKO_SKB1_HBKID_XGKON_FLAG_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_CREDIT_SKA1_GVTYP_FLAG_LIST, '_') AS ZF_CREDIT_SKA1_GVTYP_FLAG_LIST
		,ISNULL(BC40_07_IT_BSEG_BKPF_ACC_SCH.ZF_DEBIT_SKA1_GVTYP_FLAG_LIST, '_') AS ZF_DEBIT_SKA1_GVTYP_FLAG_LIST
        -- Cost center infromation needed in AP cube and based on BC40_02_TT_CSKT_DATBI
		,COALESCE(CASE 
			WHEN BC40_02_TT_CSKT_DATBI.CSKT_LTEXT ='' OR  BC40_02_TT_CSKT_DATBI.CSKT_LTEXT IS NULL THEN 'Not assigned'
			ELSE BSAIK_KOSTL 
		 END,'Not assigned') ZF_BSAIK_KOSTL
		-- Cost center description
		,COALESCE(CASE 
			WHEN BC40_02_TT_CSKT_DATBI.CSKT_LTEXT ='' OR  BC40_02_TT_CSKT_DATBI.CSKT_LTEXT IS NULL THEN 'Not assigned'
			ELSE BC40_02_TT_CSKT_DATBI.CSKT_LTEXT 
       ...
       INTO BC40_10_IT_BSAK_BSIK_AP_ACC_SCH

       FROM BC40_08_TT_BSIK_BSAK

	   -- Add some header information from BKPF tables
	   LEFT JOIN A_BKPF
			ON BC40_08_TT_BSIK_BSAK.BSAIK_BUKRS = A_BKPF.BKPF_BUKRS AND
				BC40_08_TT_BSIK_BSAK.BSAIK_GJAHR = A_BKPF.BKPF_GJAHR AND
				BC40_08_TT_BSIK_BSAK.BSAIK_BELNR = A_BKPF.BKPF_BELNR 

	   -- Add supplier information
       LEFT JOIN A_LFA1
              ON BC40_08_TT_BSIK_BSAK.BSAIK_LIFNR = A_LFA1.LFA1_LIFNR
	   -- Add supplier account group text
       LEFT JOIN B00_T077Y
              ON A_LFA1.LFA1_KTOKK = B00_T077Y.T077Y_KTOKK
	   -- Add document type description
       LEFT JOIN B00_T003T
           ON BC40_08_TT_BSIK_BSAK.BSAIK_BLART = B00_T003T.T003T_BLART
       -- Obtain company description and house currency
       LEFT JOIN A_T001
       ...
       -- Add currency factor from company currency to USD
		...
		AND B00_IT_TCURF.TCURF_GDATU = (
			...
			WHERE A_T001.T001_WAERS = B00_IT_TCURF.TCURF_FCURR AND 
					B00_IT_TCURF.TCURF_TCURR  = @currency  AND
					B00_IT_TCURF.TCURF_GDATU <= BC40_08_TT_BSIK_BSAK.BSAIK_BUDAT
			ORDER BY B00_IT_TCURF.TCURF_GDATU DESC
			)
		-- Add exchange rate from company currency to USD
		LEFT JOIN B00_IT_TCURR
			...
			AND B00_IT_TCURR.TCURR_GDATU = (
				...
				WHERE A_T001.T001_WAERS = B00_IT_TCURR.TCURR_FCURR AND 
						B00_IT_TCURR.TCURR_TCURR  = @currency  AND
						B00_IT_TCURR.TCURR_GDATU <= BC40_08_TT_BSIK_BSAK.BSAIK_BUDAT
				ORDER BY B00_IT_TCURR.TCURR_GDATU DESC
				) 
		...
		LEFT JOIN BC40_07_IT_BSEG_BKPF_ACC_SCH
		ON   BSAIK_BUKRS = BC40_07_IT_BSEG_BKPF_ACC_SCH.BSEG_BUKRS AND
				BSAIK_GJAHR = BC40_07_IT_BSEG_BKPF_ACC_SCH.BSEG_GJAHR AND
				BSAIK_BELNR = BC40_07_IT_BSEG_BKPF_ACC_SCH.BSEG_BELNR
       ...
       -- Add the controlling area in order to be able to add cost center information		
		...
		LEFT JOIN BC40_02_TT_CSKT_DATBI
			ON A_TKA02.TKA02_KOKRS = BC40_02_TT_CSKT_DATBI.CSKT_KOKRS
			AND BSAIK_KOSTL = BC40_02_TT_CSKT_DATBI.CSKT_KOSTL
			AND BC40_02_TT_CSKT_DATBI.CSKT_DATBI > @downloaddate

		-- Include profit centers descriptions
		LEFT JOIN BC40_04_TT_CEPCT_UNIQUE
		ON	BSAIK_BUKRS = BC40_04_TT_CEPCT_UNIQUE.CEPCT_KOKRS AND
			BSAIK_PRCTR = BC40_04_TT_CEPCT_UNIQUE.CEPCT_PRCTR

		-- Get posting key description
		LEFT JOIN B00_TBSLT
		ON B00_TBSLT.TBSLT_BSCHL = BC40_08_TT_BSIK_BSAK.BSAIK_BSCHL AND
		   B00_TBSLT.TBSLT_UMSKZ = BC40_08_TT_BSIK_BSAK.BSAIK_UMSKZ

		--Get special g/l indicator description
		LEFT JOIN A_T074T
		ON A_T074T.T074T_SPRAS IN ('E', 'EN') AND
		   A_T074T.T074T_SHBKZ = BC40_08_TT_BSIK_BSAK.BSAIK_UMSKZ AND
		...
		*/
		...
		EXEC SP_DROPTABLE 'BC40_13B_IT_FIN_GL_CHECK_LIST'
		SELECT DISTINCT BC40_BSEG_BUKRS, BC40_BSEG_GJAHR, BC40_BSEG_BELNR, BC40_BSEG_KUNNR, BC40_BSEG_SHKZG, BC40_BSEG_KOART INTO BC40_13B_IT_FIN_GL_CHECK_LIST FROM BC40_11_IT_FIN_GL
		WHERE BC40_BSEG_SHKZG = 'S' AND BC40_BSEG_KOART = 'D'
        EXEC sp_CREATE_INDEX 'BC40_BSEG_BUKRS, BC40_BSEG_GJAHR, BC40_BSEG_BELNR, BC40_BSEG_KUNNR, BC40_BSEG_SHKZG, BC40_BSEG_KOART', 'BC40_13B_IT_FIN_GL_CHECK_LIST'

        ALTER TABLE BC40_10_IT_BSAK_BSIK_AP_ACC_SCH ADD ZF_LIFNR_KUNNR_IS_SAME_FLAG NVARCHAR(1) DEFAULT 'N' WITH VALUES;
		UPDATE BC40_10_IT_BSAK_BSIK_AP_ACC_SCH
		SET ZF_LIFNR_KUNNR_IS_SAME_FLAG = 'Y'
		WHERE EXISTS(
			SELECT *
			FROM BC40_13B_IT_FIN_GL_CHECK_LIST
			WHERE BC40_13B_IT_FIN_GL_CHECK_LIST.BC40_BSEG_BUKRS = BC40_10_IT_BSAK_BSIK_AP_ACC_SCH.BSAIK_BUKRS AND
			   BC40_13B_IT_FIN_GL_CHECK_LIST.BC40_BSEG_GJAHR = BC40_10_IT_BSAK_BSIK_AP_ACC_SCH.BSAIK_GJAHR AND
			   BC40_13B_IT_FIN_GL_CHECK_LIST.BC40_BSEG_BELNR = BC40_10_IT_BSAK_BSIK_AP_ACC_SCH.BSAIK_BELNR AND
			   BC40_13B_IT_FIN_GL_CHECK_LIST.BC40_BSEG_KUNNR = BC40_10_IT_BSAK_BSIK_AP_ACC_SCH.LFA1_KUNNR
		)
		AND BC40_10_IT_BSAK_BSIK_AP_ACC_SCH.BSAIK_SHKZG = 'H' AND ISNULL(BC40_10_IT_BSAK_BSIK_AP_ACC_SCH.LFA1_KUNNR, '') <> ''


-- Step 10/ Append BSAD and BSID tables and create a list to which accounting schemes can be added

EXEC sp_droptable 'BC40_11_TT_BSID_BSAD'
  ...
  -- Open items
   INSERT INTO BC40_11_TT_BSID_BSAD
       SELECT
...
EXEC SP_DROPTABLE    'BC40_12_IT_BSAD_BSID_AR_ACC_SCH'
 
       SELECT BC40_11_TT_BSID_BSAD.*
	   ...
	   --Accounting schemes:
		...
		,COALESCE(CASE 
		 ...
		 KNA1_LIFNR,
		 T077X_TXT30
       INTO BC40_12_IT_BSAD_BSID_AR_ACC_SCH
       FROM BC40_11_TT_BSID_BSAD

	   --Get header informationp for BKPF table
	   LEFT JOIN A_BKPF
	   ON	BC40_11_TT_BSID_BSAD.BSAID_BUKRS = A_BKPF.BKPF_BUKRS AND
			BC40_11_TT_BSID_BSAD.BSAID_GJAHR = A_BKPF.BKPF_GJAHR AND 
			BC40_11_TT_BSID_BSAD.BSAID_BELNR = A_BKPF.BKPF_BELNR
	   ...
	   -- Add document type description
       LEFT JOIN B00_T003T
           ON BC40_11_TT_BSID_BSAD.BSAID_BLART = (B00_T003T.T003T_BLART)
       -- Obtain company description and house currency
       LEFT JOIN A_T001
		...
		AND B00_IT_TCURF.TCURF_GDATU = (
			...
			WHERE A_T001.T001_WAERS = B00_IT_TCURF.TCURF_FCURR AND 
					B00_IT_TCURF.TCURF_TCURR  = @currency  AND
					B00_IT_TCURF.TCURF_GDATU <= BC40_11_TT_BSID_BSAD.BSAID_BUDAT
			ORDER BY B00_IT_TCURF.TCURF_GDATU DESC
			)
		-- Add exchange rate from company currency to USD
		LEFT JOIN B00_IT_TCURR
			...
			AND B00_IT_TCURR.TCURR_GDATU = (
				...
				WHERE A_T001.T001_WAERS = B00_IT_TCURR.TCURR_FCURR AND 
						B00_IT_TCURR.TCURR_TCURR  = @currency  AND
						B00_IT_TCURR.TCURR_GDATU <= BC40_11_TT_BSID_BSAD.BSAID_BUDAT
				ORDER BY B00_IT_TCURR.TCURR_GDATU DESC
				) 
		...
		-- Add the accounting schemes
		LEFT JOIN BC40_07_IT_BSEG_BKPF_ACC_SCH
		ON   BSAID_BUKRS = BC40_07_IT_BSEG_BKPF_ACC_SCH.BSEG_BUKRS AND
				BSAID_GJAHR = BC40_07_IT_BSEG_BKPF_ACC_SCH.BSEG_GJAHR AND
				BSAID_BELNR = BC40_07_IT_BSEG_BKPF_ACC_SCH.BSEG_BELNR
       ...
       -- Add the controlling area in order to be able to add cost center information		
		...
		LEFT JOIN BC40_02_TT_CSKT_DATBI
			ON A_TKA02.TKA02_KOKRS = BC40_02_TT_CSKT_DATBI.CSKT_KOKRS
			AND BSAID_KOSTL = BC40_02_TT_CSKT_DATBI.CSKT_KOSTL
			AND BC40_02_TT_CSKT_DATBI.CSKT_DATBI > @downloaddate

		-- Include profit centers descriptions
		LEFT JOIN BC40_04_TT_CEPCT_UNIQUE
		ON	BSAID_BUKRS = BC40_04_TT_CEPCT_UNIQUE.CEPCT_KOKRS AND
			BSAID_PRCTR = BC40_04_TT_CEPCT_UNIQUE.CEPCT_PRCTR 

		-- Get posting key description
		LEFT JOIN B00_TBSLT
		ON B00_TBSLT.TBSLT_BSCHL = BC40_11_TT_BSID_BSAD.BSAID_BSCHL AND
		   B00_TBSLT.TBSLT_UMSKZ = BC40_11_TT_BSID_BSAD.BSAID_UMSKZ

		--Get special g/l indicator description
		LEFT JOIN A_T074T
		ON A_T074T.T074T_SPRAS IN ('E', 'EN') AND
		   A_T074T.T074T_SHBKZ = BC40_11_TT_BSID_BSAD.BSAID_UMSKZ AND
	...
	*/
		EXEC SP_DROPTABLE 'BC40_13B_IT_FIN_GL_CHECK_LIST'
		SELECT DISTINCT BC40_BSEG_BUKRS, BC40_BSEG_GJAHR, BC40_BSEG_BELNR, BC40_BSEG_KUNNR, BC40_BSEG_SHKZG, BC40_BSEG_KOART INTO BC40_13B_IT_FIN_GL_CHECK_LIST FROM BC40_11_IT_FIN_GL
		WHERE BC40_BSEG_SHKZG = 'H' AND BC40_BSEG_KOART = 'K'


		ALTER TABLE BC40_12_IT_BSAD_BSID_AR_ACC_SCH ADD ZF_LIFNR_KUNNR_IS_SAME_FLAG NVARCHAR(1) DEFAULT 'N' WITH VALUES;
		UPDATE BC40_12_IT_BSAD_BSID_AR_ACC_SCH
		SET ZF_LIFNR_KUNNR_IS_SAME_FLAG = 'Y'
		WHERE EXISTS(
			SELECT TOP 1 1
			FROM BC40_11_IT_FIN_GL
			WHERE BC40_11_IT_FIN_GL.BC40_BSEG_BUKRS = BC40_12_IT_BSAD_BSID_AR_ACC_SCH.BSAID_BUKRS AND
			   BC40_11_IT_FIN_GL.BC40_BSEG_GJAHR = BC40_12_IT_BSAD_BSID_AR_ACC_SCH.BSAID_GJAHR AND
			   BC40_11_IT_FIN_GL.BC40_BSEG_BELNR = BC40_12_IT_BSAD_BSID_AR_ACC_SCH.BSAID_BELNR AND
			   BC40_11_IT_FIN_GL.BC40_BSEG_LIFNR = BC40_12_IT_BSAD_BSID_AR_ACC_SCH.KNA1_LIFNR AND
			   BC40_11_IT_FIN_GL.BC40_BSEG_SHKZG = 'H' AND BC40_11_IT_FIN_GL.BC40_BSEG_KOART = 'K'
			   AND ISNULL(BC40_BSEG_LIFNR, '') <> ''
		)
		AND BC40_12_IT_BSAD_BSID_AR_ACC_SCH.BSAID_SHKZG = 'S' 

		

--/*Drop temporary tables*/
--EXEC SP_REMOVE_TABLES '%[_]TT[_]%'
EXEC sp_RENAME_FIELD 'BC40B_', 'BC40_12_IT_BSAD_BSID_AR_ACC_SCH'
EXEC SP_REMOVE_TABLES '%_TT_%'
/* log cube creation*/
 
INSERT INTO [DBO].[LOG_SP_EXECUTION] ([DATABASE],[OBJECT],[OBJECT_TYPE],[USER],[DATE],[TIME],[DESCRIPTION],[TABLE],[ROWS])
SELECT DB_NAME(),OBJECT_NAME(@@PROCID),'P',SYSTEM_USER,CONVERT(DATE,GETDATE()),CONVERT(TIME,GETDATE()),'Cube completed','BC40_05_IT_FIN_GL',(SELECT COUNT(*) FROM BC40_11_IT_FIN_GL)
INSERT INTO [DBO].[LOG_SP_EXECUTION] ([DATABASE],[OBJECT],[OBJECT_TYPE],[USER],[DATE],[TIME],[DESCRIPTION],[TABLE],[ROWS])
SELECT DB_NAME(),OBJECT_NAME(@@PROCID),'P',SYSTEM_USER,CONVERT(DATE,GETDATE()),CONVERT(TIME,GETDATE()),'Cube completed','BC40_07_IT_BSEG_BKPF_ACC_SCH',(SELECT COUNT(*) FROM BC40_07_IT_BSEG_BKPF_ACC_SCH)
INSERT INTO [DBO].[LOG_SP_EXECUTION] ([DATABASE],[OBJECT],[OBJECT_TYPE],[USER],[DATE],[TIME],[DESCRIPTION],[TABLE],[ROWS])
SELECT DB_NAME(),OBJECT_NAME(@@PROCID),'P',SYSTEM_USER,CONVERT(DATE,GETDATE()),CONVERT(TIME,GETDATE()),'Cube completed','BC40_10_IT_BSAK_BSIK_AP_ACC_SCH',(SELECT COUNT(*) FROM BC40_10_IT_BSAK_BSIK_AP_ACC_SCH)
