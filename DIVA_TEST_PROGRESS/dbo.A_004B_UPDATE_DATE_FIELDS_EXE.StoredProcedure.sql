...
CREATE PROCEDURE [dbo].[A_004B_UPDATE_DATE_FIELDS_EXE] 
...
-- Step 2/ For each table update the date format
  ...
  DECLARE c CURSOR FOR
		...
		INNER JOIN zzSAP_DDIC..TURKEY_DD03L as ECC ON
		...
		WHERE sys.objects.type = 'U' AND ISC.DATA_TYPE in ('datetime', 'date')
    ...
    BEGIN
        ...
        BEGIN TRY
          ...
          SET @UpdateStatement = 'UPDATE [' + @TableName + '] SET [' + @FieldName + '] = NULL WHERE [' + @FieldName + '] = ''1900-01-01'' OR [' + @FieldName + '] = ''01-01-1900'''
          EXEC(@UpdateStatement)
          PRINT 'Updated table/field ' + @TableName + '-' + @FieldName
  ...
  IF @ErrorCounter = 0  
	BEGIN
		PRINT 'Succesfully updated ' + convert(varchar, @UpdateCounter) + ' date fields with value = 1900-01-01 to NULL'
    END
  ELSE
	BEGIN
		PRINT 'Errors occured while updating date fields with value = 1900-01-01 to NULL'
