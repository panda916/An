...
-- =============================================
CREATE   PROCEDURE [dbo].[TEST_PROGRESS_2]
AS
BEGIN
...
--And start with A_02( which means they are Template sheet)
	...
	WHERE TABLE_NAME LIKE 'A_02_FY20%'
...
--Step 2.1 Create a list of variable 
	...
	EXEC SP_REMOVE_TABLES 'B_05_IT_%'
...
-- DO the loop for each table from the list
	...
	SET @tableVar2=(SELECT REPLACE(@tableVar1,'A_02_','')  FROM TT_TABLE_LIST WHERE number=@NUM)
	SET @quarter=(SELECT SUBSTRING(REPLACE(TABLE_NAME,'A_02_',''),1,CHARINDEX('_',REPLACE(TABLE_NAME,'A_02_',''))-1) FROM TT_TABLE_LIST WHERE number=@NUM)
	SET @quarter1=(SELECT CONCAT(SUBSTRING(TABLE_NAME,6,4),'_',SUBSTRING(TABLE_NAME,10,3))FROM TT_TABLE_LIST WHERE number=@NUM)
	SET @FY=(SELECT SUBSTRING(REPLACE(TABLE_NAME,'A_02_',''),1,4) FROM TT_TABLE_LIST WHERE number=@NUM)
	...
	* INTO TEMP FROM [' + @tableVar1 +']'
	...
	EXEC 
		...
		'''+@quarter1+''' AS Quarter,
		'''+@FY+''' AS Fiscal_year
		INTO B_05_IT_'+ @tableVar2 +' FROM TEMP 
...
--Step 2.3 Get the new table name list
	...
	FROM INFORMATION_SCHEMA.TABLES 
	WHERE TABLE_NAME LIKE 'B_05_IT_%'
-- STep 2.4 Append the table together, base on the list of tables
	EXEC SP_DROPTABLE B_07_TT_SOX_TEST_FINAL
	...
	SET @SQL=STUFF( @SQL,8,1,'* INTO B_07_TT_SOX_TEST_FINAL ')
...
--And start with A_02( which means they are Template sheet)
	...
	INTO TT_TABLE_LIST
	FROM INFORMATION_SCHEMA.TABLES 
	WHERE TABLE_NAME LIKE 'A_02_FY21%' OR TABLE_NAME LIKE 'A_02_FY22%'
...
--Step 5.1 Create a list of variable 
	...
	EXEC SP_REMOVE_TABLES 'B_08_IT_%'
	...
	* INTO TEMP FROM [' + @tableVar1 +']'
	...
	'''+@quarter1+''' AS Quarter,
	'''+@FY+''' AS Fiscal_year
	INTO B_08_IT_'+ @tableVar2 +' FROM TEMP 
	WHERE number >2 AND F1 IS NOT NULL AND F4 IS NOT NULL
	'
	)
	PRINT(@NUM)
...
--Step 5.2 Get the new table name list
	...
	INTO TT_TABLE_LIST_1
	FROM INFORMATION_SCHEMA.TABLES 
	WHERE TABLE_NAME LIKE 'B_08_IT_%'


--Step 5.3 Append the table together, base on the list of tables
	EXEC SP_DROPTABLE B_09_TT_SOX_TEST_FINAL
	...
	SET @SQL=STUFF( @SQL,8,1,'* INTO B_09_TT_SOX_TEST_FINAL ')
	Exec(@SQL)


--Step 6
--Update the text, so it can match with the mapping file
--Sometimes the text is not matching whith the mapping file
--WHich might cause the field empty

--FY20
	UPDATE B_07_TT_SOX_TEST_FINAL
...
--Step 7 Join FY21 and FY20 together
	...
	(
		...
		CASE WHEN Quarter_checking='FY20January08' THEN 'FY20_Jan/1st'
			...
			-- June 2020
			WHEN (Origi LIKE '%04062021' ) AND Origi LIKE '%FY20%' THEN 'FY20_June/1st'
			...
			WHEN (Origi LIKE '%21062022' ) AND Origi LIKE '%FY21%' THEN 'FY21_June/Final'
			 ...
			 ELSE Quarter END as 'Month_Week'
		FROM  B_07_TT_SOX_TEST_FINAL AS A
		LEFT JOIN AM_FY20_MAPPING 
		ON TRIM([SOX Region/ Business]) = TRIM([Office / Segment]) AND TRIM([SOX Operating Company]) = TRIM([SOX Entity])
	--WHERE 
	...
	SELECT * FROM
	(
	SELECT B_09_TT_SOX_TEST_FINAL.*,Business,
	CASE WHEN [Testing Phase] IN ('1. Walkthrough' , '2. Management testing' , '4. Others') THEN 'Sony'
			WHEN [Testing Phase]='3. PwC' THEN 'PwC'
			END AS 'Testing Phase_Group',
...
--FY21 has some entity and text rename
	...
	UPDATE B_10_IT_DEF_TEST_FINAL
	SET [SOX Operating Company]='STT'
	WHERE [SOX Operating Company]='STTB'
